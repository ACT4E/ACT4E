...
\begin{gradedexercise}[\exname{MonotoneMaps}]\label{ex:MonotoneMaps}
 (6 points) Consider the posets $\tup{\posA, \leq_\posA}$ and $\tup{\posB, \leq_\posB}$, described respectively by the following Hasse diagrams. 
...
(2 points) Let $\tup{\posA, \leq_\posA}$ be a poset and let $\Bool$ denote the set $\{ \false, \true \}$ equipped with the partial order such that $\false \leq \true$. Let $\mapa : \posA \to \Bool$ be a monotone map and consider the set 
...
(6 points) The following is a string diagram which can be interpreted to represent a composition of morphisms (having certain types) in any given monoidal category $\tup{\CatC,\mtimescat,\idmoncat}$. The resulting morphism is one of the type $\Obja \to \Objc$. 
