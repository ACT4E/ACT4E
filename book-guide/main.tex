...
You will also need \verb|git| to work with the book.Install it from \href{https://git-scm.com/book/en/v2/Getting-Started-Installing-Git}{here} or visit \url{https://git-scm.com/book/en/v2/Getting-Started-Installing-Git} \& follow the instructions for your operating system.
...
\end{bashcode}
You should now have a folder called $\verb|ACT4E|$ in your initial folder. In our example, you would have the folder $\verb|ACT4E|$ inside the folder $\verb|ACT|$. If you do not, you have not pulled from the repo. Please try again.
...
\end{bashcode}
\end{itemize}
You should now have the $\verb|ACT4E-solutions|$ folder inside the $\verb|ACT4E|$ folder. It's a bit of a mess, but you should have it somewhere in there.

\section{Compiling \& working with the book}
To interact with the book you \textbf{must} have Docker open. Just click on the icon, and let the app open. It should say "Docker is running" once it's done. It may take a few minutes to get started. You can now \textbf{from the} \verb|ACT4E| \textbf{folder} type into the terminal
\begin{bashcode}
    make shell
\end{bashcode}
This should modifiy your terminal a bit, and your terminal will look like figure \ref{fig:enter-label}.
\begin{remark}
    We will use the computer science terminology of 'root folder' or just 'root' to designate where the repository has been cloned. This is the folder called $\verb|ACT4E|$.
\end{remark}
\begin{figure}[h]
    \centering
...
\end{bashcode}
and swap out the \verb|10_arrows|, \verb|10_cats| to whatever chapter you are working on. If no errors are flagged, the last line of the output of the terminal should be 
\begin{bashcode}
...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        \item have typed \verb|make shell| in the root repo. Working from your basic shell will not work.
    \end{enumerate}
\end{remark}
Now, \textbf{keep the terminal open} and just edit the book in your editor of choice, and when you will hit \verb|Ctrl+S|, the changes will be automatically detected and the chapter will be recompiled. Obviously, you'll have to close your PDF viewer, wait for compilation to be done, and reopen once it's done to see the changes. 
...
The \verb|ACT4E| repo has a bit of a complex structure. Here's a quick breakdown of what is where (only relevant folders for TAs).
\begin{itemize}
    \item \verb|volumes/vol1| contains the files for the current version of the book. The sub-folders within are not chapters, but 'meta' chapters (or categories!). It's the folder within that is actually a chapter (which you can compile). E.g. \verb|30_design/15_poset_bounds| is a chapter, as you can also find the \verb|chapter.tex| inside of it.
    \item \verb|sag| contains \verb|.tikz| files and their compiled PDFs that we use for the book
\end{itemize}
The ACT4E-solutions repo is for the solutions to the homework exercises. Each \verb|.tex| file is a solution we use for corrections. Please don't touch them unless asked. There are also folders containing the homework sheets/exams of past \& current iterations of the course.
...
\begin{bashcode}
    git branch
\end{bashcode}
The above will display the various branches you've opened on your computer. Not all the ones on the repo. The branch \verb|alhubel-prod| is the publicly available version of the book, and \verb|alphubel-stage| is where all the changes are sent to, before being double-checked and added to \verb|alphubel-prod|.
\subsubsection{Before you start editing}
Open the terminal in the root repo, and type 
\begin{bashcode}
    git pull 
\end{bashcode}
This will just check that you have the latest version of the branch you're on. You can skip this step if you're not pulling from \verb|alphubel-stage|. Next, check that everything is okay by typing
\begin{bashcode}
    git status
\end{bashcode} 
...
\begin{remark}
    To improve efficiency, PDF files are neither pushed nor pulled, thanks to the \verb|.gitignore| files (you don't need to worry about that, though).
\end{remark}

\section{Exercises}
Generally, in the book, there are two kinds of exercises: “regular” exercises and “graded” exercises. The regular ones have solutions that are written directly in the \verb|.tex| files of the book, while the graded exercises have solutions written in a separate \verb|.tex| file that is in the ACT4E-solutions repo. The graded exercises are used both in the book, and also for homework sheets that are created each semester for the teaching at ETH.
\end{document}
