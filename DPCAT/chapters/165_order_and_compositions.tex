...
\begin{lemma}
Given~$f_1,f_2\in \Hom_\DP(\F{C}\times \F{A},\R{C}\times \R{B})$ with~$f_1\ordgeq_\DP f_2$, one has~$\Tr_{\F{A},\R{B}}^C(f_1)\ordgeq_\DP \Tr_{\F{A},\R{B}}^C(f_2)$, i.e., trace (\cref{def:dp-trace}) preserves order on~$\DP$.
\end{lemma}
