...
To say that a category $\CatC$ is \emph{compact closed} is, roughly, to say that every object $A \in \CatC$ has a dual object $A^* \in \CatC$, and that, for any $B \in \CatC$, $A$ and $A^*$ satisfy a unique formula for the internal hom: $[A,B] = A^* \times B$. In $\DP$, the dual of a poset $\F{A} \in \DP$ is $\F{A}\op$, and the formula for the internal hom is $\F{A}\op \times \R{B}$.
...
\end{lemma} 
\begin{proof}
Note that we have already proved (\cref{lemma:symmetricmonoidaldp}) that $\tup{\DP,\otimes,\singleton,\sigma}$ is a symmetric monoidal category. Define the unit $\eta_P\colon \F{\singleton} \tickar \R{P}\op \times \R{P} $ as
\begin{equation}
...
Define the counit $\epsilon_P\colon \F{P\op} \times \F{P} \tickar \R{\{1\}} $ as
\begin{equation}
\begin{aligned}
...
In the simplest case, fix some $\text{engine} \in \text{Engines} \coloneqq \Hom_\DP(\F{\text{Thrust}},\R{\text{Fuel}})$. Since $\DP$ is compact closed, we can rewrite the R\&D design problem $\text{R\&D}(\F{\text{engine}}^*, \tup{\R{t},\R{m}})$ from \cref{ex:r&d} as a design problem of the form $\One \tickar \R{\text{Time}} \times\R{\text{Money}}$ (\cref{fig:excompactclosed}).
...
\end{figure}
where $\eta \colon \F{I} \tickar \R{\text{Thrust}}\op \times \R{\text{Thrust}}$ is the \emph{unit} design problem described in \cref{eqn:unit_morphism} and the dependent design problem `R\&D $\mid$ engine' is defined by
\begin{equation}
\begin{aligned}
(\text{R\&D } \mid \text{ engine}) \colon (\F{\text{Thrust}}\op \times \F{\text{Fuel}})\op \times (\R{\text{Time}} \times \R{\text{Money}}) &\toinPos \Bool \\
\tup{\tup{\F{h}^*, \F{f}}^*, \tup{\R{t},\R{m}}} &\mapsto \text{R\&D}(\text{engine}, \tup{\R{t},\R{m}}).
\end{aligned}
\end{equation}
...
The companion functor $\comp{(\cdot)}\colon\Poset\to\DP$ preserves the 2-structure; i.e.\ if $f\Imp g$ as monotone maps $\cP \to\cQ $, then $\comp{f}\geq_\DP\comp{g}$ as profunctors $\F{\cQ} \tickar\R{\cP} $. In fact, it is locally fully faithful (\cref{def:functorfullfaith}): $f\Imp g$ iff $\comp{f}\geq_\DP \comp{g}$.
\end{proposition}
\begin{proof}
