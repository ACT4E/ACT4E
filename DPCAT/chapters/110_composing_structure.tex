...
\end{equation}
We represent it by an arrow $\adp \colon \F{F} \tickar \R{R}$. Note that a design problem is a boolean profunctor.
\end{definition}
...
We represent design problems using a diagrammatic notation. One design problem $\adp \colon \F{F} \tickar \R{R}$ is represented as a box with functionality $\F{F}$ on the \emph{left} and resources $\R{R}$ on the \emph{right} (\cref{fig:diagrammaticdp}).
\begin{figure}[h!]
\begin{center}
...
\end{center}
\caption{Diagrammatic representation of the composition of design problems. \label{fig:compositiondiagram}}
\end{figure}
...
\begin{proof}
 ...
 because it is a composition of monotone maps. Because~$f$ represents a design problem, $f(\F{a}^*,\R{b})$ is monotone in~$\F{a}^*$,
 and similarly $g(\F{b}^*,\R{c})$  is monotone in $\R{c}$. The conjunction ``$\wedge$'' is monotone in both variables, and likewise the $\vee$ operation.
 \end{proof}
...
\end{center}
\caption{Composition is associative. \label{fig:compositionassociativity}}
\end{figure}
...
\begin{lemma}
    The composition operation satisfies the left and right unit laws (\cref{fig:compositionunital}).
\begin{figure}[h!]
\begin{center}
...
\begin{equation}
    \id_A \then f\equiv f \equiv f\then \id_B.
\end{equation}
...
\end{equation}
This holds iff $\id_A(\F{a}^*,\R{a'})\wedge f(\F{a'}^*,\R{b}) \leq f(\F{a}^*,\R{b})$ for some $a'\in A$. If there is no such $a'$, then the inequality holds ($\false \leq \false$ and $\false \leq \true$). If there is such an $a'$, it means than $\id_A(\F{a}^*,\R{a'})=\true$ and $f(\F{a'}^*,\R{b})=\true$. We know that $\id_A(\F{a}^*,\R{a'})=\true \Leftrightarrow \F{a}\leq \R{a'}$, and hence $f(\F{a}^*,\R{b})=\true$.
\end{proof}
