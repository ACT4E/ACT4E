...
example, if the arrows $\mathsf{engine}$ and $\mathsf{wheel}$ \JL{the order here is the opposite as in the diagram} exist, then also the arrow ``$\mathsf{engine}$ then $\mathsf{wheel}$''
exists~(\cref{fig:e4}).
...
and the rest of the diagram is implied (\cref{fig:e5}). \JL{we refer to an $f$ and $g$ here, but they are nowhere to be seen...}
...
\GZ{Use semicolon for all the compositions once defined}
\begin{figure}[h!]
    \centering
...
In this section, we introduce an example: we define a category of currency exchangers $\mathbf{Curr}$. Our idea is to model currencies as objects of this category, and morphisms will describe ways of exchanging between currencies, \text{e.g.} as offered by a currency exchange service. 
...
To build our category $\mathbf{Curr}$ we also need to specify how currency exchangers compose. Given currencies $\mathsf{c_1}, \mathsf{c_2}, \mathsf{c_3}$, and given currency exchangers
$$
...
\begin{definition}[Category of currencies $\mathbf{Curr}$]
    A \emph{category of currencies} $\mathbf{Curr}$ is specified by:
    \begin{compactenum}
        \item \emph{Objects:} $\Ob_\mathbf{Curr}$ is a collection of currencies.
        \item \emph{Morphisms:} given two currencies $\mathsf{c_1},\mathsf{c_2}\in \Ob_{\mathbf{Curr}}$, morphisms between them are currency exchangers $\tup{a,b}$ from $\mathsf{c_1}$ to $\mathsf{c_2}$.
        \item \emph{Identity morphism:} given an object $\mathsf{c} \in \Ob_\mathbf{Curr}$, its identity morphism is the currency exchanger $\tup{1,0}$. We also call such morphisms ``trivial currency exchangers''. 
...
\begin{exercise}
Check that the above definition does indeed define a category: do the associative law and the law for identity morphisms hold?
\end{exercise}

\begin{exercise}
Consider the following graphs. For each graph $G$, how many morphisms in total are there in the associated category $\mathbf{Free}(G)$?

[INSERT EXAMPLES, maybe 4 of them]
\end{exercise}
