...
\todo[inline]{
Add remark here explaining the
relation between $;$ and $\circ$.
}
...
\begin{table}[h]
    ...
    \end{tabular}
    \caption{Three currency exchange companies operating different currencies. \AC{Let's put numbers}
    \AC{Give name to the companies}
    }
...
\begin{definition}[Composition of currency exchangers]
...
\AC{The category Curr represents the set of all possible currency exchangers that could
ever exist. However, in this set there would be very irrational agents. For example, there is a currency exchanger that given 1 USD, will give you back 2 USD.}

\AC{This highlights a recurring topic: often mathematicians will be happy to 
define a broader category of objects, while, in practice, the engineer
will find himself thinking about a more constrained set of objects.}

\AC{Let's make a particular example. Suppose that we have three currency exchange 
companies, which operate on the following currencies:}

\AC{(Three tables; each table shows each row = a morphism). Each row
is labeled as "op1", "op2", .../.}

\AC{We can represent this information as a graph, where the nodes are the
currencies and the edges are particular exchange operations.}

\AC{(figure here)}

\AC{As displayed, this is not a category. In fact, what's missing is...}

\AC{We can make it into a category by computing its closure...}

\AC{(Explain closure operation)}

\AC{(Show how we can compute the homsets recursively. Show 
that the homsets contain zero conversion (you don't get any money back)}

\AC{(Say that one would be interested in finding the best conversions anyway.
These conversions are supposed to be pareto fronts (we have a,b). }

\AC{(Note, don't write: if bias is 0, then take the log of conversion rates, and they add: solve using Dijkstra.)}

\AC{Prove: we only need to iterate a finite number of times. Sketch: 
Optimal path would never pass through a currency multiple time. 
(Valid with some slight assumption on the currency exchangers: you will
always lose changing back and forth the same currency)}
