...
\begin{compactenum}
\item \emph{Objects}: a collection\footnote{A ``collection'' is something which may be thought of as a set, but may be ``too large" to technically be a set in the formal sense. This distinction is necessary in order to avoid such issues as Russel's paradox.} $\ObC$, whose elements are called \emph{objects}.
