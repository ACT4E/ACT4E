...
\begin{definition}[Enriched category]
  ...
  Let~$\tup{\CatB,\mtimescat, \idmoncat, \associator, \leftunitor,\rightunitor}$ be a monoidal category.
  A category~$\CatC$ \emph{enriched} in~$\CatB$ is composed of:
  \begin{compactenum}
    ...
    \item For all~$\Obja, \Objb, \Objc\in \Ob_\CatC$, there exists a morphism~$\stylemorph{m}_{\Obja,\Objb,\Objc}$ in~$\CatB$:
    \begin{equation}
    ...
    \item For each~$\Obja\in \Ob_\CatC$, a morphism~$\stylemorph{j}_\Obja\colon \idmoncat \mto \HomSet{\CatC}{\Obja}{\Obja}$, called \emph{identity element}.
  \end{compactenum}
...
\end{equation*}
  \begin{equation*}
    \label{eq:upos_lat_5}
    \bot_{\UPos(\F{A},\R{B})}\colon \F{a}\mapsto \emptyset
\end{equation*}
    \begin{equation*}
    \label{eq:upos_lat_6}
...
\begin{gradedexercise}[\exname{DPIsEnrichedInBoundedLat}]\label{ex:DPIsEnrichedInBoundedLat}
Show that the category of design problems (objects are posets and morphisms are $\Bool$-profunctors) can be viewed as a category enriched in the monoidal category of bounded lattices.

We recall that a bounded lattice is a lattice $\tup{\posA,\ordleq,\join,\meet}$ which has a top element $\top$ (that means: $\ela \leq \top \ \forall \ela \in \posA$) and a bottom element $\bot$ (that means: $\ela \geq \bot \ \forall \ela \in \posA$).

Given bounded lattices $$\styleobj{L_1} = \tup{\posA_1,\ordleq_1,\join_1,\meet_1, \top_1, \bot_1}$$ and $$\styleobj{L_2} = \tup{\posA_2,\ordleq_2,\join_2,\meet_2, \top_2, \bot_2}$$ 
a morphism $\mora : \styleobj{L_1} \mto \styleobj{L_2}$ is a morphism of lattices which additionaly satisfies the equations
$$\mora (\top_1) = \top_2 \quad \text{and} \quad \mora(\bot_1) = \bot_2.$$

The monoidal product $\styleobj{L_1} \times \styleobj{L_2}$ of bounded lattices is defined a straightforwardly: the underlying set of $\styleobj{L_1} \times \styleobj{L_2}$ is $\posA_1 \times \posA_2$, and all other operations and constants are defined ``componentwise''. 
\end{gradedexercise}

\begin{gradedexercise}[\exname{ProductOfEnrichedCats}]\label{ex:ProductOfEnrichedCats}
