...
Show that the category of design problems (objects are posets and morphisms are $\Bool$-profunctors) can be viewed as a category enriched in the monoidal category of lattices.
...
The monoidal product $\styleobj{L_1} \times \styleobj{L_2}$ of bounded lattices is defined straightforwardly: the underlying set of $\styleobj{L_1} \times \styleobj{L_2}$ is $\posA_1 \times \posA_2$, and all other operations are defined ``componentwise''. The monoidal unit in this category is any choice of 1-element lattice. 
\end{gradedexercise}
