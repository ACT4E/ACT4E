...
The arrow construction of the previous section admits a useful variant, in which one of the two connecting morphisms is reversed---``twisted''.
Objects are still the morphisms of~\CatC, but a morphism from~$\mora$ to~$\morb$ now consists of a morphism between sources going \emph{backwards} and a morphism between targets going forwards.
One intuition: a morphism of the twisted arrow category witnesses that~$\morb$ ``extends''~$\mora$, in the sense that~$\mora$ occurs as the middle portion of~$\morb$, with some pre-processing prepended and some post-processing appended.
This construction appears naturally when studying tradeoffs and interval-like structures, as the example below illustrates.
\begin{ctdefinition}[Twisted arrow category]
    ...
    \begin{enumerate}
        ...
        \item \emph{Composition:}
              Composition in~$\TwistedArrow\CatC$ is given by placing \SY{commutative squares} side by side.
              Consider~$\tup{\twarrowmorasource,\twarrowmoratarget} \colon \mora \mto \morb$ and~$\tup{\twarrowmorbsource,\twarrowmorbtarget} \colon \morb \mto \morc$ in~$\TwistedArrow\CatC$, giving rise to the following composite diagram
              ...
              \begin{equation}\label{eq:twarrow-cat-composition}
                  \tup{\twarrowmorasource,\twarrowmoratarget}\mthenof{\TwistedArrow \CatC} \tup{\twarrowmorbsource,\twarrowmorbtarget}
...
\devel{
    \begin{remark}
        Recall \cref{sec:posets-as-cats} and note that the map which sends a \SY{poset} (a category) to its twisted arrow category is a \SY{functor}, which sends objects of the poset
