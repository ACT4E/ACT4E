...
\begin{ctdefinition}[Free category on a graph]
    ...
    The \emph{free category on~$\graphA$}, denoted~$\Free(\graphA)$, has as objects the vertices~$\vertices$ of~$\graphA$, and given vertices~$\vertexa\setin \vertices$ and~$\vertexb\setin \vertices$, the morphisms~$\HomSet{\Free(\graphA)}{\vertexa}{\vertexb}$ are the paths from~$\vertexa$ to~$\vertexb$.
...
\begin{widepar}
    \begin{gradedexercise}[\exname{HowManyMorphisms}]
        ...
        For each graph~$\graphA$, how many morphisms in total are there in the associated category~$\Free(\graphA)$?
        \begin{equation}
