...
\begin{equation}
    \begin{aligned}
        ...
        f                   & \mapsto \Min \{ r \setin R \colon \adp(f^*, r) = \true \}.
    \end{aligned}
\end{equation}
...
% We denote the smallest upper set in $P$ containing $p$ by $\upit(p) = \{ x \setin P : p \leq x \} \setin \uppersets P$.
...
For example, take the two distinct upper sets $S_1 = \emptyset$ and $S_2 = \{x \setin \reals : x > 0\}$.
...
\begin{proof}
    ...
    Then $A$ generates an upper set $\upit A = \{ x \setin P : a \leq x \text{ for some $a \setin A$} \}$, and $\Min \upit A = A$.
    ...
    Since $U$ is downward-closed, $\Min U$ is a non-empty antichain given by $\{ a \setin \underline{U} : a \leq x \text{ for all $x \setin \underline{U}$} \}$, and $\upit \Min \underline{U} = \underline{U}$.
\end{proof}
...
\begin{equation}
    \begin{aligned}
        \ftor_\adp \colon F & \to \antichains R \\
        f                   & \mapsto \Min \then \{ r \setin R \colon \adp(f^*, r) = \true \}
    \end{aligned}
\end{equation}
...
\begin{proof}
    ...
    \todotext{ \AC{This is not really a proof, it is just a statement of facts without proofs.} \JT{Hmm I thought I was copying a proposition in your original paper but now that I go back I can't find it.
            ...
            The issues are the $\Min$ in the composition and the coproduct in the cases with an infinite antichain.
        }}
...
\begin{equation}
    \begin{aligned}
        ...
        S                                             & \mapsto \Min \bigsetunion_{\tup{b,c} \setin S} \{ \tup{b',c'} \setin \ftor_\adp(b,a) \colon b' \leq b \}.
    \end{aligned}
\end{equation}
%\begin{align*}
%\psi_{a,\adp} : \antichains (B) &\to \antichains (B) \\
%S &\mapsto \Min \bigsetunion_{b \setin S} \{ b' \setin B : \langle b',c \rangle \setin h_\adp(b,a), b' \leq b \}.
%\end{align*}
...
\begin{equation}
    \begin{aligned}
        ...
        r          & \mapsto \Min \; \{ f^* \setin F\op : \adp(f^*, r) = \true \}
    \end{aligned}
\end{equation}
