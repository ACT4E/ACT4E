...
The fact that \DP is compact closed, which we proved in~\XXX, will help us describe and solve these optimization problems. \todotext{that is now moved in playground}
...
%         But since $S$ is an upper set, every $s \in S$ satisfies $s \geq k$ for some $k \in K$, so $s \in \upit K$; on the other hand this is the same definition of a typical element $k \in \upit K$.
%     }
...
\begin{enumerate}
    \item First, calculate $\ftor_\adp$ for some fixed, initial $\langle b_{\text{in}},a \rangle$.
          The goal is to compute the antichain of minimal elements $c \in C$ which provide $a$, across \emph{all} possible choices of $b_{\text{in}}$.
