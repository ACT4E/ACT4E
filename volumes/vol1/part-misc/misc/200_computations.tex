...
\begin{proof}
    Recall that a \SY{monotone function} is \emph{Scott-continuous} if it preserves all directed suprema, \ie,  if $D$ is a directed subset and $D^\upit$ is its supremum, then $f(D^\upit) = f(D)^\upit$.
...
\begin{proof}
    ...
    The endofunctor $F$ is given by the recursion operator $\psi_{a,\adp} : \antichains (B\times C) \to \antichains (B\times C)$, and by the previous lemmas, $\psi_{a,\adp}$ is \SY{Scott-continuous}, thus preserves suprema.
