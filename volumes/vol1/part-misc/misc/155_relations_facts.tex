...
\begin{example}
    ...
    ''
\end{example}
\begin{definition}[Biproduct of design problems]\label{def:biproduct-dp}
...
Specifically, we need to show that~$A + B$ is both a product~$\tup{+, \pi_A, \pi_B}$ and a coproduct~$\tup{+, \iota_A, \iota_B}$ in~\DP, and satisfies an extra coherence condition:~$\pi_B \circ \iota_B = \relid_B$.

\begin{ctdefinition}[Initial and terminal object]\label{def:initial-terminal-object}
...
\begin{ctdefinition}[Finite coproducts and products]\label{def:finite-products}
...
\begin{proof}
    Let~$\inj_\setA$ be the injection of posets~$\setA \sto \setA\setdisunion\setB$.
    ...
    To show that~$A+B$ is a coproduct (of posets) in~\DP, we need to show that the coproduct~$f \sqcup g$ of design problems is unique and that it satisfies~$f = \comp{\iota_A} \mthen (f \sqcup g)$ and~$g = \comp{\iota_B} \mthen (f \sqcup g)$.
...
\begin{ctdefinition}[Zero object and morphism]\label{def:zero-object}
...
\begin{proof}
    ...
    To check the second condition, we compute an explicit expression for~$(\iota_A\mthen \pi_B)\colon \F{A} \profto \R{B}$, using the definition of design problem series composition:
    \begin{equation}
        \begin{aligned}
            (\iota_A\mthen \pi_B)
