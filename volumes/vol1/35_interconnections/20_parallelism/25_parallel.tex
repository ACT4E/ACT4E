...
\begin{remark}
    ...
    \begin{itemize}
        \item Using the strictification construction of \SetL most of the monoidal categories are strict; we can ask that the composition induces semigroups of morphisms.
              ...
              In fact, we do not need to define natural transformations. \todotext{J: I'm not understanding what this comment is trying to say.}
...
\begin{example}[\SetL is an associative stacking semicategory]
    ...
    \todotext{@J: this example needs to be clarified/expanded}
\end{example}
...
Fix a set ~$\EfW$ of all possible worlds. The category $\Effects=  \Effects_\EfW$ is defined by the following:
    \begin{itemize}
...
\todotextjira{639}{the representation in (b) in the figure above is incorrect I believe -- W on the left-hand side should be at the bottom}
...
\todotext{J: in the figure (a) above, I think we should have boxes around the stacked morphisms before the are composed serially}
...
\todotextjira{640}{the representations in \cref{fig:effects7} and \cref{fig:effects9} are incorrect -- W on the left-hand sides should be at the bottom}

\section{Symmetric stacking}

Let $\CatC$ be a stacking category, and consider morphisms 
\begin{equation*}
...
Let~$\CatC$ be a stacking semicategory. For each natural number $n \geq 2$ and each permutation $\sigma \in S_n$, we define an operation $P_\sigma: \Obj_\CatC \to \Obj_\CatC$ with
\begin{equation}
P_\sigma: \Obja_1 \mtimescatob \dots \mtimescatob  \Obja_n \maptso \Obja_{\sigma(1)} \mtimescatob \dots \mtimescatob  \Obja_{\sigma(n)} 
\end{equation}

\begin{ctdefinition}[Symmetric stacking category]
    \label{def:symmetric-stacking-category}
A symmetric stacking semicategory is a stacking semicategory $\CatC$ with 
