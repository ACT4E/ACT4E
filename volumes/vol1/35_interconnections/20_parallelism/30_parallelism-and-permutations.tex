...
\begin{definition}\label{def:presentation-of-a-prop}
    A \emph{presentation} $\tup{\stylesets{G}, s, t, \stylesets{R}}$ of a prop is a set $\stylesets{G}$, source and target functions $s, t \colon \stylesets{G} \mto \natnumbers$, and a relation $\stylesets{R} \subseteq E(\stylesets{G}) \cartprod E(\stylesets{G})$ between expressions such that for any $\tup{e_1, e_2} \setin\stylesets{R}$, the expressions $e_1$ and $e_2$ have the same arity.
