...
% }
 ...
 \label{sec:symmetric-stacking-categories}
    ...
    \begin{ctdefinition}[Symmetric strict monoidal category]
        ...
        A \maindef{symmetric strict monoidal category} is a \SY{strict monoidal category} $\tup{\CatC, \mtimescat, \idmoncat}$ which is also a symmetric stacking category.
...
\begin{equation}
\relA \oplus \relB = \makeset{\tup{\tup{v,v'},\tup{w,w'}} \mid \tup{v,w} \in \relA \text{ and } \tup{v', w'} \in \relB }.
\end{equation}
...
Let $\stylesets{G}$ be a set and $s, t \colon \stylesets{G} \mto \natnumbers$ functions which specify the source and target of elements of $\stylesets{G}$. The set $E(\stylesets{G})$ of $\stylesets{G}$\emph{-generated prop expressions} (or just \emph{expressions}) is defined inductively as follows.
\begin{itemize}
...
\item The generators $g \in \stylesets{G}$ are expressions $g \colon s(g) \mto t(g)$.
...
\end{itemize}
The functions $s$ and $t$ are extended from $\stylesets{G}$ to $E(\stylesets{G})$ in the obvious way. If $\mora \colon m \mto n$ is an expression, the pair $\tup{\styleobj{m}, \styleobj{n}}$ is called its \emph{arity}.
\end{definition}


\begin{definition}\label{def:presentation-of-a-prop}
A \emph{presentation} $\tup{\stylesets{G}, s, t, \stylesets{R}}$ of a prop is a set $\stylesets{G}$, source and target functions $s, t \colon \stylesets{G} \mto \natnumbers$, and a relation $\stylesets{R} \subseteq E(\stylesets{G}) \cartprod E(\stylesets{G})$ between expressions such that for any $\tup{e_1, e_2} \in \stylesets{R}$, the expressions $e_1$ and $e_2$ have the same arity.

The prop $\CatC$ \emph{presented} by $\tup{\stylesets{G}, s, t, \stylesets{R}}$ is the prop whose morphisms are elements of $E(\stylesets{G})$ quotiented by the equations (or "relations") $e_1 = e_2$ for $\tup{e_1, e_2} \in \stylesets{R}$, and by the axioms for symmetric strict monoidal categories.
\end{definition}
