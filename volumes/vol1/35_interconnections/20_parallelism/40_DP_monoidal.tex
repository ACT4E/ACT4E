...
\devel{
    ...
    We define a category similar to \SetL, but its objects are lists of \emph{posets}.
    \begin{definition}[The category~$\PosL$]
        ...
        \begin{enumerate}
            ...
            \item \emph{Morphisms:}
                  A morphism from~$\posA =\Tupcatt {\posA_1}  \dots {\posA_n}$ to~$\posB = \Tupcatt {\posB_1} \dots {\posB_m}$ is a monotone function:
                  \begin{equation*}
            ...
            \item \emph{Identities:}
                  The identity morphism on an object~$\Tupcatt {\posA_1} \dots {\posA_n}$ is given by the identity map~$\catid_{{\posA_1} \Ctimes (\posA_2 \Ctimes (\ldots \Ctimes {\posA_n}))}$.
        \end{enumerate}
    \end{definition}

    \todo{Should we do the constructions we did for \SetL ?}

    \todo{not sure about the following two}
