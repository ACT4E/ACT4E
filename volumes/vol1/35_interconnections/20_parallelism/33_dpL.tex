...
\begin{definition}[The category \DPL]\label{def:DPL}
    ...
    The category~$\DPL$ is the \SY{subcategory} of \DP where the objects are \SY{posets} of tuples ($\Obof\PosL$).
    % \begin{enumerate}
    %     \item \emph{Objects:}
    %           \SY{posets} of tuples.
    %     \item \emph{Morphisms:}
...
\begin{widepar}
    \begin{proof}
        ...
        %     \begin{aligned}
        %         \adpa\colon \posA\op\Ptimes \posC & \toinPos \Bool, \\
        %         \adpb\colon \posB\op\Ptimes \posD & \toinPos \Bool,
        %     \end{aligned}
        % \end{equation}
        % we define
        \begin{equation}
        ...
        \end{equation}
        Note that this is a valid definition of a \SY{design problem} because the expression $\adpa(a^*,c) \booland \adpb(c^*, d)$ is monotone, being the composition of  \SY{monotone functions}.

        To show associativity, consider three DPs
        \begin{equation}
            \adpa\colon \posA\op\Ptimes \posC \toinPos \Bool, \qquad
            \adpb\colon \posB\op\Ptimes \posD \toinPos \Bool, \qquad
            \adpc\colon \posE\op\Ptimes \posF \toinPos \Bool.
        \end{equation}
        ...
        \end{equation}
        Because the operations $\mtimescatob, \tupconcat, \booland$ are \SY{associative}, we can erase all the light  parentheses in the formulas, and we find that $(\adpa \mtimescatmor  \adpb) \mtimescatmor \adpc$ and
...
%     We know that
%     \begin{equation}
%         \prfcomma{\tup{\posAnel{}\opel,\posBnel{}\opel,\posCnel{},\posDnel{}} \posleqof{\posA\op\Ptimes \posB\op\Ptimes \posC\Ptimes \posD} \tup{{\posAnel{}'}\opel,{\posBnel{}'}\opel,\posCnel{}',\posDnel{}'}}
...
%     \begin{equation}
%         \prfperiod{\adpa\colon \posA\op\Ptimes \posC \toinPos \Bool}
%         {\adpb\colon \posB\op\Ptimes \posD\toinPos \Bool}
%         {\adpa \cproddp \adpb \colon \posA\op\Ptimes \posB\op \Ptimes\posC\Ptimes\posD\toinPos \Bool}
%     \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \adpa\colon \posA\op\Ptimes \posC & \toinPos \Bool \\
            \adpb\colon \posB\op\Ptimes \posD & \toinPos \Bool \\
            \adpc\colon \posC\op\Ptimes \posE & \toinPos \Bool \\
            \adpd\colon \posD\op\Ptimes \posF & \toinPos \Bool
        \end{aligned}
    \end{equation}
