...
\todotext{\alphubel: @JL: Here the poset and the set should have different colors.}
...
In some cases, we will want to not only consider actions (which, by definition are \SY{functors} i nto the category \Set), but also other categories for modeling notions of behavior.
...
\begin{definition}
    \label{def:Beh}
    Given a semicategory \CatC (modeling some systems) and a \SY{semifunctor} $\signals: \CatC \to \Set$ (modeling sets of signals), we define the category~$\Beh_\signals$ of input-output behaviors with respect to $\signals$ as the category where:
    \begin{enumerate}
...
\todotext{J: @JL: explain how the different actions of Moore machines discussed so far fit into this picture}
