...
\todo{ Add initial state}
\todo{Add definition of actions, with time starting at 0}
\todo{Note for matrices with 0 dimension}
\todo{Correct typo with Cf}
\todo{Definition of equivalence of systems}
\todo{Functorial stacking not exercise.
    Explain not functorial stacking but with permutations you get same action.
}
\todo{Lemma: two equiv systems have same action.}
...
\begin{ctdefinition}[Category \LTI]
    ...
    \begin{enumerate}
        ...
        \item \emph{Morphisms}: A morphism in \LTI from~$\styleobj{l}\setin \natnumbers$ to~$\styleobj{m}\setin \natnumbers$.
              Such an arrow is given by a continuous time LTI system, specified as~$\tup{\mat{A},\mat{B},\mat{C},\mat{D}}$.
        ...
        \item \emph{Composition operation}: Given morphisms~$\mora \colon \styleobj{a}\mto \styleobj{b}$ and~$\morb\colon \styleobj{b}\mto \styleobj{c}$, described by the LTI systems~$\tup{\mat{A}_\mora,\mat{B}_\mora,\mat{C}_\mora,\mat{D}_\mora}$ and~$\tup{\mat{A}_\morb,\mat{B}_\morb,\mat{C}_\morb,\mat{D}_\morb}$, their composition is an arrow~$(\mora \mthen \morb)\colon \styleobj{a}\mto \styleobj{c}$, parametrized by the LTI system~$\tup{\mat{A},\mat{B},\mat{C},\mat{D}}$, where
              \begin{equation}
                  \mat{A}=\begin{bmatrix}
                      \mat{A}_\mora              & \mat{0}       \\
              ...
              \end{equation}
        \item \emph{Identity morphism}: The identity morphism for~$\styleobj{l}\setin \natnumbers$ is an arrow~$\styleobj{l}\to \styleobj{l}$, parametrized by the system~$\tup{\mat{0}^{0\times 0},\mat{0}^{0\times l},\mat{0}^{l\times 0},\mathbb{I}^{l\times l}}$.

    \end{enumerate}
\end{ctdefinition}

%describes the transformation of an input sequence~$u\colon \reals \to \reals^l$ into an output sequence~$y\colon \reals \to \reals^m$
