...
%   \begin{equation}
%     \funid_\posgenA = \tup{\funsp,\ressp,\impsp,\prov,\req}
%   \end{equation}
%   as follows:
%   \begin{equation}
...
% Take
% \begin{equation}
% \adp = \tup{\funsp,\posgenA,\impsp,\prov,\req}
% \end{equation}
% We have for the series
% \begin{equation}
%   \impsp =
%   \{
%     \tupcat {\imp_1} {\imp_2} \in (\impsp \setconcat \posgenA)
%       \mid
...
\devel{
    \begin{definition}[Intersection of DPIs]
        ...
        Given two DPIs with same functionality and resources~$\adpa=\tup{\funsp,\ressp,\impsp_{1},\prov_{1},\req_{1}}$ and~$\adpb=\tup{\funsp,\ressp,\impsp_{2},\prov_{2},\req_{2}}$, define their intersection as
        \begin{equation}
            \adpa\sqcap\adpb\definedas\tup{\funsp,\ressp,\impsp_{1}\setintersection\impsp_{2},\prov,\req} ,
        \end{equation}
        where
        \begin{eqnarray}
            \prov & \colon & \imp\mapsto\begin{cases}
                                            \prov_{1}(\imp), & \text{if }\imp\in\impsp_{1}\setintersection \impsp_{2} \text{ and }\prov_1(\imp)\posleq \prov_2(\imp) \\
                                            \prov_{2}(\imp), & \text{if }\imp\in\impsp_{1}\setintersection \impsp_{2} \text{ and }\prov_2(\imp)\posleq \prov_1(\imp) \\
                                            \posbot_\funsp,  & \text{else}.
                                        \end{cases}\label{eq:dppar-exec-2} \\
            \req  & \colon & \imp\mapsto\begin{cases}
                                            \req_{1}(\imp), & \text{if }\imp\in\impsp_{1}\setintersection \impsp_{2} \text{ and }\req_1(\imp)\posgeq \req_2(\imp) \\
                                            \req_{2}(\imp), & \text{if }\imp\in\impsp_{1}\setintersection \impsp_{2} \text{ and }\req_2(\imp)\posgeq \req_1(\imp) \\
