...
\label{sec:spans}
\todostructurejira{277}{@J: Not sure this is the place to talk about spans. Maybe define spans before DP, then reference from DPs?}
...
\begin{example}
    \todotextjira{278}{Bad example. Just arbitrary points of a category, nothing special.
    Instead, do the case of \Berg where you can only go down. Choose a famous peak
    and use as the other two points two cities reachable on either side of the peak.}
...
Precisely, given the two maps~$\prov \colon \impsp \to \funsp$ (an implementation \textbf{prov}ides a functionality) and~$\req\colon \impsp \to \ressp$ (an implementation \textbf{req}uires resources)
one has the span in~$\Set$:
\begin{center}
