...
\begin{definition}
    ...
    \begin{enumerate}
        \item $\Ob_{\CatA\times \CatB}\definedas \Ob_\CatA \times \Ob_\CatB$;
        \item $\HomSet{\CatA\times \CatB}{\tup{\Obja,\Objb}}{\tup{\Obja',\Objb'}}\definedas \HomSet{\CatA}{\Obja}{\Obja'}\mtimescat \HomSet{\CatB}{\Objb}{\Objb'}$, for two objects~$\tup{\Obja,\Objb}$ and~$\tup{\Obja',\Objb'}$ in~$\Ob_{\CatA\times \CatB}$.
    \end{enumerate}
\end{definition}
...
\showslides{
    \begin{forslides}
        ...
        \begin{definition}[Lattice homomorphism]
            ...
            Given two bounded lattices~$\posA, \posB$, a \emph{lattice homomorphism} is a map~$\mora\colon \posA\mto \posB$ which preserves meets, joins, top, and bottom:
            \begin{equation}
                \begin{aligned}
        ...
        \begin{equation*}
            ...
            \mtimescat_\Bool \definedas \wedge
        \end{equation*}
        \begin{equation*}
        ...
        \begin{equation*}
            ...
            \prfdouble{\mora \posleqof\Pos \morb}{\mora(\posAel)\posleqof\posB \morb(\posAel), \quad \forall \posAel \setin \posA}
        \end{equation*}
        \begin{equation*}
...
\begin{example}
    The category \Pos of posets is enriched in \Pos, where the partial order on monotone maps is given by $f \Imp g$ (\ie  for $f,g : A \to B$, $f(a) \leq g(a) \forall a \setin A$).
\end{example}
...
\begin{proof}
    ...
    By assumption, there is some $q\setin Q$ such that $f(\F{p}^*,\R{q})\wedge j(\F{q}^*,\res)=\true$.
...
\begin{proof}
    ...
    \begin{equation*}
        \Hom_A(a_1, a_2) \sto \Hom_B(f(a_1), f(a_2)),
    \end{equation*}
    which can be re-stated as proclaiming the existence of a morphism~$f_{a_1, a_2}$ in \Bool:
    \begin{equation*}
        f_{a_1, a_2} \colon \Hom_A(a_1, a_2) \mtoin{\Bool} \Hom_B\left(f(a_1), f(a_2)\right).
    \end{equation*}
    ...
    \begin{widepar}
        \begin{equation}
            \begin{aligned}
                ...
                \tup{\fun^*, \res}                                                                  & \mapsto \{ \imp \setin \impsp_\adp \colon ( \fun \funleq \prov(i)) \wedge (\req(i) \resleq \res) \},
            \end{aligned}
        \end{equation}
    \end{widepar}
...
\begin{definition}[Series composition of design problems with implementation]
    ...
    and $\tup{\impsp_g,\prov_g,\req_g}\colon \F{B}\profto \R{C}$, we can define their series interconnection
    %
    \begin{equation*}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \tup{i_f,i_g} \setin \impsp_f \times \impsp_g \colon
    ...
    \begin{widepar}
        \begin{equation}
            ...
            \begin{aligned}
                ...
                {} & \tup{\F{a}^*,\R{c}} \mapsto \bigsetunion_{\substack{(b,b') \setin B\op \times B\op \\ b \leq_B b'}} \Bigg[ \tup{\impsp_f,\prov_f,\req_f}(\F{a}^*,\R{b}) \times \tup{\impsp_g,\prov_g,\req_g}(\F{b'}^*,\R{c}) \Bigg].
            \end{aligned}
        \end{equation}
    \end{widepar}
\end{definition}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \tup{i_{f\then g},i_h} \setin \impsp_{f\then g} \times \impsp_{h}\colon
            \req_{f\then g}(i_{f\then g}) \ordleq_{C} \prov_h(i_h)
            \} \\
             & =\{
            \tup{\tup{i_f, i_g},i_h} \setin (\impsp_f \times \impsp_g) \times \impsp_{h}\colon
            (\req_f(i_f) \ordleq_{B} \prov_g(i_g))
            \wedge
...
\begin{example}
    \begin{equation}
        \begin{aligned}
            ...
            \tup{\F{t}^*,\R{f}}                                              & \mapsto \{ e \setin \text{Engines} \colon e(\F{t}^*, \R{f}) = \true \}
        \end{aligned}
    \end{equation}
