...
\begin{ctdefinition}
    ...
    Let \CatE and \CatF be categories enriched in a \SY{symmetric monoidal category}~\CatV.
    Their \emph{product} is a~\CatV-enriched category~$\CatE\Ctimes \CatF$ with:
    \begin{enumerate}
        \item $\Obof{\CatE\Ctimes \CatF}\definedas \Obof\CatE \Ctimes \Obof\CatF$;
        \item For two objects~$\tup{\Obja,\Objb}$ and~$\tup{\Obja',\Objb'}$ in~$\Obof{\CatE\Ctimes \CatF}$,
        \begin{equation}
...
\begin{ctdefinition}[$\CatV$-enriched functor]
    ...
    Let \CatE and \CatF be categories enriched in a monoidal category $\CatV$. A $\CatV$-enriched functor~$\funa\colon \CatE \to \CatF$ is:
...
\item Compatibility with composition: the diagram
 ...
 \item Compatibility with identity-choosing morphisms: the diagram
              ...
              commutes, where~$\mtimescat$ and~$\idmoncat$ are the monoidal product and monoidal unit in \CatV.
