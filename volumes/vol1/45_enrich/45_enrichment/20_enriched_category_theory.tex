    ...
    \item For every pair of objects $\tup{\Obja, \Objb}$ of $\CatE$, a \emph{hom-object}
        ...
        \item For each object $\Obja$ of $\CatE$, a morphism of~\CatV
              ...
              %  called \emph{identity element}.
               ...
               \condit
             \begin{enumerate}
\item Associativity: for any~$\Obja,\Objb,\Objc,\Objd\setin \ObC$, the diagram 
\equationsag{enrichment_assoc2}{eq:enrichment_assoc2}
must commute;
\item Neutrality: for any $\Obja,\Objb,\Objc \setin \ObC$ the diagram 
...
\label{def:opposite_enriched_cat}
    ...
    \begin{enumerate}
        ...
        \item For two objects~$\Obja,\Objb$,
        \begin{equation}
\enhomob{\CatE\op}{\Obja}{\Objb} \definedas \enhomob{\CatE\op}{\Objb}{\Obja};
\end{equation}
\item ... 
\item ...
\end{enumerate}
...
\Obja \mtimescat ( - ) \quad  \adjunction \quad \leftinthom{\Obja}{( - )}
\end{equation}
means that for all $\Objb, \Objc \setin \Ob_\CatC$, we have a natural isomorphism
\begin{equation}
\Obja \mtimescat \Objb \posleq \Objc  \quad \text{if and only if} \quad \Objb \posleq \leftinthom{\Obja}{\Objc},
\end{equation}
...
\label{def:enriched_profunctor}
    Let $\CatE$ and $\CatF$ be categories enriched in a \SY{closed symmetric monoidal category}~$\tup{\CatV, \mtimescat, \idmoncat, \associator, \leftunitor, \rightunitor}$. A $\CatV$-profunctor from $\CatE$ to $\CatF$ is a $\CatV$-functor
    \begin{equation}
