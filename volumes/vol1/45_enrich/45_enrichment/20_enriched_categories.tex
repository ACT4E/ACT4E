...
\begin{ctdefinition}[Enrichment structure]
    ...
    \end{enumerate}
    \equationsag{enrichment_assoc2}{eq:enrichment_assoc2}
    \equationsag{enrichment_unital2}{eq:enrichment_unital2}
\end{ctdefinition}
...
\begin{definition}
    ...
    Let \CatA and \CatB be categories enriched in a \SY{symmetric monoidal category}~\CatV.
...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            \label{eq:cost_cat}
            \CatStyle{Cost}=\tup{[0,\infty], \geq, 0, +}
        \end{equation}
        \begin{equation}
...
\devel{
    ...
    \begin{example}
        ...
        \todojira{375}{\bernina the style is wrong all over the place: no macro has been used here.}
    ...
    \begin{example}
        ...
        Recall that~\Set is a monoidal category with $\mtimesof{\Set}$ equal to the \SY{cartesian product} $\cartprod$.
...
\begin{gradedexercise}[\exname{DPIsEnrichedInPos}]
    ...
    Show that the category of \SY{design problems} (objects are \SY{posets} and morphisms are $\Bool$-profunctors) can be viewed as a category enriched in the monoidal category of \SY{posets}.
    ...
    %a morphism $\mora : \styleobj{L_1} \mto \styleobj{L_2}$ is a \SY{monotone map} which, additionally, satisfies the equations
    ...
    The monoidal product $\styleobj{P_1} \Ctimes \styleobj{P_2}$ of \SY{posets} is defined straightforwardly as the product poset.
