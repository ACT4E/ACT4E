...
\begin{ctdefinition}[Traced monoidal category]
    ...
    We say that a \SY{symmetric monoidal category}~$\tup{\CatC, \mtimescat, \idmoncat, \associator, \leftunitor, \rightunitor, \braiding}$ is \emph{traced} if it is equipped with a a \emph{trace operator}: a family of functions
    \begin{equation}
...
\begin{definition}
For the symmetric monoidal category $\tup{\Rel, \cartprod, \singleton, \associator, \leftunitor, \rightunitor, \braiding}$, a trace is defined as follows. Given a relation
\begin{equation}
...
\begin{definition}
Consider the symmetric monoidal category $\tup{\Rel, \setdisunion, \Emptyset, \associator, \leftunitor, \rightunitor, \braiding}$. In $\Rel$, the disjoint union of any two sets always comes with canonical inclusion relations
    \begin{equation}
        \begin{aligned}
...
and canonical projection relations
    \begin{equation}
        \begin{aligned}
            \proj_\setA\colon & \setA\setdisunion\setB \mto \setA \\
            &  \makeset{ \tup{\disunionA{\setAel}, \setAel}},
        \end{aligned}
\qquad \qquad
        \begin{aligned}
            \inj_\setB\colon & \setA\setdisunion\setB  \mto \setB \\
...
\end{equation}
we can pre-compose it with the inclusion $\inj_\setA \colon \setA \sto \setA\setdisunion\setC$ and post-compose it with the relation $\proj_\setA\colon \setA\setdisunion\setC \sto \setB$ to obtain a relation 
\begin{equation}
\relA_{\setA \setB} \colon \setA \overset{\inj_\setA}{\mto} \setA \setdisunion \setC \overset{\relA}{\mto} \setB \setdisunion \setC \overset{\proj_\setB}{\mto} \setB.
\end{equation}
In an analogous manner we also obtain relations
\begin{equation}
\relA_{\setC \setB} \colon \setA \mto \setB, \qquad \relA_{\setA \setC} \colon \setC \mto \setB, \qquad \relA_{\setC \setC} \colon \setC \mto \setC,
\end{equation}
induced from $\relA$. 

Now we set
\begin{equation}\label{eq:trace-rel-sum}
\Tr_{\setA,\setB}^\setC (\relA) = \relA_{\setA \setB} \cup \bigcup_{n \geq 0} \relA_{\setA \setC} \mthen \relA_{\setC \setC}^n \mthen \relA_{\setC \setB}.
\end{equation}
\end{definition}
