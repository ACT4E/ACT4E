...
\begin{remark}
For a given category $\CatC$, there is in general more than one way to turn it into a \emph{monoidal category} by equipping it with an associative stacking functor and a monoidal unit satisfying the necessary conditions. We therefore sometime refer to this latter data as a ``monoidal structure'' to emphasize that a given category $\CatC$ may be equipped with various different monoidal structures. 
\end{remark}
