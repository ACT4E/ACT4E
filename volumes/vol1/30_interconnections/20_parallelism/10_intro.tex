...
There are various properties that we can consider for categories equipped with an operation of parallel composition.
This leads to a number of definitions; here is a short overview.
%
\begin{enumerate}
    \item \emph{Stacking category}: a \SY{category} in which it is possible to stack two morphisms.
    \item \emph{Functorial stacking category}: a \SY{stacking category} in which the stacking operation is a \SY{functor}.
    \item \emph{Associative stacking category}: a \SY{functorial stacking category} in which the stacking operation is \SY{associative} (either strictly, or up to isomorphism).
    \item \emph{Monoidal category}: an \SY{associative stacking category} in which there is a special object, called the monoidal unit, which is neutral for the stacking operation.
    \item \emph{Symmetric monoidal category}: a \SY{monoidal category} equipped with a symmetric way to ``cross wires''.
\end{enumerate}
