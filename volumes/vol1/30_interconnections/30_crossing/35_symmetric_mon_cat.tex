...
\begin{ctdefinition}[Symmetric monoidal category]
    ...
    A \maindef{symmetric monoidal category} is

    \constit
    \begin{enumerate}
        \item A \SY{monoidal category}~$\tupp{\CatC, \mtimescat, \idmoncat, \associator, \leftunitor, \rightunitor}$,
        \item for any two objects $\Obja, \Objb \setin \Ob_\CatC$, an isomorphism
              \begin{equation}
                  \braiding_{\Obja, \Objb} \colon \Obja \mtimescatob \Objb \mto \Objb \mtimescatob \Obja,
              \end{equation}
    ...
    \begin{enumerate}
        \item \emph{Naturality}: For any morphisms $\mora \colon \Obja \mto \Objc$, $\morb \colon \Objb \mto \Objd$, the diagram
              \begin{equation}\label{eq:naturality-square-symm-stacking-cat-again}
                  \middlesag{naturality-square-symm-stacking-cat}
              \end{equation}
              commutes.
        \item \emph{Hexagon identities:}
              for any ~$\Obja,\Objb,\Objc\setin \ObC$, the following diagrams must commute.
              \equationsag{50_hex_1}{eq:50_hex_1}
              \equationsag{50_hex_2}{eq:50_hex_2}
        \item \emph{Symmetry}: for any $\Obja, \Objb \setin \ObC$,
              \begin{equation}
...
\begin{remark}
    In the presence of the symmetry condition \cref{eq:braiding-symmetry}, the two hexagon identities are actually redundant and only one of them is needed.
    However, if one drops the condition \cref{eq:braiding-symmetry}, then the above (with both hexagon identities) gives the definition of a \emph{braided monoidal category}.
\end{remark}

\begin{remark}
    If $\tup{\CatC,\mtimescat, \idmoncat, \associator, \leftunitor, \rightunitor, \braiding}$ is a \SY{symmetric monoidal category} (or even a \SY{braided monoidal category}), we can show that the following diagram commutes for all~$\Obja \setin \ObC$.
    \equationsag{50_sym_2}{eq:50_sym_2}
\end{remark}

\subsubsection*{\DP as a symmetric monoidal category}
