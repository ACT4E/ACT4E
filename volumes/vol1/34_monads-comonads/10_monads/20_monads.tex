...
\devel{
    ...
    \begin{equation} \label{eq:powersetfunctor}
...
\begin{remark}
    ...
    In terms of components, associativity condition states that for every object~$\Obja \setin \ObC$, the following diagram commutes:
    %
    \equationsag{monad_components_bis}{eq:monad_components_bis}     %
    
    The unitality conditions state that for every object~$\Obja \setin \ObC$, the following diagram commutes:
    %
...
The \SY{powerset} \SY{functor} $\powersetfunctor : \Set \fto \Set$ can be made into a monad when equipped with the following \emph{multiplication} and \emph{unit}.


    We define  $\moncomp \colon \powersetfunctor \fthen \powersetfunctor \nto \powersetfunctor$ and $\monunit \colon \funidof\Set \nto \powersetfunctor$ in terms of components: given an object $\setA \setin \Set$, let
    \begin{equation}
...
Let's fix a one-element set $\makeset{ * }$ and denote it by $\singleton$. We define the ``maybe functor'' $\maybefunctor \colon \Set \fto \Set$ as follows. 

First, let's consider the level of objects. Given any set $\setA$, we define $\maybefunctor (\setA) = \setA \setdisunion \singleton$. 

Next, let's consider the level of morphisms. Given any function $\mora \colon \setA \mto \setB$, we define 
\begin{equation}
\maybefunctor(\mora) \colon \setA \setdisunion \singleton  \mto \setB \setdisunion \singleton 
\end{equation}
to be the function
\begin{equation}
\maybefunctor(\mora) = \mora \funcsum \catid_\singleton.
\end{equation}

This maybe functor is a monad when equipped with the following multiplication and unit, which we define in components. 

Because we are dealing with direct sums of sets, writing things out in terms of elements involves messy notation and cases. We therefore describe the multiplication and unit components in terms of (compositions) of functions for which we are already clear about how they work. 

Namely, we define 
\begin{equation}
        \moncomp_{\setA} \colon (\setA \setdisunion \singleton) \setdisunion \singleton \mto \setA \setdisunion \singleton
    \end{equation}
as the composition
\begin{equation}
(\setA \setdisunion \singleton) \setdisunion \singleton \quad \overset{\associator}{\mto} \quad \setA \setdisunion (\singleton \setdisunion \singleton ) \quad \overset{\catid_\setA \funcsum \ \stylemorph{ ! }}{\mto} \quad \setA \setdisunion \singleton.
    \end{equation}

And we define
    \begin{equation}
       \monunit{_\setA} \colon \setA \mto \setA \setdisunion \singleton
    \end{equation}
to be simply the inclusion function into the first factor of the sum of sets:
\begin{equation}
\monunit{_\setA} = \injA.
\end{equation}
...
\begin{example}[Writer Monad]

Let $\monoidAdefinition$ be a monoid. We keep it fixed for the duration of this example. The writer functor 
\begin{equation}
\writerfunctor \colon \quad \Set \quad \fto \quad \Set
\end{equation}
(with respect to the monoid $\monoidA$) is defined as follows. 

First let's consider the level of objects. Given any set $\setA$, we define 
\begin{equation}
\writerfunctor(\setA) = \setA \cartprod \monoidAset.
\end{equation}

Next let's consider the level of morphism. Given a function $\mora \colon \setA \mto \setB$, we define
\begin{equation}
\writerfunctor(\mora) \colon \setA \cartprod \monoidAset \mto \setB \cartprod \monoidAset
\end{equation}
to be 
\begin{equation}
\writerfunctor(\mora)  = \mora \funcprod \catid_\monoidAset.
\end{equation}

The functor is a monad when equipped with the following multiplication and unit.

Given any set $\setA$, we define 
\begin{equation}
        \moncomp_{\setA} \colon (\setA \cartprod \monoidAset) \cartprod \monoidAset \mto \setA \cartprod \monoidAset
    \end{equation}
as the composition
\begin{equation}
(\setA \cartprod \monoidAset) \cartprod \monoidAset \quad \overset{\associator}{\mto} \quad \setA \cartprod (\monoidAset \cartprod \monoidAset) \quad \overset{\catid_\setA \funcprod \ \mthen}{\mto} \quad \setA \cartprod \monoidAset
    \end{equation}
and we define 
    \begin{equation}
    \begin{aligned}
       \monunit{_\setA} \colon & \setA & \mto & \setA \cartprod \monoidAset \\
        & \ela & \mapsto & \tup{\ela, \idmon_\monoidA}.
       \end{aligned}
    \end{equation} 
\end{example}
