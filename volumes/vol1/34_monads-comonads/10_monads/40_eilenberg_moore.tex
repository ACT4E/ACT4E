...
\todotext{\alphubel: do not use the list brackets.}
...
\begin{ctdefinition}[$\monA$-algebra morphism]
    ...
    A morphism $\tup{\Obja_1, \monact_1} \mto \tup{\Obja_2, \monact_2}$ of $\monA$-algebras is: \
...
\todotext{JL: the proposition below appear at the moment without context; it is supposed to be related to the list monad I believe.
    There needs to be text and context added beforehand.
}

\begin{proposition}\label{prop:monad-monoid}
    Let $\tup{\monA, \moncomp, \monunit}$ denote the list monad on the category $\Set$ of sets and functions. There is an $\monA$-algebra structure on a set $\Obja$ if and only if there
