...
\begin{lemma}
    ...
    The composition of \SY{functors} i s a functor.
\end{lemma}
...
\end{exercise}
\begin{solution}
    In the following, we want to show that \SY{functors} c ompose.
    Given categories~$\CatA,\CatB,\CatC$ and \SY{functors}~$\funa\colon \CatA\fto \CatB$,~$\funb \colon \CatB\fto \CatC$, we want to show that~$\funa\fthen\funb$ is a functor.
