...
We can define an identity \SY{functor}.

\begin{ctdefinition}[Identity functor]
    \label{def:identity_functor}
    Given a category~\CatC, we define the identity \SY{functor} ~$\funidC\colon \CatC \fto \CatC$, where
    \begin{itemize}
...
\begin{lemma}
    ...
    The identity \SY{functor} is indeed a \SY{functor}.
\end{lemma}
