...
\begin{ctdefinition}[Monotone Galois Connection]
  ...
  \begin{equation}
    \posAel\posAleq \mapb(\mapa(\posAel)) \text{ and } \posBel\posBleq \mapa(\mapb(\posBel)).
  \end{equation}
\end{ctdefinition}
...
Consider a boolean profunctor~$d\colon \posA \profto \posB$. We can define the maps that work on single functionality and resources:
%
\begin{equation}
  \begin{aligned}
...
\begin{lemma}
  $(\alpha, \beta)$ does not form an \textbf{antitone} Galois connection between~$\posLA$ and~$\posUB$.
\end{lemma}
\begin{proof}
  In~\cref{lem:alfabetaantitone} we have proved that$\alpha$ and~$\beta$ are antitone maps. For~$L\in \posA$,~$U\in \posB$, we want to show that the following does not hold:
  \begin{equation}
  ...
  Take any~$L\in \posA$. We know that~$\alpha(L)=\emptyset$, and~$\beta(\alpha(L))=\beta(\emptyset)=\emptyset$. But~$L\subseteq \emptyset$ is not true.
\end{proof}
