...
\begin{example}
    Consider a functor which maps a category \CatC to its pre-order structure \CatD, that is a category with the same objects as \CatC and a \emph{unique} morphism from~$\Obja\setin \Ob_\CatD$ to~$\Objb\setin \Ob_\CatD$ if and only if there is at least one morphism from~$\Obja$ to~$\Objb$ in \CatC.
