...
\begin{example}
    Consider a functor which maps a category \CatC to its preorder structure \CatD, that is a category with the same objects as \CatC and a \emph{unique} morphism from~$\Obja\setin \Ob_\CatD$ to~$\Objb\setin \Ob_\CatD$ if and if only if there ist at least one morphism from~$\Obja$ to~$\Objb$ in \CatC.
    This functor is full by construction, and it is faithful if and only if \CatC is a preorder.
    \todojira{136}{maybe add pic.
        OK, so what?
        examples? why is it useful?}
\end{example}

\section{Forgetful functors}
\linkvideo{spring2021-functors:semi-and-fun:forgetful-fun} % Forgetful functors

\todotextjira{411}{@J: Give more examples of forgetful functors, and more explanation/intuition. }
