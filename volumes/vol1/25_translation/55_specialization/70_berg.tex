...
\todojira{132}{
    @Gioele: More generally, define $\Berg_\alpha$ as the subcategory of \Berg with limit $\alpha$.
    Then show subcategory inclusion indexed by $\alpha$.
    This should also be a functor from \reals to \Category.
    It is also useful to have parametricity, so that we can say $\Berg_0$ is the category where you can only go down (to be used in the definition of span).
}
...
Is \Bergama a subcategory of \Berg? Let's check the different conditions:
\begin{enumerate}
...
This shows that \Bergama is a subcategory of \Berg.
What would an example of non-subcategory of \Berg be? Let's define a new category \Berglazy, which now discriminates morphisms based on the lengths of the paths they represent.
