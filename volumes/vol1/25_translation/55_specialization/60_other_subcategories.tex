...
\begin{example}\index{\textbf{InjSet}|textbf}
    \label{ex:Injset}
...
\devel{
    ...
    \begin{example}
        ...
        \begin{equation}
            \Ob_\Cat{Cont}=\makeset{\reals, \reals^2, \reals^3, \dots }
        \end{equation}
    ...
    \begin{definition}[Differentiable functions]
        A function $f\colon U\subset \reals\to \reals$, defined on an open set $U$, is \emph{differentiable} at $a\setin U$ if the derivative
        \begin{equation}
    ...
    \begin{definition}[Lipschitz continuous function]
        A real valued function $f\colon \reals\to \reals$ is called \emph{Lipschitz} continuous if there exists a positive real constant $\kappa$ such that, for all $x_1,x_2\setin \reals$:
        \begin{equation}
