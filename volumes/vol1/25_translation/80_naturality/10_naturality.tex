...
A database instance~$\funa \colon \CatC \fto \Set$ entails specifying a set~$\funa(\styleobj{S})$ of all student IDs, a set~$\funa(\styleobj{D})$ of university disciplines such as mechanical engineering, civil engineering, applied mathematics, pure mathematics, and so on.
...
For simplicity, we will just focus on two aspects of the data: student ID numbers and the disciplines of study.
...
For concreteness, let's suppose the student IDs are in some standardized format, for example a code of the kind 17-371-802, where each of the three parts of the code are calculated/assigned by some rule.
For example, the ``17'' here stands for 2017, the year the student registered with the university.
...
First, we look at the situation only on the level of objects.
...
Now consider not only a single morphism~$\mora \colon \Obja \mto \Objb$ in~$\CatC$ being mapped by~$\funa$ and~$\funb$, respectively, but \emph{all}  the category~$\CatC$.
...
These give rise to lots of squares of the kind in \cref{fig:naturality-square}, which we will require to be commutative.
...
\todo{@JL: write up example here involving monoid actions / state machines}

\todo{@JL: write up example here about natural transformations between monotone maps}
