...
\end{equation}
\todo{@J: ideally say more here to motivate or explain this. Perhaps include somewhere this perspective: 

Natural transformations are meant to capture the idea that a transformation is natural in the sense of not depending on any arbitrary choices. If a transformation does depend on arbitrary choices, the arrows $\ntrafoa_X$ and $\ntrafoa_Y$  would not be reusable but would have to change when f changes.}
