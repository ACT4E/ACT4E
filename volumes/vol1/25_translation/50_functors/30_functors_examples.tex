...
\todotext{@JL: we never define ``vector space''. This is the first time mentioned.}
\begin{example}[Vector spaces]
    \label{exa:double-dual-functor}
    Let~\CatC be the category whose objects are all real \SY{vector spaces} and whose morphisms are~\reals-linear maps.
    Composition is the usual composition of linear maps.
    \todotextjira{403}{\alphubel: @JL: Can you give a specific name to this category?}
    There is an \SY{endofunctor}~$\funa \colon \CatC \mto \CatC$ whose action on objects is
    \begin{equation}
        \funa ( \vecspB ) = \vecspB^{**}.
    \end{equation}
...
We now consider the \SY{functor}~$\listfun\colon \Set \fto \CatMon$ from the category of sets to the category of \SY{monoids}.
\todotext{We never defined \CatMon}
Given a set~\setA, the \SY{functor} returns a specific \SY{monoid} $\listfun(\setA)=\tup{\listsof{\setA},\emptylist, \listconcat}$.
...
\begin{example}
    ...
    When we talk about \textbf{planning} in this context, we refer to the action of finding a \SY{functor}~$\funa$ from \Plans to \Berg.
...
\begin{gradedexercise}[\exname{GraphsViaFunctors}]
    ...
    ``Specifying a \SY{functor}~$\CatStyle{G} \fto \Set$ is the ``same thing'' as specifying a directed graph''.
\end{gradedexercise}
...
\begin{gradedexercise}[\exname{UpperSetsViaFunctors}]
    ...
    Let~\posA be a poset.
    View it as a category~$\CatStyle{P}$, and let~$\funa \colon \CatStyle{P} \mto \Bool$ be a \SY{functor}.
