...
We now consider the functor~$\listfun\colon \Set \fto \CatMon$ from the category of sets to the category of \SY{monoids}.
...
\begin{gradedexercise}[\exname{GraphsViaFunctors}]
    ...
    ``Specifying a functor~$\CatNoIndex{G} \fto \Set$ is the ``same thing'' as specifying a directed graph''.
\end{gradedexercise}
...
\begin{gradedexercise}[\exname{UpperSetsViaFunctors}]
    ...
    Let~\posA be a poset.
    View it as a category~$\CatNoIndex{P}$, and let~$\funa \colon \CatNoIndex{P} \mto \Bool$ be a \SY{functor}.
