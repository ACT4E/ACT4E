...
\begin{example}[Powerset functor]
    ...
    For instance, consider the two sets~$\setA=\{1,2,3\}$ and~$\setB=\{3,6,9\}$.
    \todotext{@J: It is confusing if $\setA$ and $\setB$ both use numbers (with some in common). Use happy symbols? }
    ...
    \todotext{@J: write maps using \texttt{definemap}}
...
\begin{example}[Vector spaces]
    ...
    \todotext{@J: Can you give a specific name to this category?}
...
\devel{
    ...
    \todojira{119}{@Gioele: This is the free monoid functor.
...
\begin{example}
    ...
    When we talk about \textbf{planning} in this context, we refer to the action of finding a functor from \Plans to \Berg.
    ...
    Such areas are, for instance, the ``mountain lodge'', ``panoramic lake'', and the ``peak'' (note that the ``peak'' represents an area corresponding to a single location).
    ...
    % \iflabelexists{fig:plans_functor}{
...
\devel{
    \begin{figure}[h!]
        \centering
        \todographicsjira{120}{@Gioele: create planning in Berg figure}
        \caption{}
