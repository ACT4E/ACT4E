...
\begin{gradedexercise}[\exname{GraphsViaFunctors}]
    ...
    Consider the following category, which has two objects,~$\styleobj{V}$ and~$\styleobj{A}$, and four morphisms: besides the \SY{identity morphism}s, there are two morphisms,~$\stylemorph{s}$ and~$\stylemorph{t}$, from~$\styleobj{A}$ to~$\styleobj{V}$.
...
\todojira{484}{\alphubel: @Gioele: Show that there is a functor from reals to cat, which maps an alpha to the category bergalpha}
