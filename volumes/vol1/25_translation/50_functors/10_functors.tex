...
\begin{ctdefinition}[\iindex{Semifunctor}]
    \label{def:semi-functor}
    Given two semicategories~\CatC and~\CatD, a \emph{\iindex{semi-functor}}~$\funa\colon \CatC\fto \CatD$ from~\CatC to~\CatD is defined by the following constituents and conditions.
    \\
...
\begin{figure}[h!]
    ...
    \caption{
        Commuting diagrams for semi-functors, with verbose notation (left) and synthetic notation (right).
    }
\end{figure}
...
\showslides{
    \begin{forslides}
        \begin{ctdefinition}[\iindex{Semifunctor}]
            ...
            A \emph{\iindex{semifunctor}}~$\funa\colon \CatC\fto \CatD$ between two semicategories is defined by a map
            \begin{equation}
                \funaob: \ObC \to \ObD
            \end{equation}
...
Categories have identities, and \SY{functors} a re required to preserve the identities.

\begin{ctdefinition}[\iindex{Functor}]
    \label{def:functor}
    A \SY{functor} from category~\CatC to category~\CatD is a semi-functor~$\funa\colon \CatC \fto \CatD$
...
A \SY{functor} from a category to itself is called an \emph{\iindex{endofunctor}}.
