...
Consider the category $\pEuc$ from \linkifexists{ex:pointed-euclidean-spaces}, as well as the category $\RVect$ of real vector spaces studied in \linkifexists{ex:category-real-vect}. In this exercise we will define a functor $\funa \colon \pEuc \fto \RVect$ corresponding to differentiation and it is your task to check that it is in fact a functor. 
...
We will propose a functor $\funa \colon \EndSet \fto \Set$ from the category $\EndSet$ defined in \linkifexists{ex:endofunctions} to the category of sets and functions. Your task is to check if this is a functor. 
