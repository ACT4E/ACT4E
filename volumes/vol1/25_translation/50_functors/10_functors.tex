...
\begin{ctdefinition}[Semifunctor]
    ...
    Given two semicategories~\CatC and~\CatD, a \maindef{semifunctor}~$\funa\colon \CatC\fto \CatD$ from~\CatC to~\CatD is defined by the following constituents and conditions.
    \\
...
\showslides{
    \begin{forslides}
        \begin{ctdefinition}[Semifunctor]
            ...
            A \emph{semifunctor}~$\funa\colon \CatC\fto \CatD$ between two semicategories is defined by a map
            \begin{equation}
                \funaob: \ObC \to \ObD
            \end{equation}
...
\begin{ctdefinition}[Functor]
    \label{def:functor}
    A \maindef{functor} from category~\CatC to category~\CatD is a semifunctor~$\funa\colon \CatC \fto \CatD$
...
The simplest example of an \SY{endofunctor} is the identity \SY{functor}.
