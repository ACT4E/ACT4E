...
\todographics{Make the figures fit into the margins. Add shaded area for category C and D.}
...
% [i)]
% \item For every object~$\Obja\in \ObC$, one has~$\funa(\catid_\Obja)=\catid_{\funa(\Obja)}$;

\begin{ctdefinition}[\iindex{Functor}]
  \label{def:functor}
  A functor from category~\CatC to category~\CatD is a semi-functor~$\funa: \CatC \fto \CatD$
  that satisfies the condition
  \begin{equation}\label{eq:functor-condition}
    \funamor(\catid_\Obja)=\catid_{\funamor(\Obja)}.
  \end{equation}
\end{ctdefinition}
...
Consider the following category, which has two objects, $V$ and $A$, and four morphisms: besides the identity morphisms, there are two morphisms, $s$ and $t$, from $A$ to $V$. See \cref{fig:graph-cat}. Call this category $G$. 
%\begin{center}
%\begin{tikzcd}
%A  \arrow[rr, bend right=20, "t"'] \arrow[rr, bend left=20, "s"] & & V
%\end{tikzcd}
%\end{center}

Can you explain the following statement? ``Specifying a functor $G \mto \Set$ is the `same thing' as specifying a directed graph''.  
\end{gradedexercise}
...
Recall that $\Bool$ denotes the category with two objects, $\true$ and $\false$, and with precisely one non-identity morphism which goes from $\false$ to $\true$. Let $\posA$ be a poset. View it as a category \Cat{P}, and let $\funa : \Cat{P} \mto \Bool$ be a functor. In other words, $F = \funa_{Ob}$ is a monotone function. Prove that the set 
\begin{equation}
S = \{ p \in \posA \mid F(p) = \true \}
\end{equation}
is an upper set. 
\end{gradedexercise}
