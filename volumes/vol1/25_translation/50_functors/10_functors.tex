...
\begin{example}[Powerset functor]
    ...
    We define a \SY{functor}~$\funa\colon \Set \fto \Set$ which maps each set to its power set. In other words, $\funaob(\setA) = \powerset \setA$ for any set $\setA$. On the level of morphisms, given a function $\mora \colon \setA \mto \setB$, we define
...
\begin{example} There is a \SY{functor}
