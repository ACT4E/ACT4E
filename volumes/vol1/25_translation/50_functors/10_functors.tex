...
Recall that $\Bool$ denotes the category with two objects, $\true$ and $\false$, and with precisely one non-identity morphism which goes from $\false$ to $\true$. Let $\posA$ be a poset. View it as a category \Cat{P}, and let $\funa : \Cat{P} \mto \Bool$ be a functor. In other words, $\funaMap = \funa_{Ob}$ is a monotone function. Prove that the set 
\begin{equation}
