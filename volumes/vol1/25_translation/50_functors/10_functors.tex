...
%\begin{example}
%\label{ex:battery}
%Suppose you have to choose a battery model based on its cost and its weight, both to be minimized. There may be models which dominate others. For instance, a model~$\tup{\unit[10]{USD},\unit[1]{kg}}$ is always better than a model~$\tup{\unit[11]{USD},\unit[1.1]{kg}}$. Also, there may be models which are uncomparable, \ie  which form an antichain. For example, you cannot say whether~$\tup{\unit[10]{USD},\unit[1]{kg}}$ is better than~$\tup{\unit[5]{USD},\unit[2]{kg}}$. The uncomparable models form an antichain.
%\end{example}
%
...
%
\todostructure{@Gioele: Are we sure we didn't forget things around? This figure is cited somewhere.}
\begin{figure}[h!]
    \centering
