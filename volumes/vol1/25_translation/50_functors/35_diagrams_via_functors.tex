...
\todo{Make a nice diagram here illustrating the functor D}

%\begin{figure}[h!]
%    \centering
%    \includesag{diagram_functor_example}
%    \caption{}
%    \label{fig:diagram-functor-example}
%\end{figure}
...
\begin{remark}
We could also use other shape categories and functors to encode the same diagram in \cref{fig:diagram-as-functor-example}. For example, we could use a category $\CatJ$ as in \cref{fig:shape-cat-redundant} and define a functor $\stylefunctors{E}: \CatJ \fto \CatC$ by setting
\begin{align*}
...
\end{align*}
Then $\stylefunctors{E}$ will also have the diagram in \cref{fig:diagram-as-functor-example} as its image. The only difference to  the functor $\stylefunctors{D}: \CatI \fto \CatC$ above is that we have added, in the shape category, an additional object (called $\styleobj{4}$) which is also mapped to $\Objc$. This isn't really useful for anything here, we are just trying to make the point that a given diagram might be encoded by different functors, some of which ``lose'' information when mapping a shape category to its image in $\CatC$. 
\end{remark}
...
\todojira{486}{@J: Make the analogy between functors as diagrams and sequences/indexed sets... }
