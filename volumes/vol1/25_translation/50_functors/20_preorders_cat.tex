...
\todotext{J: @J: fill this subsection more}
...
\begin{lemma}
    ...
    A \SY{monotone map} between \SY{posets}~$\posA,\posB$ is the same thing as a \SY{functor} between the ``posetal categories''~$\poscat{\posA}$ and~$\poscat{\posB}$.
\end{lemma}
\begin{proof}
...
\begin{enumerate}[label=(\alph*)]
\item If the diagram \begin{tikzcd}
{\styleobj{A}} \arrow[r,"\stylemorph{k}"] \arrow[d,"\stylemorph{h}"'] & {\styleobj{B}} \arrow[d,"\stylemorph{l}"] \\
{\styleobj{C}} \arrow[r,"\stylemorph{j}"']           & {\styleobj{D}}          
\end{tikzcd}
in $\CatSymbol{C}$ commutes, then the diagram \begin{tikzcd}
{\stylefunctors{F}(\styleobj{A})} \arrow[r,"\stylefunctors{F}(\stylemorph{k})"] \arrow[d,"\stylefunctors{F}(\stylemorph{h})"'] & {\stylefunctors{F}(\styleobj{B})} \arrow[d,"\stylefunctors{F}(\stylemorph{l})"] \\
{\stylefunctors{F}(\styleobj{C})} \arrow[r,"\stylefunctors{F}(\stylemorph{j})"']           & {\stylefunctors{F}(\styleobj{D})}          
\end{tikzcd}
