...
\begin{ctdefinition}[Categorification of a poset]\label{def:poscat}
    ...
    \begin{enumerate}
        ...
        \item \emph{Identity morphism}: Given any~$\Obja \setin \Obof{\poscat{\posA}}$, the \SY{identity morphism} is given by \emph{reflexivity} in posets.
...
\begin{lemma}
    ...
    A monotone map between posets~$\posA,\posB$ is isomorphic to a \SY{functor} between the ``posetal categories''~$\poscat{\posA}$ and~$\poscat{\posB}$.
\end{lemma}
\begin{proof}
