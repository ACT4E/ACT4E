...
Recall that, \AC{from where? link.} given sets~$\Obja$ and~$\Objb$, their disjoint union~$\Obja \setdisunion \Objb$ is a set that contains a distinct copy of~$\Obja$ and~$\Objb$ each. If an element is contained in both~$\Obja$ and~$\Objb$, then there will be two distinct copies of it in the disjoint union~$\Obja \setdisunion \Objb$.

\begin{definition}[Disjoint union of sets]
  \label{def:disjoint-union}
  The \emph{disjoint union} (or sum) of sets~$\Obja$ and~$\Objb$ is
  \begin{equation*}
    \Obja \setdisunion \Objb=\{\disunionA\obja \mid \obja\in \Obja\}\cup\{\disunionB\objb \mid \objb\in \Objb\}.
  \end{equation*}
\end{definition}
...
\begin{example}
  ...
  \begin{figure}[h!]
    ...
    \end{center}
    \caption{Example of a disjoint union of sets. \todographics{add labels for the sets}}
    \label{fig:disjoint}
  \end{figure}
\end{example}
...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{Disjoint union of a set with itself. \todographics{add labels for the sets}}
...
\begin{ctdefinition}[Coproduct]
  ...
  Let~\CatC be a category and let~$\Obja, \Objb \in \ObC$ be objects. The \emph{\iindex{coproduct}} of~$\Obja$ and~$\Objb$ consists of the following constituent data, satisfying the following condition. \\
  \underline{Data:}
  \begin{compactenum}
...
\begin{example}
  ...
  Let~$S$ be a set, and~$X,Y\subseteq S$ subsets. We can draw an arrow~$X\to Y$ if~$X\subseteq Y$. By following the coproduct's commutative diagram, it is easy to see that the coproduct of~$X$ and~$Y$ is given by~$X\cup Y$, \ie , the ``smallest'' set containing both~$X$ and~$Y$.
\end{example}
