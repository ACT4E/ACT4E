...
\begin{ctdefinition}[Monotone Galois Connection]
    A \emph{monotone Galois connection} between posets~$A$ and~$B$ is a pair of monotone maps~$f\colon A\to B$ and~$g\colon B\to A$ such that for all~$a\in A$,~$b\in B$:
    \begin{equation}
        f(a) \ordleq_B b \quad \Leftrightarrow \quad a \ordleq_A g(b).
    \end{equation}
    This is equivalent to ask, for all~$a\in A$,~$b\in B$, that:
    \begin{equation}
        a\ordleq_A g(f(a)) \qquad \text{and} \qquad b\ordgeq_{B}f(g(b)).
    \end{equation}
\end{ctdefinition}
...
\begin{ctdefinition}[Antitone Galois Connection]
    ...
    \begin{equation}
        b \ordleq_B f(a) \quad \Leftrightarrow \quad a \ordleq_A g(b).
    \end{equation}
    This is equivalent to ask for all~$a\in A$,~$b\in B$:
    \begin{equation}
        a \ordleq_A g(f(a))   \qquad \text{and} \qquad  b \ordleq_B f(g(b)).
    \end{equation}
\end{ctdefinition}

\todographics{Here on need to use $\fun, \funsp, \res, \ressp$}
Consider a boolean profunctor~$d\colon F\profto R$. We can define the maps that work on single functionality and resources:
%
\begin{equation}
    \begin{aligned}
        \theta\colon F&\to \UR\\
...
\begin{ctdefinition}[Opposite category]
    ...
    Given a category~\CatC, the \emph{\iindex{opposite category}}~$\CatC\op$ has the same objects as~\CatC, but a morphism~$\mora\colon \obja\to \objb$ in~$\CatC\op$ is the same as a morphism~$\mora\colon \objb\to \obja$ in~\CatC. Furthermore, a composite of morphisms~$\mora \then \morb$ in~$\CatC\op$ is the composite~$\morb\then \mora$ in~\CatC.
\end{ctdefinition}
