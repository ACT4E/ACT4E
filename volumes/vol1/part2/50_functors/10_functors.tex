...
\begin{ctdefinition}[\iindex{Functor}]
  ...
  \begin{compactenum}
    [i)]
    \item For every object~$\obja \in \ObC$, an object~$\funa(\obja)\in \ObD$;
    \item For every morphism~$\mora \colon \obja \to \objb$ in~\CatC, a morphism~$\funa(\mora)\colon \funa(\obja)\to \funa(\objb)$ in~\CatD.
  \end{compactenum}
  ...
  \begin{compactenum}
    ...
    \begin{equation}
      \funa(\mora \then \morb)=\funa(\mora)\then \funa(\morb)
    \end{equation}
    holds in~\CatD.
  \end{compactenum}
  This situation is graphically reported in \cref{fig:functor_representation}.
\end{ctdefinition}
...
\begin{figure}[h!]
  ...
  \end{center}
  \caption{\label{fig:functor_representation}}
\end{figure}
%\begin{comment}
%%%%%%%%%
%\clearpage
