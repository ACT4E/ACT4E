...
Consider a graph~$\graph=\tup{\vertices,\arcs,\source,\target}$ and denote a path (\cref{def:path}) by~$p=\tup{\arcan{1},\ldots, \arcan{n}}$, where~$n\setin \natnumbers$,~$\source(p)\definedas\source(\arcan{1})=\vertexa$ is the source of the path,~$\target(\arcan{i})=\source(\arcan{i+1})$ for all~$i\setin \{1,\ldots,n-1\}$, and~$\target(p)=\target(\arcan{n})=\vertexb$ which is the target of the path.
