...
\begin{ctdefinition}[Free semicategory on a graph]
    ...
    The \emph{free \SY{semicategory} on $\graph$}, denoted $\CatStyle{SCat}(\graph)$, has as objects the vertices~$\vertices$ of~$\graph$, and given vertices~$\vertexa\setin \vertices$ and~$\vertexb\setin \vertices$, morphisms~$\HomSet{\CatStyle{SCat}}{\vertexa}{\vertexb}$ are the non-trivial paths from~$\vertexa$ to~$\vertexb$.
