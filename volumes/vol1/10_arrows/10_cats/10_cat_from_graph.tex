...
\begin{ctdefinition}[Free semicategory on a graph]
    \label{def:free-semicategory}
    Let~$\graph=\tup{\vertices,\arcs,\source,\target}$ be a graph.
    The \emph{free semicategory on} $\graph$, denoted $\CatStyle{SC}(\graph)$, has as objects the vertices~$\vertices$ of~$\graph$, and given vertices~$\vertexa\setin \vertices$ and~$\vertexb\setin \vertices$, morphisms~$\HomSet{\CatStyle{SC}(\graph)}{\vertexa}{\vertexb}$ are the non-trivial paths from~$\vertexa$ to~$\vertexb$.
...
\begin{ctdefinition}[Free category on a graph]
    ...
    Given~a graph $\graph=\tup{\vertices,\arcs,\source,\target}$, the \emph{free category on} $\graph$, denoted~$\CatStyle{C}(\graph)$, is defined analogously to $\CatStyle{SC}(\graph)$ but with the modification that $\HomSet{\CatStyle{C}(\graph)}{\vertexa}{\vertexb}$ is equal to \emph{all} paths from~$\vertexa$ to~$\vertexb$.
    Identity morphisms are the trivial paths.
\end{ctdefinition}
\todotext{In the above, cannot use $\CatStyle{Cat}$, it's taken.
    Also make a macro for that and \CatStyle{SCat}}

\todotext{\alphubel: This is a trivial example; there are no paths of length $>1$.
    Also the edge $a$ must become $\makelist{a}$.
    Also show the identities as empty lists.
}
...
\end{equation}
%
\todotext{\alphubel: Wrong - we defined a path as a list of edges.
