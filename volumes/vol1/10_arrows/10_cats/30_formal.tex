...
\todotext{\alphubel: @JL: the footnote mark after ``collection'' goes nowhere.}
\SYNDEF{semicategory}
\SYNDEF{hom-set}
...
\begin{remark}
    \begin{enumerate}
        \item When we want to emphasize which semicategory we are working with, we will sometimes write
              ...
              \end{equation}
        \item Sometimes we will use the notation $\Mor_\CatC$ to denote the collection of \emph{all} morphisms a semicategory $\CatC$, not just a certain hom-set. 
    \end{enumerate}
\end{remark}
...
\begin{remark}
We will often visualize objects and morphisms using diagrams where symbols or dots indicate objects and arrows indicate morphisms. For instance, if~$\Obja$,~$\Objb$,~$\Objc$ are objects in a semicategory~\CatC, and~$\mora\colon \Obja \mto \Objb$ and~$\morb\colon \Objb \mto \Objc$ are morphisms, we draw this as in \cref{fig:points-and-two-arrows-diagram}, for example.
...
\begin{example}
    ...
    We will describe a semicategory~\CatC with only two objects,~$\ObC = \makeset{ \setA, \setB }$.
...
\todotextjira{394}{\bernina: @JL: Add remark about relationship between semicategories and semigroups}
...
Categories are like semicategories, but with one more ingredient added: \SY{identity morphisms}.
A good analogy is that categories are to semicategories as \SY{monoids} are to semigroups.
A \SY{monoid} is a \SY{semigroup} that additionally has an identity element, and similarly a category is a semicategory that additionally has \SY{identity morphisms}.
...
\begin{ctdefinition}[Category]
    ...
    A \maindef{category}~\CatC is a semicategory in which there is an \SY{identity morphism} for every object.
\end{ctdefinition}
...
\begin{example}
    ...
    The semicategory of sets and functions described above in \cref{exa:the-semicategory-of-sets} is in fact a category.   Given a set $\Obja$, the \SY{identity morphism} for this set is the function
    \begin{equation}
        \defmapperiod{
            \catidat\Obja
        }{
...
\begin{definition}
Let $\CatC$ be a category. A morphism $\mora \colon \Obja \mto \Objb$ in $\CatC$ is an \emph{isomorphism} if there exists a morphism $\morb \colon \Objb \mto \Obja$ such that 
\begin{equation}
...
\begin{remark}
Note that the above definition coincides, for the category $\Set$ of sets and functions, with \cref{def:function-isomorphism}. We saw in \cref{ex:bijective-functions-are-isomorphisms} that an isomorphism in the category \Set is the same thing as a bijective function. 
\end{remark}
