...
\begin{quiz}[\exname{GroupSemiCategory}]
Let $(\styleobj{G},\mthen)$ be a group and consider the semicategory $\CatSymbol{C}$ with one object $\Ob_{C}=\{\styleobj{\bullet}\}$ and $\HomSet{C}{\styleobj{\bullet}}{\styleobj{\bullet}}=\{\styleobj{G}\}$ and composition
$$\mthen_{\styleobj{\bullet},\styleobj{\bullet}}:\styleobj{G}\times \styleobj{G}\to \styleobj{G}$$
$$(\stylemorph{g},\stylemorph{h})\in \styleobj{G}\times \styleobj{G}\mapsto \stylemorph{g}\mthen_{\styleobj{\bullet},\styleobj{\bullet}}\stylemorph{h}\coloneqq \stylemorph{g}\mthen \stylemorph{h}$$. Then the following holds ... (choose the correct statements)
\begin{enumerate}[label=(\alph*)]
\item There is an identity morphism $\catidof{\styleobj{\bullet}}\in \HomSet{C}{\styleobj{\bullet}}{\styleobj{\bullet}}$, i.e. such that $\catidof{\styleobj{\bullet}}\mthen \styleobj{g}=\styleobj{g}\mthen\catidof{\styleobj{\bullet}}=\catidof{\styleobj{\bullet}}$ for any morphism $g$.
\item For any morphism $\stylemorph{g}\in\HomSet{C}{\styleobj{\bullet}}{\styleobj{\bullet}}$ is an inverse morphism $\stylemorph{g}^{-1}$, i.e. such that $\stylemorph{g}^{-1}\mthen\stylemorph{g}=\stylemorph{g}\mthen\stylemorph{g}^{-1}=\catidof{\styleobj{\bullet}}$.
\item The composition $\mthen_{\styleobj{\bullet},\styleobj{\bullet}}$ is commutative, i.e. $\stylemorph{g}\mthen\stylemorph{h}=\stylemorph{h}\mthen\stylemorph{g}$ for all $\stylemorph{g},\stylemorph{h}\in\styleobj{G}$.
\end{enumerate}
\end{quiz}
...
\begin{example}
    ...
    Given a set $\Obja$, the \SY{identity morphism} for this set is the \SY{identity function}
    \begin{equation}
        \defmapperiod{
            \catidat\Obja
        }{
...
\begin{gradedexercise}[\exname{LinearMaps}]
    \label{ex:linear-maps}

    Morphisms between real vector spaces are called \emph{linear maps}.
    Given real vector spaces~$\vecspA = \tup{\vecspAset, +_{\vecspA}, 0_{\vecspA}, \cdot_{\vecspA}}$ and $\vecspB = \tup{\vecspBset, +_{\vecspB}, 0_{\vecspB}, \cdot_{\vecspB}}$, a linear map $\mora \colon \vecspA \mto \vecspB$ is a function
    \begin{equation}
        \mora \colon \vecspAset \sto \vecspBset
    \end{equation}
    which satisfies the following conditions:
    \begin{enumerate}
        \item $\mora(\ela +_{\vecspA} \elb) = \mora(\ela) +_{\vecspB} \mora(\elb) \quad \forall \ela, \elb \setin \vecspAset$;
        \item $\mora(\lambda \cdot_{\vecspA} \ela) = \lambda \cdot_{\vecspB} \mora(\ela) \quad \forall \ela \setin \vecspAset, \ \forall \lambda \setin \reals$.
    \end{enumerate}

    Your task in this exercise is to prove that the composition of linear maps is again a linear map.
    Concretely, suppose that you are given linear maps $\mora \colon \vecspA \mto \vecspB$ and $\morb \colon \vecspB \mto \vecspC$ and check that the composition of functions $\mora \mthen \morb \colon \vecspAset \mto \vecspCset$ is again a linear map.

\end{gradedexercise}

\solutionof{LinearMaps}

\begin{gradedexercise}[\exname{CategoryRealVect}]
    \label{ex:category-real-vect}

    Show that real vector spaces and linear maps between them form a category.
    This means:
    \begin{enumerate}
...
\begin{gradedexercise}[\exname{HwkGSet}]
    ...
    Consider the definition below of the category $\mathbf{GSet}$.
    Your task: check/justify that this does indeed define a category.
...
\devel{
    ...
    \todotext{J: @J: write this subsection}
}
