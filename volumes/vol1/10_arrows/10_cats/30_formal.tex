...
In other words, if $\mora: \Obja \mto \Objb$ and $\morb: \Objb \mto \Objc$ are morphisms in some semicategory (note: they are composable), then the semicategory has an operation for compositing them, and the result is another morphism. Our notation for composition of $\mora$ and $\morb$ is $\mora \then \morb$, read ``$\mora$ then $\morb$''. (This is in contrast to the more traditional notatation $\morb \circ \mora$.) Thus we have this rule
\begin{equation}
                      \prfperiod{
