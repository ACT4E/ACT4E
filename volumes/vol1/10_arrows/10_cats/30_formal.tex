...
This condition says that if we are given a string of three composable morphisms, then it doesnt' matter in which order we choose to compose them:
...
Have a read through, then we we'll illustrate it further with examples.
...
\begin{example}\label{exa:the-category-of-sets}
    ...
    Suppose $\mora \colon \Obja \mto \Objb$,  $\morb\colon \Objb \mto \Objc$, and  $\morc \colon \Objc \mto \Objd$ are three composable functions, and let $\ela \in \Obja$ be an arbitrary element.
    On the one hand
    \begin{equation*}
...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            \sgrpA = \{ \text{set of non-empty lists of elements of }\setA\}
        \end{equation}
