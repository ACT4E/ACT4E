...
\begin{example}
    ...
    \end{equation}
    Given some function $\mora \colon \Objf \mto \Obja$, then the function composition $\mora \mthen \catid_\Obja$ is the same function as just $\mora$ on its own:
    \begin{equation}
        (\mora \mthen \catid_\Obja)(\ela)
        = \catid_\Obja(\mora (\ela)) = \mora (\ela).
    \end{equation}
    Similarly, given a function $\morb \colon \Obja \mto \Objb$, then also $\catid_\Obja \mthen \morb = \morb$.
...
\showslides{
    \begin{forslides}
            ...
            \begin{body}
                ...
                \begin{enumerate}
                    ...
                    \item Unitality: for every morphism $\mora \colon \Obja \mto \Objb$,
                          \begin{equation}
                              \catid_{\Obja} \mthen \mora = \mora \qqand \mora \mthen \catid_{\Objb} = \mora.
                          \end{equation}
                \end{enumerate}
            \end{body}
        \end{ctdefinition}
        ...
        \begin{equation}
            \label{eq:21}
            \catid_{\Obja} \mthen \mora = \mora
        \end{equation}

        \begin{equation}
            \label{eq:22}
            \mora \mthen \catid_{\Objb} = \mora
        \end{equation}
