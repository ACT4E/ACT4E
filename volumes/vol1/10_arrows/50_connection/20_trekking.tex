...
\begin{definition}[\Berg]
    ...
    \begin{itemize}
        ...
        \item \emph{Morphisms:} A morphism~$\tup{\styleobj{p_1}, \styleobj{v_1}} \mto \tup{\styleobj{p_2}, \styleobj{v_2}}$ is~$\tup{\stylemorph{\gamma}, \stylemorph{T}}$,
              where
              \begin{itemize}
              ...
              \end{itemize}
        \item \emph{Identity morphism:} For any object~$\tup{\styleobj{p}, \styleobj{v}}$, we define its identity morphism~$\catid_{\tup{\styleobj{p},\styleobj{v}}} = \tup{\stylemorph{\gamma}, 0}$ formally: its path~$\stylemorph{\gamma}$ is defined on the closed interval~$[0,0]$, (with $\stylemorph{T}=0$ and $\stylemorph{\gamma}(0) = \styleobj{p}$).
              We declare this path to be~$C^1$ by convention, and declare its derivative at~$0$ to be~$\styleobj{v}$.

        \item \emph{Composition of morphisms:} Given morphisms
              \begin{equation}
