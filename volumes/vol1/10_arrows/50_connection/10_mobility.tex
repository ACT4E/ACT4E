...
\todo{We didn't define what is a graph yet.}
\todo{True, should we first introduce last section?}
...
\begin{example}
  ...
  \begin{equation*}
    V_\mathrm{c}=\{\SFOc, \transmuted{S. Mateo},\transmuted{H. M. Bay},\SBPc,\transmuted{Balboa},\LAXc\},
  \end{equation*}
  ...
  Furthermore, consider the \Flight category with $V_\mathrm{f}=\{\SFOf, \SJC, \SBPf, \LAXf\}$ and arrows as in~\cref{fig:flight}. The nodes represent airports in California and the arrows represent connections, offerend by specific flight companies.
  ...
  \begin{equation*}
    V_\mathrm{b}=\{\SFOf,\SFOc,\SBPf,\SBPc,\LAXf,\LAXc\}
  \end{equation*}
  ...
  The combination of the three, which we call the \emph{intermodal graph}, can be represented as a graph, in which we use dashed arrows for intermodal morphisms, arising from composition of morphisms involving multiple modes (\cref{fig:intermodal}). Imagine that you are in the parking lot of \transmuted{LAX} airport and you want to reach \transmuted{S. Mateo}. From there, you can onboard to a \transmuter{United} flight to \transmuted{SFO}$_\transmuted{f}$, will then offboard reaching the parking lot $\SFOc$, and drive on highway \transmuter{US-101} reaching \transmuted{S. Mateo}. This is intermodality.
  ...
  \end{figure}
\end{example}
\todo{What arrows do we want for this kind of diagrams?}
