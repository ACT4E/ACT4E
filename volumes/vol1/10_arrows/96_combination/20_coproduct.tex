...
Just like the notion of categorical product generalized the definition of the \SY{cartesian product} of two sets, the categorical coproduct generalizes the definition of the \emph{disjoint union} of two sets.
...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            \label{eq:coprod_2}
            \begin{aligned}
                \coprodMap{f}{g} \colon  \setA \setdisunion \setB & \sto \stylesets{P} \\
                \stylesets{x}                                     & \mapsto
                \begin{cases}
...
\devel{
    ...
    \begin{example}
        ...
        The categorical product of two elements is their \SY{join} (least upper bound); the universal property is illustrated in \cref{fig:exa_prod_meet}.
...
\todotext{The symbol $\psi$ is not macroed and should be blue.}
...
\begin{example}
    ...
    To this end, the \SY{disjoint union} of the sets of technology is considered:
    \begin{equation}
...
\begin{example}[Adapted from~\cite{spivak2014category}]
    ...
    \todotext{\alphubel: @Gioele: wrong definition}
    \begin{equation}
        \begin{aligned}
