...
\begin{ctdefinition}[Power set as lattice]
    ...
    Given a set~$\stylesets{S}$, its power set~$\powerset \stylesets{S}$ (the set of all subsets) is a \SY{lattice} where, given~$\setA,\setB\setin \powerset \stylesets{S}$:
    \begin{itemize}
        \item Order is given by inclusion:
              \begin{equation}
                  \setA\posleq \setB \definedas \setA\setsubseteq \setB;
              \end{equation}
        \item The \SY{join} is given by the union of sets:
              \begin{equation}
                  \setA\join \setB \definedas \setA\setunion \setB;
              \end{equation}
        \item The \SY{meet} is given by the intersection of sets:
              \begin{equation}
