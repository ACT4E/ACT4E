...
\begin{example}
    ...
    The categorical product of two elements is their \SY{meet} (greatest lower bound); the universal property is illustrated in \cref{fig:exa_prod_meet}.
\end{example}
...
\begin{example}
    ...
    The category~$\poscat{(\posA\Ptimes \posB)}$ arising from the product poset~$\posA\Ptimes \posB$ (\cref{def:productposet}) is the product category~$\poscat{\posA} \Ctimes \poscat{\posB}$ of the two posetal categories:
    \begin{equation}
...
\begin{gradedexercise}[\exname{PosetProductCategorical}]
    Prove that the product poset~$\posA \Ptimes \posB$ ``is'' the categorical product of~\posA and~\posB within the category~$\Pos$ of \SY{posets} and \SY{monotone maps}.
\end{gradedexercise}
