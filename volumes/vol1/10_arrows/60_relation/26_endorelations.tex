...
\begin{ctdefinition}[Transitive closure]
    ...
    The \emph{transitive closure} of an endorelation~$\relA$ on a set~$\setA$ is the smallest relation on~$\relA$ that contains~$\relA$ and is transitive.
\end{ctdefinition}
...
\begin{example}
    ...
    \begin{equation}\label{eq:transitive_clos_a}
        \middlesag{transitive_clos_a}
    \end{equation}
    \todotext{@Gioele: macro the transitive closure symbol (``+'' superscript), introduce in the definition.
    }
    ...
    \begin{equation}\label{eq:transitive_clos_b}
