...
\showslides{
    \begin{forslides}
        ...
        \begin{definition}[Union of sets]
            ...
            \begin{equation}
                \setA \setunion \setB=\{\ela \colon \ela \setin \setA \text{ or }\ela \setin \setB\}.
            \end{equation}
        \end{definition}
        \begin{equation*}
            \label{eq:divides_1}
...
%    \begin{equation}
%        \setA \setdisunion \setB=\{\disunionA\ela \mid \ela\setin \setA\}\setunion\{\disunionB\elb \mid \elb\setin \setB\}.
%    \end{equation}
%\end{definition}
...
\begin{example}
    ...
    It says that if~$t \setin \reals$ is such that~$t \geq \elna{1}$ and $t \geq \elna{2}$, then~$t \geq \min \{ \elna{1}, \elna{2} \}$.
...
\begin{example}
    ...
    As a concrete example, consider again~$\setA = \{ \sbretzel, \sfondue, \schoco, \sburger \}$,~$\subAn{1} = \{ \sbretzel, \sfondue, \schoco \}$, and~$\subAn{2} = \{\sfondue, \schoco, \sburger \}$.
    So~$\subAn{1} \setunion \subAn{2} = \setA$.
    If we choose~$\setC = \{ \sfondue \}$, we see that~$\setC \setsubseteq \subAn{1}$ and~$\setC \setsubseteq \subAn{2}$, and that also~$\setC \setsubseteq \subAn{1} \setunion \subAn{2}$ (as it must, according to the universal property).
...
\begin{example}
    ...
    \begin{figure*}[tbh]
        \centering
        \maxsizebox{0.9\textwidth}{!}{\includesag{60_coprod_batt_bis}}
        \caption{Battery technologies, companies, prices, and a catalogue.}
        \label{fig:coprod_batteries_1}
    \end{figure*}
    ...
    \begin{figure*}[tbh]
        \centering
        \maxsizebox{0.9\textwidth}{!}{\includesag{60_coprod_batt_2_bis}}
...
\begin{example}
    Let's consider the ordered set~$\tup{\reals,\leq}$, where given~$\elna{1},\elna{2}\setin \reals$ we can draw an arrow~$\elna{1}\to \elna{2}$ if~$\elna{1}\leq \elna{2}$.
    ...
    \end{itemize}
    In other words, the coproduct of~$\elna{1},\elna{2}\setin \reals$ is given by~$\max\{\elna{1},\elna{2}\}$, and is also called \emph{join}.
\end{example}
...
\todotextjira{338}{@Gioele: Do the proof of \cref{lem:coproduct-in-cat}}
