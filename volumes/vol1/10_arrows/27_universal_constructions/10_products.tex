...
\begin{figure}[h!]
    ...
    \caption{Each participant chooses a combination of food and a drink.}
    \label{fig:snacks_2}
\end{figure}
...
\begin{equation}
    ...
    \Obja \mfrom \styleobj{O} \mto \Objb
\end{equation}
in a way which is analogous to the situation in \cref{fig:snacks_3} (here~$\styleobj{O}$ stands for any set that plays the role of the set~$\Participants$) .
...
\begin{ctdefinition}[Categorical Product]
    ...
    \condit
    \begin{enumerate}
        \item For any~$\styleobj{O} \setin \ObC$ and any morphisms~$\mora \colon \styleobj{O} \mto \Obja, \morb \colon \styleobj{O} \mto \Objb$, there exists a \emph{unique} morphism~$\catprodphi_{\mora,\morb} \colon \styleobj{O} \mto \Objc$ such that $\mora = \catprodphi_{\mora,
...
\begin{remark}
    ...
    The stated condition involves the product~$\Objc$ of~$\Obja$ and~$\Objb$ \emph{interacting} with every possible choice of object~$\styleobj{O}$ and every possible choice of morphisms~$\mora \colon \styleobj{O} \mto \Obja$ and~$\morb\colon \styleobj{T} \mto \Objb$.
    ...
    We think of ``$\styleobj{O}$'' as a ``test object'' (similar, for instance, to how in electrodynamics a ``test charge'' is used to probe an electromagnetic field).
\end{remark}

\todotext{J: edit the above remark to reflect the fact that not only Z is the product, but also the projections...}

\todotext{J: add an example showing that changing the projections matters... perhaps put lower down somewhere...}
...
\begin{remark}
    ...
    However, for simplicity, it is common to refer only to~$\styleobj{Z}$ as ``the product''.
    Furthermore, we will usually use the notation~$\prodMapob\Obja\Objb$ to denote the product of~$\Obja$ and~$\Objb$, in place of~$\styleobj{Z}$, and we sometimes write~$\tup{\mora,\morb}$ in place of~$\catprodphi_{\mora, \morb}$.
    ...
    However, any two candidates which satisfy the definition of being ``the product of~$\Obja$ and~$\Objb$'' will necessarily be isomorphic in a canonical manner; see \cref{prop:prod-unique-up-to-canon-iso} below.
    Thus, for simplicity, we will often be slightly sloppy and speak of ``the product of~$\Obja$ and~$\Objb$'' as if it were unique.
    ...
    However, other representatives of the product of~$\Obja$ and~$\Objb$ are possible; \cref{exa:univ-prop-prod} illustrates this.
\end{remark}

\todotext{J: the cross reference using cref just above to the example about product codes below is displying theorem instead of example... not sure why... }

\begin{proposition}\label{prop:prod-unique-up-to-canon-iso}
    Let $\CatC$ be a category, $\Obja, \Objb \setin \ObC$, and suppose both
    \begin{equation}
        \Obja \overset{\stylemorph{pr_1}}{\mfrom} \Objc \overset{\stylemorph{pr_2}}{\mto} \Objb
    \end{equation}
    and
    \begin{equation}
        \Obja \overset{\stylemorph{pr_1'}}{\mfrom}  \Objcprime \overset{\stylemorph{pr_2'}}{\mto} \Objb
    \end{equation}
    are categorical products of $\Obja$ and $\Objb$.
    Then there exists a unique isomorphism $\stylemorph{\phi} : \Objc \mto \Objcprime$ such that
    \begin{equation}
        \begin{tikzcd}[column sep=large, every arrow/.append style={arrowmorphismstyle}]
                &  & \Objc \arrow{dddll}[swap]{\stylemorph{pr_1}} \arrow{dddrr}{\stylemorph{pr_2}}  \arrow{dd}{\stylemorph{\phi}} & & \\
                  & & & & \\
        &    &   \Objcprime \arrow{dll}{\stylemorph{pr_1'}}[swap]{} \arrow{drr}[swap]{\stylemorph{pr_2'}}   & &  \\
             \Obja       &   & & & \Objb
        \end{tikzcd}
    \end{equation}
    commutes.
...
\begin{remark}
    A typical jargon phrase for this situation is to say that the categorical product is ``unique up to unique isomorphism''.
\end{remark}

\linkvideo{spring2021-products:products:cat-prod:example-prod} % Examples of products

\begin{example}[Product code]
    \label{exa:univ-prop-prod}
    ...
    %
    To see concretely what this isomorphism between them looks like, note that there is a unique map $\catprodphi_{\text{first 8},\text{last 4}}$ making the following diagram commute: % ok
    %
...
\begin{gradedexercise}[\exname{PosetProductCategorical}]
    \label{ex:PosetProductCategorical}
...
\begin{ctdefinition}[Finite categorical products]
    \label{def:finite-categorical-product}
    Let~\CatC be a category and let~$\Objan{1}, \dots, \Objan{n} \setin \ObC$ be $n$ objects, with $n \setin \natnumbers$.
    The \maindef{categorical product} of~$\Objan{1}, \dots, \Objan{n}$ is: \\
    \constit
    \begin{enumerate}
        \item an object~$\Objc \setin \ObC$;
        \item \emph{projection morphisms}~$\stylemorph{pr_i} \colon \Objc \mto \Obja$ for $i =1, \dots, n$;
    \end{enumerate}
    \condit
    \begin{enumerate}
        \item For any~$\styleobj{O} \setin \ObC$ and any morphisms~$\stylemorph{f_i} \colon \styleobj{O} \mto \Objan{i}$, with $i = 1, \dots, n$, there exists a unique morphism~$\catprodphi \colon \styleobj{O} \mto \Objc$ such that $\stylemorph{f_i} = \catprodphi \mthen \stylemorph{pr_i}$ for each $i = 1, \dots, n$.
    \end{enumerate}
\end{ctdefinition}
...
\devel{
    ...
    \begin{ctdefinition}\label{def:cartesian-categories}
        ...
    \end{ctdefinition}
