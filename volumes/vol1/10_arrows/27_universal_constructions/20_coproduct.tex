...
In the case of the \SY{cartesian product} of two sets we had projection maps, as in \cref{sec:products}.
...
\subsection{Categorical coproduct}
...
\begin{ctdefinition}[Categorical Coproduct]
    ...
    \condit
    \begin{enumerate}
        \item For any~$\styleobj{O} \setin \ObC$ and any morphisms~$\mora \colon \Obja \mto \styleobj{O}, \morb \colon \Objb \mto \styleobj{O}$, there exists a \emph{unique} morphism~$\catcoprodpsi_{\mora, \morb} \colon \styleobj{Z} \mto \styleobj{O}$ such that~$\mora = \injA\mthen \catcoprodpsi_{\mora, \morb}$ and~$\morb = \injB \mthen \catcoprodpsi_{\mora,\morb}$.
    \end{enumerate}
\end{ctdefinition}
...
\begin{remark}
    ...
    Nevertheless, we will usually simply write~$\coprodMapob{\Obja}{\Objb}$ for ``the'' coproduct (in place of~$\Objc$ above).
...
\begin{example}
    ...
    \begin{figure*}[tbh]
        ...
        \includesag{60_coprod_batt_bis_turn}
        % }
        \caption{Battery technologies, companies, prices, and a catalogue.}
        \label{fig:coprod_batteries_1}
    \end{figure*}
    ...
    \begin{figure*}[tbh]
        \centering
        % \maxsizebox{0.9\pagewidth}{!}{
        \includesag{60_coprod_batt_2_bis_turn}
        % }
...
\begin{example}[Adapted from~\cite{spivak2014category}]
    \label{def:ex_graph}
    Given two graphs~$\graphA=\tupp{\vertices,\arcs,\source,\target}$ and~$\graphA'=\tupp{\vertices',\arcs',\source',\target'}$, their coproduct is a graph
    \begin{equation}
        \graphA\Cplus\graphA'=\tupp{\vertices \setdisunion\vertices',\arcs \setdisunion\arcs',\source\funcsum\source',\target\funcsum\target'}.
    \end{equation}
    In~$\graphA\Cplus\graphA'$, an arrow connects~$\vertexa_1$ to $\vertexa_2$ if:
    \begin{itemize}
        ...
        \item an arrow from~$\vertexa_1$ to~$\vertexa_2$ exists in~$\graphA$ or~$\graphA'$.
    \end{itemize}
    ...
    We can illustrate this nicely as follows.
    ...
    Roughly speaking, their coproduct is a graph that includes both original graphs, but without connecting them (\cref{fig:graphs_2}).
...
\begin{example}[Coproduct in \Category]
    The \SY{disjoint union} of categories is the coproduct in \Category.
    Let~$\CatC$ and~$\CatD$ be categories.
    We verify that $\CatC \Cplus \CatD$, as defined in \cref{def:disjoint-union-category}, together with two injection functors, satisfies \cref{def:categorical-coproduct}.
    \paragraph{Injection functors.}
    Define~$\injA \colon \CatC \mto \CatC \Cplus \CatD$ and~$\injB \colon \CatD \mto \CatC \Cplus \CatD$ by
    \begin{equation}
        \injA(\Obja) \definedas \disunionA{\Obja},
        \qquad
        \injA(\mora) \definedas \mora,
    \end{equation}
    for each object~$\Obja \setin \ObC$ and each morphism~$\mora$, and symmetrically~$\injB(\Objb) \definedas \disunionB{\Objb}$,~$\injB(\morb) \definedas \morb$ for~$\Objb \setin \ObD$.
    Both are functors, since identity morphisms are preserved because~$\catidofat{\CatC \Cplus \CatD}{\disunionA{\Obja}} = \catidofat{\CatC}{\Obja}$ by definition, and composition is preserved because the composition law of~$\CatC \Cplus \CatD$ restricted to morphisms between objects of the form~$\disunionA{-}$ (resp.~$\disunionB{-}$) coincides with~$\mthenof{\CatC}$ (resp.~$\mthenof{\CatD}$).

    \paragraph{Existence of the universal functor.}
    Let~$\CatO$ be a category and let~$\mora \colon \CatC \mto \CatO$,~$\morb \colon \CatD \mto \CatO$ be functors.
    Define~$\catcoprodpsi_{\mora,\morb} \colon \CatC \Cplus \CatD \mto \CatO$ by
    \begin{equation}
        \catcoprodpsi_{\mora,\morb}(\disunionA{\Obja}) \definedas \mora(\Obja),
        \qquad
        \catcoprodpsi_{\mora,\morb}(\disunionB{\Objb}) \definedas \morb(\Objb),
    \end{equation}
    on objects, and on morphisms:
    \begin{equation}
        \catcoprodpsi_{\mora,\morb}(\phi) \definedas
        \begin{cases}
            \mora(\phi) & \text{if } \phi \setin \HomSet{\CatC}{\Obja}{\Objb}, \\
            \morb(\phi) & \text{if } \phi \setin \HomSet{\CatD}{\Obja}{\Objb}.
        \end{cases}
    \end{equation}
    This is well-defined, as every morphism of~$\CatC \Cplus \CatD$ lies in exactly one of these two cases, since~$\HomSet{\CatC \Cplus \CatD}{\disunionI{\Obja}}{\disunionJ{\Objb}} = \Emptyset$ whenever~$i \neq j$.
    Since~$\mora$ and~$\morb$ are each functors,~$\catcoprodpsi_{\mora,\morb}$ preserves identity morphisms and composition, hence is itself a functor.
    By construction,
    \begin{equation}
        \injA \mthen \catcoprodpsi_{\mora,\morb} = \mora
        \qqand
        \injB \mthen \catcoprodpsi_{\mora,\morb} = \morb.
    \end{equation}

    \paragraph{Uniqueness.}
    Suppose~$\Phi \colon \CatC \Cplus \CatD \mto \CatO$ is any functor satisfying~$\injA \mthen \Phi = \mora$ and $\injB \mthen \Phi = \morb$.
    Every object of~$\CatC \Cplus \CatD$ has the form~$\disunionA{\Obja}$ or~$\disunionB{\Objb}$, so
    \begin{align}
        \Phi\bigl(\disunionA{\Obja}\bigr)
         & = \Phi\bigl(\injA(\Obja)\bigr)
        = \mora(\Obja)
        = \catcoprodpsi_{\mora,\morb}\bigl(\disunionA{\Obja}\bigr), \\
        \Phi\bigl(\disunionB{\Objb}\bigr)
         & = \Phi\bigl(\injB(\Objb)\bigr)
        = \morb(\Objb)
        = \catcoprodpsi_{\mora,\morb}\bigl(\disunionB{\Objb}\bigr),
    \end{align}
    and the same equalities hold on morphisms by the same argument.
    Hence~$\Phi = \catcoprodpsi_{\mora,\morb}$, establishing uniqueness.
\end{example}