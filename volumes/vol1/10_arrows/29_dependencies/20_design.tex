...
\todotextjira{35}{@Gioele: This was moved after the definition of a category.
    Change accordingly.
}
...
\equationsag{generic_transmuter}{eq:arrow1}
...
then we conclude that using an~$\transmuter{a}$ and a~$\transmuter{b}$ we can get an~$\transmuted{X}$ from a~$\transmuted{Z}$.
...
\begin{example}
    Given any category~\CatC, and any object~$\Obja\in \CatC$, the set of \emph{endomorphisms}~$\Hom_{\CatC}(\Obja,\Obja)$ is a monoid.
