...
\begin{example}[Intervals]
    ...
    The twisted arrow category~$\TwistedArrow \posA$ is isomorphic to the poset (viewed as a category)~$(\posint\posA) \op$ of nonempty \emph{intervals} in~$\posA$.
    Recall that, given elements~$\posela, \poselb \in \posA$, the interval~$\interv{\posela}{\poselb}$ is
    \begin{equation*}
        \interv{\posela}{\poselb}
        \definedas \{\poselc \in \posA \mid \posela\posAleq \poselc \posAleq \posela\}.
    \end{equation*}
\end{example}
