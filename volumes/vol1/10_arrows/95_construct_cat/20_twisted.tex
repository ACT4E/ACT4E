...
\begin{example}[Intervals]
    ...
    The twisted arrow category~$\TwistedArrow \poscat{\posA}$ is isomorphic to the poset (viewed as a category)~$\poscat{\left((\posint\posA) \op\right)}$ of nonempty \emph{intervals} in~$\posA$.
    Recall that, given elements~$\posela, \poselb \setin \posA$, the interval~$\interv{\posela}{\poselb}$ is
    \begin{equation*}
        \interv{\posela}{\poselb}
        \definedas \{\poselc \setin \posA \mid \posela\posAleq \poselc \posAleq \posela\}.
    \end{equation*}
\end{example}
