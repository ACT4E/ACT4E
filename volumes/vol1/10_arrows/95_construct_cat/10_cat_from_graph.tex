...
\begin{ctdefinition}[Free category on a graph]
    ...
    The composition of morphisms is given by concatenation of paths, and for any object~$\vertexa \setin \vertices$, the associated identity morphism~$\catid_\vertexa$ is the trivial path which starts and ends at~$\vertexa$.
\end{ctdefinition}
...
A trivial path is a path~$\catid_\vertexa$, consisting of a vertex and a trivial arc, starting and ending at~$\vertexa$.
Such a path can be concatenated with another path~$q=\tup{\arcb_1,\ldots,\arcb_m}$ in case~$\vertexa=\source(q)$.
In this case, one obtains the path~$\catid_\vertexa \mthen q=q$.
Similarly,~$p$ can be concatenated with~$\catid_\vertexa$ if~$\target(p)=\vertexa$, obtaining~$p\mthen \catid_\vertexa=p$, and proving unitality.
