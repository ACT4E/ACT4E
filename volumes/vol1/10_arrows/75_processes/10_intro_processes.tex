...
We will also choose an \emph{initial state} denoted~$\prstart \setin \prst$.
...
\subsection{Semantics of Moore machines as signal transformers}

\todo{Add semantics here, right away.}

\subsection{Composition of Moore machines}
...
The issue lies essentially with the Cartesian product of state spaces~$\prst_{\mora} \cartprod \prst_{\morb}$, and the fact that such product is not associative.
\todo{Did we mention this before?}

Given three composable systems~$\mora$,~$\morb$, and~$\morc$, if we compose them in the two ways~$(\mora \mthen \morb) \mthen \morc$ and~$\mora \mthen (\morb \mthen \morc)$, then their respective state spaces are~$(\prst_{\mora} \cartprod \prst_{\morb}) \cartprod \prst_{\morc}$ and~$\prst_{\mora} \cartprod (\prst_{\morb} \cartprod \prst_{\morc})$.
...
\end{equation}
%
However, they are \emph{isomorphic}.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
If we want to have Moore machines be a semicategory, we have two options: either we relax the definition of category to allow associativity up to isomorphisms, or we modify slightly the definition of Moore machines to have an associative composition.
