...
In this section we will generalize from actions of a semigroup to actions of a semicategory, using the example of Moore machines acting on signal sequences.
...
\begin{equation}
\act \colon  \HomSet{\Moore}{\prinL}{\prinL} \to \End ({\prinL_\mora}^{\natnumbers}).
\end{equation}
And this function is in fact a semigroup action, since we proved in \cref{prop:moore-action-is-a-morphism} that
\begin{equation}
...
A More machine in~$\HomSet\More\Obja\Objb$  will map the set~$\listsof \Obja$ to the set~$\listsof \Objb$.
