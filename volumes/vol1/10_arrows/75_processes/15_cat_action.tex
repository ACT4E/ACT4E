...
\todojira{456}{Fix mistakes in signals}
\todojira{457}{Introduce LTI story}

Now it is time to generalize from actions of a semigroup to action of a semicategory.

Let~$\CatC$ a general process category; either \Moore or \More. % as in \cref{tab:processes-types}.
...
\end{equation}
%
\todo{J: the second case above, that of \textbf{More}, does not make sense to me. According to the definition of More machines, $\seqsof \Obja$ is already the input set of such a machine, no? This does not seem to parallel the construction for Moore machines. More appropriate might be to take sequences in $\seqsof \Obja$, i.e. $(\natnumbers \to \seqsof \Obja)$.}
