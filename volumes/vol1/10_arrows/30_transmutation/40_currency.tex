...
Given an amount~$x \in \reals$ of \USD, this function is:
\begin{equation*}
...
Its collection of objects is the set~$\makeset{\EUR, \USD, \CHF, \SGD}$, and it morphisms are, in total:
\begin{itemize}
...
\begin{solution}
    ...
    In general, we can write a temperature converter (morphism) from \transmuted{e} to \transmuted{f} as a pair of numbers~$\tup{a,d}$,~$a,d\in \reals$.
    ...
    \begin{itemize}
        ...
        \item \emph{Morphisms:} There is a single morphism from \transmuted{e} to \transmuted{f} for any~$\transmuted{e},\transmuted{f}\in \Ob_\Temp$, given by~$\tup{a,d}$, with~$a,d$ as in \cref{tab:temperature_conversions};
