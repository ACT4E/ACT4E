...
\todotext{This was moved way before, we shouldn't reference categories anymore.}
\begin{definition}[Monoid]
  \label{def:monoid}
  A \emph{\iindex{monoid}} is a set~$\monoidA$, together with a binary operation~$\mtimes \colon \monoidA \cartprod \monoidA \sto \monoidA$, called \emph{multiplication}, and a \emph{neutral element}~$\idmon\in \monoidA$, which satisfy:
  \begin{enumerate}
