...
\begin{ctdefinition}[Category of sets]
    ...
    \begin{enumerate}
        ...
        \item \emph{Identity morphism}: given a set~$\Obja$, its \SY{identity morphism}~$\catidat\Obja$ is the identity function~$\Obja \mto \Obja, \ \catidat\Obja(\ela) = \ela$.
    \end{enumerate}
\end{ctdefinition}
...
Similarly, there is a category~$\Surj$ of \SY{surjective} functions.

\begin{exercise}
    Spell out a definition of the category~$\Injset$ of \SY{injective} functions, and check that it is indeed a category.
    In particular:
    \begin{enumerate}
        \item Specify what the composition operations are and check if the composition of two composable \SY{injective} functions is again injective;
...
\begin{solution}
    ...
    \begin{enumerate}
        \item We show that the composition of two \SY{injective} functions is injective.
