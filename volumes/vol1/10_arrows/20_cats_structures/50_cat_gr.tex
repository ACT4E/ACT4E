...
Given \SY{graph homomorphisms}~$\funa\colon \graph_1\mto \graph_2$ and $\funb\colon \graph_2\mto \graph_3$, their composition is $\funa\mthen \funb \colon \graph_1 \fto \graph_3$, and acts on vertices with~$\funaob \mthen \funbob$ and on edges with~$\funamor \mthen \funbmor$.
...
\end{lemma}
%
\todotext{\alphubel: @Gioele: Dont use the functions evaluated at $(a)$ in the proof: just use equality of functions.}
\begin{proof}
    ...
    Then,~$\funa\mthen \funb$ is a \SY{graph homomorphism}, since for all~$\arc \setin \arcsn{1}$ we have:
    \begin{equation}
        \begin{aligned}
...
\begin{ctdefinition}[Category \Grph]
    ...
    \begin{enumerate}
        ...
        \item \emph{Morphisms}: for graphs~$\Obja$ and~$\Objb$, $\HomSet{\Grph}{\Obja}{\Objb}$ is the set of \SY{graph homomorphisms} (\cref{def:graph_homom}) from~$\Obja$ to~$\Objb$.
        ...
        \item \emph{Identity morphisms}: for a graph~$\Obja =\tup{\vertices,\arcs,\source,\target}$,
              its identity morphism $\catid_\Obja$ is the \SY{graph homomorphism} $\funa$ with $\funaob = \catid_{\vertices}$ and $\funamor= \catid_{\arcs}$.
    \end{enumerate}
\end{ctdefinition}
