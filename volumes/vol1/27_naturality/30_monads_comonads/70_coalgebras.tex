...
\section{Co-algebras of a co-monad}
...
\begin{ctdefinition}[Co-algebra of a co-monad]
    \SYNDEF{co-algebra of a co-monad}
...
Recall from \cref{exa:cowriter-comonad} that the cowriter co-monad (first variant) has $\stylefunctors{\aword{Cowrite}}(\setA) = \setA \cartprod \monoidAset$, with extraction $\comonex_\setA(\ela, \styleelements{m}) = \ela$ and duplication $\comondu_\setA(\ela, \styleelements{m}) = ((\ela, \styleelements{m}), \styleelements{m})$.

Since the underlying functor and duplication are the same as for the product/environment co-monad (\cref{exa:coalgebra-product-comonad}), a $\stylefunctors{\aword{Cowrite}}$-co-algebra on a set $\setA$ is a function $\comoncoact \colon \setA \mto \setA \cartprod \monoidAset$. Writing $\comoncoact(\ela) = (\ela', \styleelements{m}_\ela)$, the counit condition gives $\ela' = \ela$, and the cocomposition condition is automatically satisfied (see \cref{exa:coalgebra-product-comonad}).

So a $\stylefunctors{\aword{Cowrite}}$-co-algebra on $\setA$ is a function $\morc \colon \setA \mto \monoidAset$, with $\comoncoact(\ela) = (\ela, \morc(\ela))$, exactly as in the product co-monad case. The monoid structure on $\monoidAset$ does not impose additional constraints on the co-algebra, but it enriches the interpretation: one can think of $\morc$ as a ``measurement'' or ``observation'' that assigns a monoid value to each element of $\setA$.
\end{example}
