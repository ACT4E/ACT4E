...
basic definitions to advanced higher-order category constructs.
We hope that
...
\gray{To write.
    I thought here we could give some pointers to other papers
    ...
    \begin{itemize}
        \item In addition, our paper is also of a tutorial nature, for people who do not already know category theory.
        \item Moreover, this theory is not only ``descriptive'', but also ``prescriptive'', in the sense that we can use these results to create optimization algorithms (see ``Computation'' section.)
    \end{itemize}
}
