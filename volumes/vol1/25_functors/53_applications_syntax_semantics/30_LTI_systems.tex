...
\todotext{J: in the definition of LTI below the starting state is the first entry of a tuple, while for Moore the starting state is the last -- we should choose a common convention.
    I'd be for having the state last.
}
...
\begin{definition}[Proper LTI System]
...
\begin{example}
    ...
    \begin{figure}[h]
        ...
        \caption{A mass attached to two springs on a frictionless surface.}
        \label{fig:LTI-example_two-springs-one-mass}
    \end{figure}
...
\todotext{J: is this the definition of equivalence of systems that is most commonly used in control theory? Can you Gioele or Andrea confirm? Alternatively, one might consider a definition where there is also a coordinate transformation on the input and output spaces, too.
    Compare e.g. with the definition given on page 22 of the book ``Mathematical Control Theory'' by Zabcyzk.
}
...
\begin{ctdefinition}[Category \LTI]
    ...
    \begin{enumerate}
        ...
        \item \emph{Morphisms}: A morphism in \LTI from~$\styleobj{l}\setin \natnumbers$ to~$\styleobj{m}\setin \natnumbers$ is a continuous time LTI system~$\genericlti{}$ such that $\styleobj{l}$ is the dimension of the input space, and $\styleobj{m}$ the dimension of the output space.
        ...
        \item \emph{Composition}: Given morphisms~$\mora \colon \styleobj{a}\mto \styleobj{b}$ and~$\morb\colon \styleobj{b}\mto \styleobj{c}$, described by the LTI systems
              ...
              \begin{equation}
                  ...
                  \end{aligned}
              \end{equation}
        \item \emph{Identities}: the identity for~$\styleobj{l}\setin \natnumbers$ is the system~$\tup{\mat{0}^{\matdim{0}{1}},\mat{0}^{\matdim{0}{0}},\mat{0}^{\matdim{0}{l}},\mat{0}^{\matdim{l}{0}},\mat{1}^{\matdim{l}{l}}}$, where $\mat{1}^{\matdim{l}{l}}$ denotes the $l\times l$ identity matrix.
    \end{enumerate}
\end{ctdefinition}
...
\begin{ctdefinition}[Category action]
    \label{def:category-action}
    A \maindef{category action} of a \SY{category}~\CatC is
    ...
    \begin{enumerate}
        \item For all composable morphisms $\mora$ and $\morb$,
              ...
              \end{equation}
        \item For all objects $\Obja \setin \Obof{\CatC}$,
              \begin{equation}
                  \funmorspace{\act}(\idmon_\Obja) = \idmon_{\funobspace{\act}(\Obja)}
              \end{equation}
    \end{enumerate}
\end{ctdefinition}
...
\todotext{In the above, and generally throughout this section/chapter, drop the "(t)" when we are talking about a funciton, and only evaluate when we are talking about the value taken by a given function}
...
\begin{remark}
    ...
    There is general formula for the solution $\mat{s}$ of \cref{eq:LTI-Cauchy-problem}, namely
    \begin{equation}
...
\begin{lemma}
    ...
    Two equivalent systems have the same standard LTI action.
\end{lemma}
\todotext{phrase the above more precisely}
...
\begin{proof}
    ...
    We note that control theory has developed tools to deal with hidden parts of the dynamics.
