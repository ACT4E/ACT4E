...
\begin{ctdefinition}[Isomorphism]
  ...
  Let~\CatC be a category, let~$\Obja,\Objb \in \CatC$ be objects, and let~$\mora \colon \Obja \cto \Objb$ be a morphism. We say that~$\mora$ is an \emph{\iindex{isomorphism}} if there exists a morphism~$\morb\colon \Objb \to \Obja$ such that~$\mora \then \morb = \catid_{\Obja}$ and~$\morb \then \mora = \catid_{\Objb}$.
\end{ctdefinition}
...
\begin{exercise}
  ...
  That is, show that if~$\mora \colon \Obja \to \Objb$ is an isomorphism, and if~$\morb_1\colon \Objb \to \Obja$ and~$\morb_2\colon \Objb \to \Obja$ are morphisms such that~$\mora \then \morb_1 = \catid_\Obja$ and~$\morb_1 \then \mora = \catid_\Objb$, and~$\mora \then \morb_2 = \catid_\Obja$ and~$\morb_2 \then \mora = \catid_\Objb$, then necessarily~$\morb_1 = \morb_2$.
\end{exercise}

\begin{ctdefinition}[Isomorphic objects]
  Let~$\Obja,\Objb$ be two objects in a category. We say that~$\Obja$ and~$\Objb$ are \textbf{isomorphic} if there exists an isomorphism~$\Obja \cto \Objb$ or~$\Objb \cto \Obja$.
\end{ctdefinition}
...
\begin{example}
In \FinSet, isomorphisms from a set to itself are automorphisms, and correspond to \emph{\iindex{permutations}} of the set. Assuming a cardinality of~$n$ for the set (\ie , the set has~$n$ elements), the number of isomorphisms is given by the number of ways in which one can ``rearrange'' $n$ elements of the set, which is $n!$.
\end{example}
