...
\begin{ctdefinition}[Isomorphism]
  Let~\CatC be a category, let~$\Obja,\Objb \in \CatC$ be objects, and let~$\mora \colon \Obja \to \Objb$ be a morphism. We say that~$\mora$ is an \emph{\iindex{isomorphism}} if there exists a morphism~$\morb\colon \Objb \to \Obja$ such that~$\mora \then \morb = \text{id}_\Obja$ and~$\morb \then \mora = \text{id}_\Objb$.
\end{ctdefinition}
