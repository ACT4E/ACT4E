...
\begin{ctdefinition}[Category \Rel]
  ...
  \begin{compactenum}
    ...
    \end{equation}
    \item \emph{Composition}: Given relations~$\relA \colon \Obja\cto \Objb$,~$\relB \colon \Objb\cto \Objc$, their composition is given by
    \begin{equation}
      \label{eq:RelCompRule}
      \relA \then \relB \definedas \{\tup{\obja,\objc} \mid  \exists \objb \in \Objb \colon \ \left(\tup{\obja,\objb} \in \relA\right) \wedge \left(\tup{\objb,\objc} \in \relB\right)\}.
    \end{equation}
  \end{compactenum}
\end{ctdefinition}
