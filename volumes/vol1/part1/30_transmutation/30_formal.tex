...
%a category consists of:
  \begin{compactenum}
    ...
    an element~$\catid_{\Obja} \in \HomC({\Obja,\Obja}) $ which is called \emph{the identity
    morphism of~$\Obja$}.
  ...
  \begin{compactenum}
    \item[a)] \emph{Unitality}: for any \iindex{morphism}~$\mora\in\HomC(\Obja,\Objb)$,
    \begin{equation}
      \catid_\Obja \then \mora= \mora = \mora \then \catid_\Objb.
    \end{equation}
    \item[b)] \emph{Associativity}: for~$\mora\in \HomC(\Obja,\Objb)$,~$\morb\in \HomC(\Objb,\Objc)$, and~$\morc\in \HomC(\Objc,\Objd)$,
    \begin{equation}
    (\mora\then \morb)
