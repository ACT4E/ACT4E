...
\todojira{223}{Style in the whole chapter needs to be adapted.}
Consider an arbitrary graph of design problems, in which nodes are design problems and edges are interconnections.
...
\begin{problem}
  ...
  that we can evaluate the map~$\ftor_{\cdpin}$ \todo{This notation is not yet introduced, I think.} for all design problems~$\cdpin\in\cdpiN$.
...
\begin{example}
  ...
  \begin{equation*}
    ...
    \end{cases}
  \end{equation*}
  The \uline{fea}sible \todo{why underlined?} set~$\Phi\subseteq\funsp\times\ressp$ is
  ...
  \end{equation}
  The \uline{p}rojection \todo{why underlined?} ~$P$ of~$\Phi$ to the functionality space
  is:
  \begin{equation*}
