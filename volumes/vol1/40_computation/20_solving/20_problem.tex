...
\todojira{216}{OK so now we can define this map \ftor. Note each dp has a \ftor.}


\begin{problem}
  \label{prob:MCDP}Suppose that we are given a CDPI~$\tup{ \funsp,\ressp,\tup{ \cdpiN,\cdpiE} }$, and
  that we can evaluate the map~$\ftor_{\cdpin}$ for all~$\cdpin\in\cdpiN$.
