...
\begin{marginfigure}
    ...
    \todographicsjira{711}{\alphubel: remake figure}
\end{marginfigure}
...
Hasegawa~\cite{hasegawa02uniformity} discusses the equivalence in detail.
...
\begin{figure*}[h]
    ...
    \todographicsjira{711}{\alphubel: Re-do graphics with symbols. loopb = trace}
\end{figure*}

The following theorem establishes a closed form for~$\ftor_{\dploop(\adp)}$ as a \SY{least fixed point}.
Henceforth, we consider~$\fAressp$ as a \SY{poset} with the order given by
\begin{equation}
    \prfdoubleperiod{
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \label{eq:condition3}
    \end{equation}
    Substituting~\cref{eq:fea} into~\cref{eq:condition3}, we obtain (cf.~\cref{lem:antichain_union})
    \begin{equation}
        \RR\posgeq_{\fAressp}\Min_{\resleq}\bigsetunion_{\res\setin\RR}\ftor_{\funan{1}}(\res)\ \setintersection\ \upit\res.
        \label{eq:recursive}
    \end{equation}
    %
    Conversely, it is also true that if an \SY{antichain}~$\RR$ satisfies~\cref{eq:recursive} then all~$\res\setin\RR$ are feasible.
    ...
    We can rewrite~\cref{eq:conc} by substituting~$\resan{0}$ for both~$\resan{1}$ and~$\resan{2}$ to obtain~$\forall\resan{0}\setin\RR\colon \resan{0}\setin\ftor_{\funan{1}}(\resan{0})$, which means that~$\resan{0}$ is a feasible resource.
...
\end{lemma}
\begin{proof}
    Davey and Priestley~\cite{davey02} leave this as Exercise~8.26.
