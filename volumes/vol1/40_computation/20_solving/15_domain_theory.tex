...
\begin{remark}
    ...
    For example, the ceiling function $x\mapsto\left\lceil x\right\rceil $ is not continuous in the usual sense, but it is \scottcontinuous (\cref{fig:gmcdp_ceil}).
...
We call this map~$\stylemaps{\phi} \colon \powerset \setA \to \powerset \setA$, so that if~$\subA$ is the initial group,~$\stylemaps{\phi}(\subA)$ is the complete set of invites.
...
\begin{marginfigure}
    \begin{tikzpicture}
        \draw[draw=setcolor, rounded corners, fill=setcolor] (-2.5,-1.3) rectangle ++(2,2.55);
        \draw[draw=setcolor, rounded corners, fill=setcolor] (0.5,-1.3) rectangle ++(2,2.55);
        \node at (0,0) {
            \begin{tikzcd}[column sep=1cm,row sep=0.01, every arrow/.append style={|-Triangle}]
                \text{Gioele}\arrow{dr}     & \text{Gioele} \\
                \text{Andrea}\arrow{dr}     & \text{Andrea} \\
                \text{Jonathan}             & \text{Jonathan} \\
                \text{Alessandro}\arrow{dr} & \text{Alessandro} \\
                \text{Emilio}               & \text{Emilio}
            \end{tikzcd}
        };
    \end{tikzpicture}
