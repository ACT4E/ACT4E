...
\begin{example}
    ...
    For example, consider~\nonNegReals as a subset of itself.
    ...
    One way to make~$\tupp{ \nonNegReals,{{\Rleq}}} $ a \CPO is by adding an artificial \SY{top element}~$\postop$ that we think as ``a point at infinity''.
...
It also might have multiple fixed points; and also in that case there might not be a \emph{least} fixed point.
...
\todotext{
    @Jonathan: We actually are not checking that $\phi$ is Scott continuous and that doesn't seem obvious.
}