...
\begin{remark}
    \scottcontinuity is not the same as the notion of continuity as used in analysis
    ...
    \end{equation}
    which is, in spirit, the same as
...
We call this map $\phi: \powerset \setA \to \powerset \setA$, so that
...
\todographics{@Gioele: Make a figure instead of the equation below}
...
\begin{equation}
    \posA = \upit \subA  = \makeset{ \subB \setin \powerset \setA : \subA \subseteq \subB }.
\end{equation}
...
% 
\begin{equation}
    \phi(\subA) = \min_{\subseteq}\, \makeset{ \subB \in \posA : \subB = m(\subB) }
\end{equation}
...
\begin{equation}
    \posbot\posleq \mora(\posbot)\posleq \mora(\mora(\posbot))\posleq\cdots\posleq \mora^{(n)}(\posbot)\leq\cdots.
\end{equation}
Because the bottom $\posA$ is $\subA$, we can rewrite it as:
\begin{equation}
    \subA \subseteq  m(\subA ) \subseteq m(m(\subA)) \subseteq m(m(m(\subA))) \dots .
\end{equation}
