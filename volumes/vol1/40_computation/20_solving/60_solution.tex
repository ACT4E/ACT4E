...
This is a simple example that can show two interesting properties
of MCDPs:
\begin{enumerate}
...
The diagram contains three primitive DPIs: $\adp_{1},$
$\adp_{2}$ (used twice), and $\adp_{3}$.
Their \ftor maps
are:
\begin{eqnarray*}
...
for~\ftor.
By substituting in~\cref{eq:h} the definitions given in~\cref{def:opseries1,def:oploop1}, we obtain that
...
\begin{figure}
    ...
    \end{centering}
    \caption{Kleene ascent to solve the problem~\cref{eq:example-1}
    for $\F{c}=20$.
...
the antichain~$\RR_{\infty}$ divides~\ressp in two.
Below
the antichain, all resources are infeasible.
However, above the antichain
...
The sequence~$\RR_{k}$ can be considered
