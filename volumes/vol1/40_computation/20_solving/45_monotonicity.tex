...
\begin{proof}
    ...
    \begin{eqnarray}
        ...
        \RR                     & \mapsto & \Min_{\resleq}\bigcup_{\res\in\RR}\ftor_{\fun_{1}}(\res)\ \setintersection\ \upit\res.\nonumber
    \end{eqnarray}
    If we want the \emph{minimal} resources, we are looking for the \emph{least} antichain:
    \begin{equation*}
        \min_{\posleqof{\Aressp}}\{\,\RR\in\Aressp\colon\ \Phi_{\fun_{1}}(\RR)\posleqof{\Aressp}\RR\,\},
    \end{equation*}
...
\begin{lemma}
    ...
    Then
    \begin{equation*}
        a\in A\qquad\equiv\qquad\{a\}=A\,\setintersection\upit a.
    \end{equation*}
\end{lemma}
...
\begin{lemma}
    ...
    For~$A,B,C,D\in\antichains\posA$, $A\posleqof{\Aressp}C$
