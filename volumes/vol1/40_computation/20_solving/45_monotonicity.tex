...
\todojira{257}{All of these are conseguence of functor and DP, UPos formalization. What should we do?}


\begin{proposition}
    \label{pro:dppar-monotone}If~$\adp_{1}$ and~$\adp_{2}$ are
...
\begin{proposition}
    \label{pro:series-monotone}If $\adp_{1}$ and $\adp_{2}$ are
...
\begin{proposition}
    \label{pro:loop-continuous}If $\adp$ is monotone (\cref{def:DPI}),
    so is~$\dploop(\adp)$.
\end{proposition}
\begin{proof}
