...
\begin{definition}
    ...
    \begin{eqnarray*}
        ...
        \fun& \mapsto & \resMin\{\req(\imp)\mid\left(\imp\setin\impsp\right)\,\wedge\,\left(\fun\posleq\prov(\imp)\right)\}.
    \end{eqnarray*}
...
\todotext{Say somewhere in the above how we are ordering antichains.
    Also, is "objective function" above a technical term?
    Is it clear what is meant?
}
...
\begin{remark}
    ...
    In short, the condition is satisfied if in the poset \todotext{In the poset or resources or in the poset of upper subsets of the poset or resources?
        And/or this equivalent?
    } no \emph{infinite descending chain} exists.
    For instance, consider~$\tup{\mathbb{Z},\leq}$.
    The chain $-1, -2, -3, \ldots$ is an infinite descending chain.
\end{remark}
...
\todotextjira{299}{@Andrea: I commented out the below because the lemma is not included in public. }
%where~$\posleqof{\Aressp}$ is the order on antichains defined in \cref{lem:antichains-are-poset}.
...
\todotext{J: Is $\varphi$ a good choice of notation here?
    It seems better to choose a notation that is more similar to $h$.
    The distance/clash between latin and greek seems too large to me.
}
