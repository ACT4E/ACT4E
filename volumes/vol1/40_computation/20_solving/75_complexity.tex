...
\Cref{prop:complexity} above assumes that the MCDP is already in the form $\adp=\dploop(\adp_{0})$, and relates the complexity to the poset~$\ressp_{0}$.
...
\begin{definition}
	...
	is defined as
	\begin{equation}
		\text{DC}(\tup{ \cdpiN,\mathcal{E}} )=\min_{F\text{ is an AFS}}\posetwidth(\prod_{e\in F}\ressp_{e}).
		\label{eq:complexity}
	\end{equation}
\end{definition}
