...
Consider first the case of an MCDP that can be described as~$\adp=\dploop(\adp_{0})$, where~$\adp_{0}$ is an MCDP that is described only using the~$\dpseries$ and~$\dppar$ operators. Suppose that~$\adp_{0}$ has resource space~\ressp.
