...
Solving a CDP with cycles reduces to computing a Kleene ascent sequence~$\RR_{k}$.
...
If the iteration converges to a non-empty \SY{antichain}~$\RR_{\infty}$, the \SY{antichain}~$\RR_{\infty}$ divides~\ressp into two.
