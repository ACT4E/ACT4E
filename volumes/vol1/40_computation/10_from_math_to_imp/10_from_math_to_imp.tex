...
\todojira{250}{@Andrea: Do discussion: descriptive \vs actionable, workflow systematization}
...
Mathematically, for any vector~$v\setin \reals^3$, there exists a~$n\setin \reals^3$ such that~$\tup{n,v}=\Vert v \Vert$.
How many such~$n$ are there?
If~$v=\begin{bmatrix}
        0 & 0 & 0
    \end{bmatrix}^\intercal$ any~$n\setin \reals^3$ will satisfy the condition.
...
\todotextjira{427}{@Andrea: Not just any map, but the curried and op-ed version of the original design problem, right?
    Also, we need to say explicitly here that we are ordering upper sets by containment.
...
\end{equation}
%
\todotextjira{419}{@Andrea: J: is this perhaps a bit misleading?
    Isn't what we actually want a map to antichains?
    Why not say it here rather than below?
    From a mathematical perspective, $h$ above is just the curried version of the original design problem \dots  so it seems almost a bit strange to pose that as the problem perhaps?
}
%
