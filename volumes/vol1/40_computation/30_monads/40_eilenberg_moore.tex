...
Let $\tup{\monA, \moncomp, \monunit}$ be a monad on a category $\CatC$, and let $\Obja$ be an object of $\CatC$. Your task is to prove that the object $\monA \Obja$, together with the morphism
\begin{equation}
\moncomp_\Obja \colon \monA \monA \Obja \mto \monA \Obja
\end{equation}
defines an algebra for the monad $\monA$. \emph{Hint}: use the axioms/conditions in the definition of a monad. 
\end{gradedexercise}
