...
\begin{gradedexercise}[\exname{PowersetImageFunctor}]\label{ex:PowersetImageFunctor}
...
\todojira{229}{@Marius: you could write a solution for this exercise}
...
\todotextjira{230}{I'm not sure if we every introduced ``whiskering'' and the notation for it (which is used in the diagrams in the definition of a monad). We should check and if needed introduce this in the section on natural transformations.}
...
\begin{gradedexercise}[\exname{PowersetMonad}]\label{ex:PowersetMonad}
...
\todojira{231}{@Marius: you could write a solution for this exercise. explain how the three conditions in the definition of a monad spell out in the case of the powerset monad (as Paolo does in his notes)}
...
\begin{equation}\label{eq:fruit-sum-7-bis}
 [ [ \sapple ]  * [ \sbanana ] ] *  [[\scarrot]]
\end{equation}
...
\todo{I think we should use two different notations for the functor for the monad and the actual arrow construction...}
\begin{equation}
...
\begin{center}
  \todographicsjira{260}{add associativity diagram in tikz. Take it from GZ recital.}
\end{center}
...
\begin{center}
  \todographicsjira{261}{add left and right diagrams in tikz. Take it from GZ recital.}
\end{center}