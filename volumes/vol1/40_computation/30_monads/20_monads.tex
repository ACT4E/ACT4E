...
Firstly, note that the ``powerset operation'' can be viewed as \SY{functor} $\Set \fto \Set$.
...
\begin{gradedexercise}[\exname{PowersetImageFunctor}]
    ...
    Prove that ``taking the \SY{powerset}'' defines a \SY{functor} $\powerset : \Set \fto \Set$ if we define its component on morphisms as in \cref{eq:powerset-functor}.
\end{gradedexercise}
...
\begin{ctdefinition}[Monad]
    ...
    \begin{enumerate}
        ...
        \item A natural transformation~$\monunit \colon \funidC \nto \monA$, called the \emph{unit}.
    \end{enumerate}
    \condit
    \begin{enumerate}
...
\begin{gradedexercise}[\exname{PowersetMonad}]
    ...
    The aim of this exercise is to prove in full detail that the \SY{powerset} \SY{functor} $\stylefunctors{\powerset} : \Set \fto \Set$ from \cref{ex:PowersetImageFunctor} is a monad, when equipped with the following \emph{unit} and \emph{multiplication}.
    We define $\monunit : \stylefunctors{\text{Id}}_{\Set}  \nto \stylefunctors{\powerset}$ and $\moncomp : \stylefunctors{\powerset} \fthen \stylefunctors{\powerset} \nto \stylefunctors{\powerset}$ in terms of components: given an object $\setA \setin \Set$, let
    \todotext{@JL: write these using defmap, on two lines}
    \begin{equation}
        \label{eq:powerset-mult-component}
        \moncomp_{\setA} : (\stylefunctors{\powerset} \fthen \stylefunctors{\powerset})(\setA) \mto \stylefunctors{\powerset}(\setA), \ \stylesets{D} \mapsto \bigsetunion_{\subA \setin \stylesets{D}} \subA
    \end{equation}
    and
    \begin{equation}
        \label{eq:powerset-unit-component}
        \monunit{_\setA} \colon \setA \mto \stylefunctors{\powerset}(\setA), \ \ela \mapsto \makeset{ \ela }.
    \end{equation}
...
Given \Pos, we define the action of the $\Arrow$ \SY{functor} on objects (posets) as
...
\end{equation}
%
First, we need to show that both the multiplication and the unit are indeed natural transformations.
...
We now check that the unit defines a natural transformation.
We have
%
\begin{equation}
