...
Firstly, note that the ``powerset operation'' can be viewed as \SY{functor} $\Set \mto \Set$.
...
\begin{gradedexercise}[\exname{PowersetImageFunctor}]
    ...
    Prove that ``taking the \SY{powerset}'' defines a \SY{functor} $\powerset : \Set \mto \Set$ if we define its component on morphisms as in \cref{eq:powerset-functor}.
\end{gradedexercise}
...
\begin{gradedexercise}[\exname{PowersetMonad}]
    ...
    The aim of this exercise is to prove in full detail that the \SY{powerset} \SY{functor} $\stylefunctors{\powerset} : \Set \mto \Set$ from \cref{ex:PowersetImageFunctor} is a monad, when equipped with the following \emph{unit} and \emph{multiplication}.
