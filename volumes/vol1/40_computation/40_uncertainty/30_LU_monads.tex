...
\begin{lemma}
  ...
  Given a poset~$\posA$ and a family of upper sets of~$\posA$, denoted~$\{\posA_i\}_{i\in I}$ (with index set~$I$), their union is also an upper set.
\end{lemma}

\begin{lemma}
  \label{lem:setunionset}
  Given a set~$\setA$ and a family of subsets of~$\setA$, denoted~$\{\setA_i\}_{i\in I}$ (with index set~$I$), one has:
  \begin{equation}
