...
In this section, we propose another example of monads related to \SY{posets} and upper/lower sets.
...
\begin{definition}[$\Uendo$ endofunctor]
    ...
    \begin{enumerate}
        \item \emph{On objects}: Given a poset~$\posA \setin \Obof\Pos$,~$\Uendo$ maps~\posA to its upper set\footnote{Recall that in \cref{lem:u_bounded_lat} we proved that the upper set is itself an object of \Pos.
              }.
        \item \emph{On morphisms}: Given \SY{posets}~$\posA,\posB$, and a \SY{monotone maps}~$\mora \colon \posA\mto \posB$, the~$\Uendo$ endofunctor acts as:
              \begin{equation}
                  \begin{aligned}
...
\begin{lemma}
    ...
    The~$\Uendo$ endofunctor is indeed a \SY{functor}.
\end{lemma}
