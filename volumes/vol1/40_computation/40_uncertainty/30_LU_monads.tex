...
\todotext{JL: the phrasing here seems unnecessarily complicated... maybe we phrase the lemma as something like ``$\upit  f( \upit S) = \upit f(S) $" for any set $S$ and any monotone map $f$ ??}
...
\end{proof}
\todotext{JL: I wouldn't make the above its own lemma, that seems a bit silly/overkill.}
...
\begin{proof}
  $\Uendo$ has a valid form \todotext{JL: Not clear to me what ``valid form'' means} and given a poset~$\posA$, maps~$\catid_\posA$ to~$\catid_{\Up \posA}$.
  \todotext{JL: I think the compatibility with identities is non-trivial and should be shown here.}
  ...
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  \todotext{JL: I think there is a mistake: the source and target should be UP and UR respectively, not ``Pos''.}
