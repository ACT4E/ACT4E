...
One can define a \emph{poset of intervals} on~$\posA$, denoted~$\posint{P}$. Intervals can be ordered using the following rule:
\begin{equation}
...
\begin{equation}
    ...
    &=\moncomp_{\uncmon}^{\posAn}(\interval{\interval{{\F{\posAeln_{11}}}, {\F{\posAeln_{12}}}},\interval{{\F{\posAeln_{21}}}, {\F{\posAeln_{22}}}}}^*, \interval{\R{\posAeln_1},\R{\posAeln_2}})\wedge \moncomp_{\uncmon}^{\posAn}(\interval{\interval{{\F{\posAeln_{11}'}}, {\F{\posAeln_{12}'}}},\interval{{\F{\posAeln_{21}'}}, {\F{\posAeln_{22}'}}}}^*, \interval{\R{\posAeln_1},\R{\posAeln_2}})
    \end{aligned}
\end{equation}
\todo{copy from notes last part}
\end{proof}

\todo{Copy from notes the proof that the monad is enriched in Pos as well, meaning that lower dp implies lower unc (from which the RAL construction follows)}
