...
\begin{marginfigure}
    ...
    \todographics{@Gioele: redo picture}
\end{marginfigure}
...
\begin{figure}[h!]
    ...
    \caption{}
    \todographics{@Gioele: redo picture, make DP using \DP}
\end{figure}
...
Suppose that we have an MCDP composed of many DPs, among which one named~$\ftor_{a}$ (\cref{fig:consider1}).
...
Given the semantics of interconnections of UDPs, this is equivalent to considering a pair of MCDPs, in which we choose either the lower bound or the upper bound~(\cref{fig:pair}).
...
\begin{marginfigure}
    \centering
    \subfloat{\includegraphics[scale=0.53]{unc_approx1a}}

    \subfloat{\includegraphics[scale=0.53]{unc_approx1b}}

    \subfloat{\includegraphics[scale=0.53]{unc_approx1c}}

    \caption{
        The identity and its two relaxations~$\ufloor_{\alpha}$ and $\uceil_{\alpha}$.
    }
    \label{fig:identity_approximation}
    \todographics{@Gioele: redo pictures.
...
We can take any edge, or multiple edges, in an MCDP and apply this relaxation.
...
\begin{figure}[h]
    ...
    \todographics{@Gioele: do SURGERY on figure.
...
Even if we are happy to first approximation $\reals$ using the (finite) floating point numbers, the representation that antichain would take billions of elements.
...
\begin{marginfigure}
    ...
    \todographics{@Gioele: redo the picture}
\end{marginfigure}
...
\begin{align*}
    ...
    \fun_{1}                         & \mapsto\makeset{\tupp{ x,\fun_{1}-x} \mid x \in \interv0{\fun_1}}.
\end{align*}
\todotext{Redo using defmapperiod}
