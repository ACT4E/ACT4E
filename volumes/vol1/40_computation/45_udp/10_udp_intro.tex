...
\end{equation}
%
Therefore, the sequence $\UId_{\alpha}$ is a descending chain that converges to~$\UId$ as~$\alpha\rightarrow0$~(\cref{fig:other}).
...
Formally, we first introduce an identity~$\UId$ and then relax it using~$\UId_{\alpha}$~(\cref{fig:introduce}).
