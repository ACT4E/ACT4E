...
The partial order~$\udpleq$ has a top~$\postop_{\udpsp}=\interv {\posbot_{\dpsp}} {\postop_{\dpsp}}.
$
...
A pair $\interv\ftor\ftor$ is a minimal element of~$\udpsp$, because it cannot be dominated by any other.
...
\Cref{fig:Example1} shows the reference model for these applications.
...
\subsection{Application: Introducing Tolerances}
...
to be the floor and ceiling functions with step size~$\alpha$~(\cref{fig:identity_approximation}).
By construction, $\ufloor_{\alpha}\dpleq\catid\dpleq\uceil_{\alpha}.
$
...
\begin{figure}[h]
    \includegraphics[scale=0.33]{unc_make_lower}
    \caption{}
