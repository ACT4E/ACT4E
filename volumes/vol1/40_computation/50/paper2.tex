...
The user then queries the model, to obtain the design with minimal resource usage, subject to a lower bound on the provided functionality.
This paper concerns the introduction of uncertainty in the framework.
Uncertainty has two roles: first, it allows dealing with limited knowledge in the models; second, it also can be used to generate consistent relaxations of a problem, as the computation requirements can be lowered should the user accept some uncertainty in the answer.

\section{Introduction}

The design of a robotic platform involves the choice and configuration of many hardware and software subsystems (actuation, energetics, perception, control, \dots) in a harmonious entity in which all \emph{co-design constraints} are respected.
...
Therefore, robot design is a lengthy design process mainly based on empirical evaluation and trial and error.
...
For a poset~$\left\langle \posA,\posleq\right\rangle $, these are sets of the type~$\makeset{x\setin\posA\colon a\posleq x\posleq b}$.
...
With these additions, the MCDP framework allows describing even richer design problems and to efficiently solve them.
...
The graphical representation uses nodes for DPs and green (red) edges for \F{functionality} and \R{resources}~(\cref{fig:dp}).
...
\begin{definition}[Algebraic definition Monotone Co-Design Problems]
    ...
    \begin{enumerate}
        ...
        \item The term~$\atree$ in the $\makeset{\dpseries,\dppar,\dploop}$ algebra
              describes the structure of the graph:
              \begin{equation}
                  \atree\setin\terms(\makeset{\dpseries,\dppar,\dploop},\atoms).
              \end{equation}
...
\begin{example}
    ...
    $\atoms=\makeset{a,b,c}$, the term $\atree=\dploop(\dpseries(a,\dppar(b,c)),$
    plus the valuation $\val:\makeset{a\mapsto\ftor_{a},b\mapsto\ftor_{b},c\mapsto\ftor_{c}}.
    $
...
\Cref{def:dpsem} gives a way to evaluate the map~$\ftor$ for the graph, given the maps~$\makeset{\ftor{}_{a}\mid a\setin\atoms}$ for the leaves.
...
\begin{example}
    ...
    \begin{equation}
        \begin{cases}
            {\colR R}
            _{0}            & \leftarrow\makeset{\posbot_{\ressp}}, \\
...
\begin{minipage}[t]{0.4\columnwidth}
    \begin{align*}
        ...
        \fun                   & \mapsto\makeset{\posbot_{\ressp}}.
    \end{align*}
...
The partial order~$\udpleq$ has a top~$\postop_{\UDP}=\left\langle \posbot_{\dpsp},\postop_{\dpsp}\right\rangle.
$
...
A pair $\langle\ftor,\ftor\rangle$ is a minimal element of~$\UDP$, because it cannot be dominated by any other.
...
The only difference between \cref{def:MCDP-algebraic} and~\cref{def:umcdp} below is that the valuation assigns to each atom a UDP, rather than a~DP.
\begin{definition}[Algebraic definition of UMCDPs]
    \label{def:umcdp}
    An Uncertain MCDP (UMCDP) is a tuple~$\left\langle \atoms,\atree,\val\right\rangle$, where~$\atoms$ is a set of atoms, $\atree\setin\terms(\makeset{\dpseries,\dppar,\dploop},\atoms)$ is the algebraic representation of the graph, and~$\val:\atoms\rightarrow\UDP$ is a valuation that assigns to each atom a UDP.
\end{definition}
...
\begin{definition}[Semantics of UMCDPs]
    ...
    \begin{equation}
        \udpsem\llbracket\left\langle \atoms,\atree,\val\right\rangle \rrbracket\setin\UDP,
    \end{equation}
