...
(If~$\min S$ exists, then~$\Min S=\makeset{\min S}$.)
...
\begin{align*}
    ...
    S                          & \mapsto\makeset{x\setin S:\ (y\setin S)\wedge(y\posleq x)\Imp(x=y)\ }.
\end{align*}
...
\begin{definition}[Upper closure]
    ...
    \begin{eqnarray*}
        ...
        S & \mapsto & \makeset{y\setin\posA:\exists\,x\setin S:x\posleq y}.
    \end{eqnarray*}
\end{definition}
...
is the singleton containing only the bottom for~$\posA$: $\bot_{\antichains\posA}=\makeset{\bot_{\posA}}$.

Call ``Monotone Co-Design Problems'' (MCDPs) the set of CDPIs for which all subproblems respect the conditions in~\XXX
...
% 2) An \textbf{algorithmic result }(\cref{thm:CDP-solvig}) says that the functionality-resources map~$\ftor$ for the entire MCDP has an explicit expression in terms of the maps~$\makeset{\ftor_{\cdpin},\,\cdpin\setin\cdpiN}$ for the subproblems.
