...
So far in the book we have discussed Applied Category Theory from a modeling perspective:
we have described various mathematical structures, their properties, and how they can be used for
modeling certain engineering phenomena.
...
How many such~$n$ are there? If~$v=\begin{bmatrix}
                                       0& 0& 0
\end{bmatrix}^\intercal$ any~$n\in \reals^3$ will satisfy the condition.
...
\todotext{@Andrea: Not just any map, but the curried and op-ed version of the original design problem, right? Also, we need to say explicitly here that we are ordering upper sets by containment.
We can refer back to \cref{ex:CurryingDesignProblems} which was about showing that design problems can be viewed this way.}
%
\begin{equation}
...
\end{equation}
%
\todotext{J: is this perhaps a bit misleading? Isn't what we actually want a map to antichains? Why not say it here rather than below? From a mathematical perspective, $h$ above is just the curried version of the original design problem... so it seems almost a bit strange to pose that as the problem perhaps?}
%
...
\todotextjira{298}{@Andrea: Make the above remark clearer or more precise. (I which include a recollection of the relevant fact from analysis).}
...
\todojira{253}{@Andrea: I think it is good to have an overview that connects to the four phases before starting the meat.
Should we also talk about functors DP to UPos, UPoscomp, UPosdiscrete?}
