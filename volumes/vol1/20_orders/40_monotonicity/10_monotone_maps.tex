...
\devel{\todojira{80}{Add some motivation from design.
...
\end{definition}
\begin{remark}
    Given a poset~$\posA$, the map~$\id_\posA\colon \posA \mto \posA$ is monotone, since one has:
    \begin{equation*}
        \begin{aligned}
...
\begin{example}[Rounding functions]
    ...
    \todojira{83}{Define IEEE754 formally}
    \todographicsjira{84}{Add picture with 3}
\end{example}
