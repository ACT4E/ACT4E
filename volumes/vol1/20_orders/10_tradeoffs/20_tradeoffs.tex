...
\showslides{
    \begin{forslides}
        ...
        \begin{definition}
            \label{def:posdef}
            A symmetric matrix~$\mat{M}\in \reals^{n\cartprod n}$ is \emph{positive semi-definite} if~$x^\intercal \mat{M}x\geq 0$ for all non-zero~$x\in \reals^n$. We call the set of all such matrices~$\mathcal{P}^n$.
        \end{definition}
        \begin{equation*}
            \label{eq:posdef_1}
...
\begin{definition}[Pre-ordered set]
    ...
    \begin{compactitem}
        \item \emph{Reflexive:}
        \begin{equation}
            \prftree{\true}{\posAel \posAleq \posAel}
        \end{equation}
...
\begin{definition}[Partially ordered set]
    \label{def:poset}
    A pre-ordered set~$\posA = \tupp{\posAset,\posAleq}$ is a \emph{partially-ordered set (\iindex{poset})} if the relation~$\posAleq$ is \emph{antisymmetric}. In other words, if:
    \begin{equation}
...
\begin{definition}[Totally ordered set]
    ...
    In other words, if:
    \begin{equation}
        \prftree{\true}{(\posAel \posAleq \posBel) \vee (\posBel \posAleq \posAel)}
    \end{equation}
\end{definition}
...
\begin{example}
    Consider a poset~$\posA$ over the set~$\posAset=\{a,b,c,d,e\}$ with~$a\posAleq b$,~$a\posAleq c$,~$d\posAleq c$, and~$d\posAleq e$.
...
\begin{example}
    A symmetric matrix~$\mat{M}\in \reals^{n\cartprod n}$ is \emph{positive semi-definite} if~$x^\intercal \mat{M}x\geq 0$ for all non-zero~$x\in \reals^n$.
...
\begin{definition}[Upper bounds in a poset]
...
\begin{definition}[Lower bounds in a poset]
