...
\begin{definition}[Upper bounds in a poset]\label{def:least-upper-bound}
    ...
    $$
    A \emph{least upper bound} of $\setA \subseteq \posA$, if it exists, is a least element among the upper bounds of~$\setA$. It is denoted $\sup\setA$ or $\join \setA$, and also called the \emph{supremum} or \emph{join} of $\setA$. So, given $\setA \subseteq \posA$ and $\elb \in \posA$, $\elb =  \join \setA$ if and only if 
\begin{enumerate}
...
\begin{example}
    Least upper bounds need not necessarily exist even in total orders. For instance,
    \begin{equation*}
...
Dually, one can define lower bounds and greatest lower bounds.
...
\begin{definition}[Lower bounds in a poset]
    ...
    The \emph{greatest lower bound}, if it exists, is the greatest among the lower bounds of~$\setA$. This is also called \emph{meet}, \emph{infimum}, or~$\meet \setA$.
\end{definition}
