...
The order can be interpreted as an inclusion of ellipsoids.
    \todo{add practical example and draw ellipse inclusion in 2D}
...
Let $\setA$ be the set of all polynomials with coefficients in $\reals$. Recall that a polynomial $p$ \emph{divides} a polynomial $q$ if there exists a polynomial $m$ such that $p \cdot m = q$. If $p$ divides $q$ we denote this by $p \vert q$. Divisibility defines an endorelation on $\setA$ by saying $p$ is related to $r$ iff $p \vert q$. Does this define a preorder structure on $\setA$? Does is define a poset structure on $\setA$? Justify your answer. 
\end{gradedexercise}
