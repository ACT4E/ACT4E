...
\begin{definition}[Lattice]
  \label{def:lattice}
  A \emph{lattice} is a poset~$\tupp{\posA,\posAleq}$ with the additional property that, for any two-element subset $\{\ela, \elb \} \subseteq \posA$, both the join $\join \{\ela, \elb \}$ and the meet $\meet \{\ela, \elb \}$ exist. Usually these are written using infix notation as $\ela \join \elb$ and $\ela \meet \elb$, respectively. 
\end{definition}

\todographicsjira{74}{Add the wooden lattice figure here.}

\todojira{75}{These can be defined for preorders as well... move before.}
\begin{remark}[Bounded lattices]\label{rem:bounded-lattices}
  \label{def:top}
  \label{def:bot}
