...
Recall that a \SY{comonad} on a \emph{category}~$\CatC$ (\cref{def:comonad}) consists of an endofunctor~$\comonW \colon \CatC \fto \CatC$ together with natural transformations for \emph{comultiplication} (duplication)~$\comondu \colon \comonW \nto \comonWW$ and \emph{counit} (extraction)~$\comonex \colon \comonW \nto \funidC$, satisfying coassociativity and counitality.

This is analogous to the definition of monad, but with the ``natural transformation arrows'' reversed: the multiplication~$\moncomp \colon \monA \mthen \monA \nto \monA$ becomes a comultiplication~$\comondu \colon \comonW \nto \comonW \mthen \comonW$, and the unit~$\monunit \colon \catidof{\Obja} \nto \monA$ becomes a counit~$\comonex \colon \comonW \nto \catidof{\Obja}$.

Similarly, in the context of bicategories, the definition of comonad is obtained from the definition of a monad by reversing the direction of all the 2-cells.
Furthermore, whenever an adjunction~$\funl \adjunction \funr$ exists in a bicategory, the composite~$\funl \mthen \funr$ carries a monad structure and the composite~$\funr \mthen \funl$ carries a comonad structure. Thus, also for bicategories, monads and comonads may be seen as two sides of a single coin.

\subsection{The definition}

A comonad in a bicategory is an endo-1-cell equipped with a comultiplication and counit, and satisfying axioms of ``co-associativity'' and ``co-unitality''.
    ...
    \begin{enumerate}
        \item \emph{Coassociativity:}
              ...
              \begin{equation}\label{eq:bicat-comonad-coassoc}
                  \begin{tikzcd}[every arrow/.append style={arrowntrafostyle}, column sep=large, row sep=large]
