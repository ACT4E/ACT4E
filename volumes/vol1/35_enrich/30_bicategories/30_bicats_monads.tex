...
\todotext{rewrite/improve the intro -- give a bit more context/motivation}

Just as the notion of adjunction generalizes from ordinary categories to bicategories, so does the notion of \SY{monad}.

Recall that a monad on a category~$\CatC$ involves an endofunctor~$\monA \colon \CatC \fto \CatC$, natural transformations for multiplication and unit, and associativity and unitality conditions --- all of which take place inside the hom-category~$\Category(\CatC, \CatC)$.
