...
\begin{example}
  ...
  \begin{figure}[h!]
    ...
    \end{center}
    \caption{Example of feedback in design problems. \todo{close the loop beloew} }
...
\begin{figure}[h!]
  ...
  \caption{The shaded area marks a portion of the feasibility set of a traced design problem, `motor'. Note that it is not an upper set in the subspace ``weight $\times$ weight'' of `motor'. \todo{close the loop beloew}}
...
\begin{figure}[h!]
  ...
  \todo{have C be below so that we can close the looop below}
  }
  \label{fig:extrace_1}
\end{figure}
...
The connection between the two is more apparent if one decomposes the trace of a square matrix as a set of properties that any linear map from a space to itself should satisfy. One can find the trace axioms in any standard text on category theory, e.g. \cite{maclane}; these are equivalent to certain diagrammatic conditions \cite{joyal96}, as in \cref{tab:traceaxioms}.
