...
\begin{example}
  ...
  \end{equation}
  We now have to think about \R{resources} and \F{functionalities}.
  Each motor \R{requires} some \R{weight} (in \unit[]{g}), \R{power} (in \unit[]{W}), and has some \R{cost} (in \stdcurr), and \F{provides} some \F{torque} (in~$\unit[]{kg\cdot cm}$).
...
\begin{example}
  The \F{functionality} of a DC motor~(\cref{fig:dc_motor}) is to provide a certain \F{speed} and \F{torque}, and the \R{resources} are \R{current} and \R{voltage}.
\end{example}
