...
Given the definitions of~$\meet$ and~$\join$ in the previous sections, we can prove that every \DP \SY{hom-sets} have a lattice structure.
...
\begin{lemma}
    ...
    $\HomSet{\DP}{\F{\posgenA}}{\R{\posgenB}}$ is a bounded lattice with union~$\join$ as join, intersection~$\meet$ as meet, top~$\postop_{\F{\posgenA},\R{\posgenB}}$ and bottom~$\posbot_{\F{\posgenA},\R{\posgenB}}$.
\end{lemma}
...
\begin{definition}[Distributive Lattice]
    ...
    A lattice~$\posA=\tup{\posA,\meet, \join}$ is a \maindef{distributive lattice} if for all~$\posela,\poselb,\poselc\setin \posAset$:
    \begin{equation}
