...
Given the definitions of~$\meet$ and~$\join$ in the previous sections, we can prove that every \DP \SY{hom-sets} have a \SY{lattice} structure.

This \SY{lattice} is bounded by a ``true'' and a ``false'' DP.
For any functionality-resource pair~$\F{\posgenA},\R{\posgenB}$, we denote by~$\postop_{\F{\posgenA},\R{\posgenB}}$ the \SY{design problem} which is always feasible.
We denote by~$\posbot_{\F{\posgenA},\R{\posgenB}}$ the \SY{design problem} which is never feasible, for any functionality-resource pair~$\F{\posgenA},\R{\posgenB}$.
\todotext{\alphubel: define $\postop$ and $\posbot$ explicitly }
\begin{lemma}
    \label{lem:dpboundedlattice}
    $\HomSet{\DP}{\F{\posgenA}}{\R{\posgenB}}$ is a \SY{bounded lattice} with union~$\join$ as \SY{join}, intersection~$\meet$ as \SY{meet}, top~$\postop_{\F{\posgenA},\R{\posgenB}}$ and bottom~$\posbot_{\F{\posgenA},\R{\posgenB}}$.
\end{lemma}
...
\begin{proof}
    ...
    Their greatest lower bound (meet) is~$\adpa\meet\adpb$, since it is the greatest \SY{design problem} implying both~$\adpa$ and~$\adpb$.
    Their least upper bound (join), instead, is~$\adpa\join \adpb$, since it is the least \SY{design problem} implied by both~$\adpa$ and~$\adpb$.
    This proves that~$\Hom_\DP$ is a \SY{lattice}.
    To prove that it is bounded, we identify the top element as~$\postop_{\F{\posgenA},\R{\posgenB}}$ (it is implied by all other \SY{design problems}) and the bottom element as~$\posbot_{\F{\posgenA},\R{\posgenB}}$ (it implies by all the other \SY{design problems}).
\end{proof}
...
\begin{definition}[Complete Lattice]
    ...
    A poset~$\posA=\tup{\posAset,\posleq}$ is a \maindef{complete lattice} if every subset~\subA of~\posAset has both a \emph{greatest lower bound} (often referred to as the \emph{infimum, meet}) and a \emph{least upper bound} (often referred to as the \emph{supremum, join}) in~$\tup{\posAset, \posleq}$.
\end{definition}
...
\begin{proof}
    ...
    In other words, for any pair~$\tup{\FposgenAelop,\RposgenBel}\setin \F{\posgenA}\op\Ctimes \R{\posgenB}$, we need to show~$(\bigvee_{\adp \setin \setB} \adp)(\FposgenAelop,\RposgenBel)\posleqof\Bool \adpb(\FposgenAelop,\RposgenBel)$.
    ...
    In other words, for any pair~$\tup{\FposgenAelop,\RposgenBel}\setin \F{\posgenA}\op\Ctimes \R{\posgenB}$, we need to show~$\adpb(\FposgenAelop,\RposgenBel) \posleqof\Bool (\bigwedge_{\adp \setin \setB} \adp)(\FposgenAelop,\RposgenBel)$.
    Fix any~$\tup{\FposgenAelop,\RposgenBel}$.
...
\begin{definition}[Distributive Lattice]
    ...
    A \SY{lattice}~$\posA=\tup{\posA,\meet, \join}$ is a \maindef{distributive lattice} if for all~$\posela,\poselb,\poselc\setin \posAset$:
    \begin{equation}
