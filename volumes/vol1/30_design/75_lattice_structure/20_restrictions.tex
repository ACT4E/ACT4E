...
We define the \emph{union}~$\adpa \join \adpb$ to be the \SY{design problem} which is feasible whenever \emph{either}~$\adpa$ or~$\adpb$ is feasible.
...
\begin{definition}[Union of design problems]
    \label{def:union_dp}
    Given two \SY{design problems}~$\adpa \colon \F{\posgenA} \profto \R{\posgenB}$ and~$\adpb\colon \F{\posgenA} \profto \R{\posgenB}$, their \emph{union}~$\adpa \join \adpb\colon \F{\posgenA} \profto \R{\posgenB}$ is defined by
    \begin{equation}
...
Given two \SY{design problems}~$\adpa, \adpb \colon \F{\posgenA} \profto \R{\posgenB}$, we can define a \SY{design problem}~$\adpa \meet \adpb$ that is feasible if and only if~$\adpa$ and~$\adpb$ are both feasible.
