...
\begin{definition}[Lattice]
    \label{def:lattice}
    A \maindef{lattice} is a poset~$\posA=\tupp{\posAset,\posAleq}$ with the additional property that, for any two-element subset~$\makeset{\ela, \elb} \setsubseteq \posA$, both the \SY{join}~$\join \makeset{\ela, \elb}$ and the \SY{meet}~$\meet \makeset{\ela, \elb }$ exist.
...
\begin{definition}[Bounded lattices]
    ...
    If both a top and a bottom exist, we call the \SY{lattice} \emph{bounded}, and denote it by~$\posA = \tup{\posAset,\ordleq,\join,\meet,\posbot,\postop}$.
\end{definition}
...
\begin{proof}
    ...
    We have therefore proved that~$\uppersets \posA=\tup{\setOfUppersets \posA,\setsupseteq}$ is a \SY{lattice}.
