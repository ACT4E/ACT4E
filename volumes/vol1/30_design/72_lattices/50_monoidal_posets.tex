...
In detail, monotonicity means that, for all~$\posela_1, \posela_2, \poselb_1, \poselb_2 \setin \posA$:
\begin{equation}
...
\begin{example}[Reals with addition]
    ...
    \begin{enumerate}[(a)]
        \item Given~$p_1,p_2,q_1,q_2\setin \reals$, we know:
              \begin{equation*}
                  \prfperiod{
                      p_1\Rleq  p_2
                  }{
              ...
              \end{equation*}
        \item $0+p=p+0=0$,~$\forall p\setin \reals$.
        \item $(p+q)+r=p+(q+r)$,~$\forall p,q,r\setin \reals$.
    \end{enumerate}
\end{example}
