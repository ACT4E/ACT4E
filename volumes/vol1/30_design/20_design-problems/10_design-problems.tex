...
\begin{example}[Motor design]
    ...
    a given set. The \F{functionality} of a motor could be parametrized
    by \textF{torque} and \textF{speed}. The \R{resources} to consider
...
\todostructure{Yet another different model of a motor. Either harmonize all in one, or explicitly
have discussion about how the same thing can have different models}
\begin{example}
...
\end{example}
\todographics{296}{Put here an additional crank-rocker mechanism figure so that
people who don't know that can get an idea.}
\todotextjira{296}{@Andrea: What are the implementations?}
\begin{figure}[h]
    \centering
