...
\begin{figure}[h]
    ...
    \todographics{There is an extra parenthesis}
    \caption{}
...
\todostructure{Yet another different model of a motor.
Either harmonize all in one, or explicitly have discussion about how the same thing can have different models}
\begin{example}
...
\begin{example}
    A gearbox (\cref{fig:gearbox}) provides a certain \textF{output torque~$\tau_o$} and \textF{speed~$\tau_o$}, given a certain
    \R{input torque $\tau_i$} and \R{speed $\omega_i$}.
...
\begin{example}
    A \emph{crank-rocker} (\cref{fig:crack}) converts \R{rotational motion} into a \F{rocking motion}.
\end{example}
\todographics{296}{Put here an additional crank-rocker mechanism figure so that people who don't know that can get an idea.}
\todotextjira{296}{@Andrea: What are the implementations?}
\begin{figure}[h]
    \centering
...
\begin{example}[Wireless link]
    ...
    Given the established convention about the the preference relations for functionality, in which a \emph{lower} functionality is ``easier'' to achieve, one needs to choose ``\F{\emph{minus} the latency}" and ``\F{\emph{minus} the packet drop probability}" for them to count as functionality.
...
\todographicsjira{154}{redo right-hand side in tikz}
...
\showslides{
    ...
    \begin{forslides}
        ...
        \begin{equation*}
            ...
            J=\lim_{T\to \infty} \frac{1}{T}\mathbb{E}\left\{\int_{0}^{T} \left(\left( \mat{x}_t^\intercal \mat{Q} \mat{x}_t\right)+\left( \mat{u}_t^\intercal \mat{R}\mat{u}_t\right)\right) \text{d} t\right\},
        \end{equation*}
        
        
        \begin{lemma}
            \label{lem:lqgstandard}
            The optimal control law for the LQG problem is~$\mat{u}_t^\star =-\mat{K}\hat{\mat{x}}_t=-\mat{R}^{-1}\mat{B}^*\bar{\mat{S}}\hat{\mat{x}}_t$, where $\hat{\mat{x}}_t$ is the unbiased minimum-variance estimate of $\mat{x}_t$ given previous measurements and $\bar{\mat{S}}\in \posdef$ solves the Riccati equation
            \begin{equation}
                \label{eq:cont_ric_1}
                \mat{SA}+\mat{A}^*\mat{S}-\mat{SBR}^{-1}\mat{B}^*\mat{S}+\mat{Q}=\mat{0}.
            \end{equation}
            The minimum cost~$J^\star$ achieved by the optimal control is:
            %\footnote{Note that~\cite{davis1977} contains a typo at p.188 (one extra $\mat{V}^{-1}$ factor).
            Instead,~\cite{kwakernaak1972} has a cleaner derivation and exposition.}
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
where~$\bar{\mat{\Sigma}}\in \posdef$ is the solution of the Riccati equation
\begin{equation}
    \label{eq:cont_ric_2}
...
\begin{example}
Nardi\,\,\etal~\cite{zia16comparative} describe a benchmarking system for visual SLAM that provides the empirical characterization of the monotone relation between \F{the accuracy} of the visual
SLAM solution, the \F{throughput {[}frames/s{]}} and \R{the energy for computation {[}J/frame{]}}.
...
studies the relation between the \F{performance of a visual task}, and the \R{minimal representation} that is needed to perform that task.
