...
\label{eq:cont_dyn}
    \begin{aligned}
    \text{d} \bm{x}_t&=\mat{A}\bm{x}_t\text{d} t+\mat{B}\bm{u}_t \text{d} t+\mat{E} \text{d} \bm{w}_t\\
    \text{d} \bm{y}_t &= \mat{C}\bm{x}_t \text{d} t+\mat{G}\text{d} \bm{v}_t,
    \end{aligned}
\end{equation*}

\begin{equation*}
\label{eq:general_continuous_cost}
    J=\lim_{T\to \infty} \frac{1}{T}\mathbb{E}\{\int_{0}^{T} \left(\left( \bm{x}_t^\intercal \mat{Q} \bm{x}_t\right)+\left( \bm{u}_t^\intercal \mat{R}\bm{u}_t\right)\right) \text{d} t\},
\end{equation*}


\begin{lemma}
\label{lem:lqgstandard}
The optimal control law for the LQG problem is~$\bm{u}_t^\star =-\mat{K}\hat{\bm{x}}_t=-\mat{R}^{-1}\mat{B}^*\bar{\mat{S}}\hat{\bm{x}}_t$, where $\hat{\bm{x}}_t$ is the unbiased minimum-variance estimate of $\bm{x}_t$ given previous measurements and $\bar{\mat{S}}\in \posdef$ solves the Riccati equation
\begin{equation}
\label{eq:cont_ric_1}
