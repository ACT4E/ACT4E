...
\label{eq:general_continuous_cost}
    J=\lim_{T\to \infty} \frac{1}{T}\mathbb{E}\{\int_{0}^{T} \left(\left( \mat{x}_t^\intercal \mat{Q} \mat{x}_t\right)+\left( \mat{u}_t^\intercal \mat{R}\mat{u}_t\right)\right) \text{d} t\},
\end{equation*}
...
Let~$\mat{Q}(\alpha)=\alpha \mat{Q}_0$ and~$\mat{R}(\alpha)=\frac{1}{\alpha}\mat{R}_0$, $\alpha\in \mathbb{R}_+$. Let~$\bm{u}^\star(\alpha)$ be the solution of the LQG problem with $\mat{Q}(\alpha)$ and $\mat{R}(\alpha)$. Then, under optimal control one has:
\begin{compactitem}
