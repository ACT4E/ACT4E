...
\begin{definition}[Co-design problem with implementation]
    ...
    Each node~$\adp\in\cdpiN$ is a DPI~$\adp=\tup{\funsp_{\adp},\ressp_{\adp},\impsp_{\adp},\prov_{\adp},\req_{\adp}}.$
    An edge~$\cdpie\in\cdpiE$ is a tuple $\cdpie=\tup{ \tup{ \adp_1,\cdpiresindA} ,\tup{\adp_2,\cdpifunindB}}$, where~$\adp_1,\adp_2\in\cdpiN$ are two nodes and~$\cdpiresindA$ and~$\cdpifunindB$ are the indices of the components of the functionality and resources to be connected, and it holds that~$\pi_{\cdpiresindA}\ressp_{\adp_1}=\pi_{\cdpifunindB}\funsp_{\adp_2}$~(\cref{fig:mcdps}).
    
    \begin{figure}[h]
        \centering
        \includesag{gmcdptro_cdpi}
        \todographics{@Gioele: wrong letters in figure}
        \caption{}
        \label{fig:mcdps}
    \end{figure}
...
\begin{figure}[h]
    ...
    \includesag{gmcdptro_cdpi}
    \todographicsjira{435}{@Gioele: wrong letters in figure}
    \caption{}
    \label{fig:mcdps}
\end{figure}

A CDPI is equivalent to a DPI with an implementation space~\impsp that is a subset of the product $\prod_{\adp\in\cdpiN}\impsp_{\adp}$, and contains only the tuples that satisfy the co-design constraints.
...
\begin{example}
    ...
    The formalism allows to consider \R{mass} and \R{cost} as independent resources, meaning that we wish to obtain the Pareto frontier for the minimal resources.
...
\todojira{158}{discussion about \emph{decomposition is not decoupling}}
