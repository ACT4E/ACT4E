...
While in previous examples we covered the problem ``feasibility'', we still miss \FixFunMinReq, \FixResMinFun, and \FeasibleImp.

The first can be translated to ``Given a lower bound on the functionality~\fun, what are the implementations that have minimal resources usage?''~(\cref{fig:setup-1}).
...
\begin{remark}[Minimal \emph{vs} least solutions]
    ...
    multi-objective optimization problems. The algorithm to be developed will directly
