...
\todotext{@J: Seems like that here string diagrams would help.}
...
In the following,  $\styleobj{V} $ denotes a \emph{finite-dimentsional} real vector space.
...
Let $\{e_1, ...., e_n \}$ be a basis of $\styleobj{V}$, and let $\{e_1^*, \dots, e_n^* \}$ be the corresponding dual basis of $\styleobj{V}^*$.
Then
\begin{equation}
...
Under this identification, $\coev_\styleobj{V}$ maps the scalar $\lambda$ to the linear endomorphism of $\styleobj{V}$ which is ``multiplication by $\lambda$''. (In terms of matrices, this is a diagonal matrix, with $\lambda$ at every entry of the diagonal.)
...
\todographics{@Gioele: add color for $\ev$, $\coev$.}
\todotext{@Gioele: use colored $\then$ below.}
\begin{ctdefinition}[Dualizable object]
    \label{def:dualizable-object}
    Let $\tup{\CatC,\mtimesC,\idmoncat_{\CatC}}$ be a monoidal category, and let $\Obja \in \ObC$.
