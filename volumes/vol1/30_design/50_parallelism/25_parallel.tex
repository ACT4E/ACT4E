...
\begin{marginfigure}
    ...
    \subfloat[\label{fig:effects1}
        A morphism in \Effects]{
        \includegraphics[width=4.5cm]{effects1}
    }\\
    \subfloat[\label{fig:effects2} Its representation in \SetStar]{
...
\begin{marginfigure}
    ...
    \subfloat[\label{fig:effects5} Its representation in \SetStar]{
...
\begin{ctdefinition}[Functorial stacking semi-category]
    ...
    A functorial stacking semi-category is a stacking semi-category where the two stacking operation $\mtimescatob$ and $\mtimescatmor$ are the two components of a functor $\mtimescat$.
\end{ctdefinition}

More in detail, we have for each

In particular we obtan that
\begin{equation}
    (\mora \mtimescat \morb )
