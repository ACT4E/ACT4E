...
\begin{example}
    Someone proposes now to substitute the monoidal unit in \cref{ex:monoidal_pos_reals} with 1 and the monoidal product with
    multiplication ``$\cdot$''. 
...
\begin{remark}[We cannot define monoidal \emph{semi}-categories]
    Note that in \cref{def:monoidal_cat} we used the category's identities in the diagrams. It is not possible to ``patch'' the definition to work with semi-categories.
\end{remark}
...
\begin{example}
    ...
    \todographicsjira{355}{@Gioele: The outermost arrows in \cref{fig:set_mon_triangle} have wrong final points.}
...
\end{gradedexercise}
\todojira{187}{Style of solutions is wrong, adjust}
