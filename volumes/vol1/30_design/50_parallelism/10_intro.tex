...
In this chapter, we will consider composition both in series and \emph{in parallel}. For example, given functions $\mora: \setA \mto \setB$ and $\morb: \setB \mto \setC$, because the target set of the function $\mora$ matches the source set of $\morb$ they may be composed in series to obtain a function $\mora \then \morb: \setA \mto \setC$. On the other hand, any two functions $\mora_1 : \setA \mto \setB$ and $\mora_2 : \setC \mto \setD$ may be composed ``in parallel'' by taking their cartesian product: we obtain the function $\mora_1 \times \mora_2: \setA \times \setB \mto \setC \times \setD$. This parallel composition of $\mora_1$ and $\morb_2$ does not rely on any match-up of target and source sets, but it does rely on the ``additional structure'' provided by the cartesian product. Such ``additional structure'' will be formalized in this chapter using the notion of a \emph{monoidal structure}.
...
\todographicsjira{185}{@Gioele: figure for parallelism 1/2}
