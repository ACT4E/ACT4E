...
For example, given functions $\mora: \setA \mto \setB$ and $\morb: \setB \mto \setC$, because the target set of the function $\mora$ matches the source set of $\morb$ they may be composed in series to obtain a function $\mora \then \morb: \setA \mto \setC$.
On the other hand, any two functions $\mora_1 : \setA \mto \setB$ and $\mora_2 : \setC \mto \setD$ may be composed ``in parallel'' by taking their cartesian product: we obtain the function $\mora_1 \times \mora_2: \setA \times \setB \mto \setC \times \setD$.
...
In this chapter, we will also use the basic pattern of a monoid as inspiration, but now for parallel composition! Thus parallel composition is also "monoid-like", and hence the name \emph{monoidal structure}.
