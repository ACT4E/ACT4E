...
\begin{definition}[Diagonal function]
    ...
    Define the \emph{\iindex{diagonal function}}~$\diag_\posA\colon \posA \to {\posA \Ctimes \posA}$:
    \begin{equation}
        \begin{aligned}
...
\begin{definition}[Codiagonal function]
    ...
    Define the \emph{\iindex{codiagonal function}}~$\codiag_\posA\colon \posA \Cplus \posA \to \posA $:
    \begin{equation}
        \begin{aligned}
...
\devel{
    ...
    \begin{lemma}
        Given~$\adpa, \adpb\colon \funposA \profto \resposB$, we have:
        \begin{equation}
            \adpa \meet \adpb = \comp{\codiag}_\posgenA \dpthen(\adpa + \adpb) \dpthen \conj{\codiag}_\posgenB.
        \end{equation}
    \end{lemma}
    \missingproof
    \devel{
        \begin{proof}
            ...
            \begin{equation}
                ...
                \end{aligned}
            \end{equation}
            We start by looking at~$\comp{\codiag}_\posgenA \dpthen (\adpa+\adpb) \colon \funposA\profto \resposB+\resposB$:
            \begin{equation}
                \begin{aligned}
                    \left(\comp{\codiag}_\posgenA\dpthen (\adpa+\adpb)\right)\left(\tup{\funposAel^*,\resposBel}\right) & =\bigvee_{} \XXX
                \end{aligned}
            \end{equation}
            \todotextjira{418}{@Gioele: Adjust signatures, have to find a good way to write it down}
        \end{proof}
