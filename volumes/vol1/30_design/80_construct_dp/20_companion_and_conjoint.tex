...
\begin{definition}[Companion and conjoint]
    \label{def:comp_conj}
    Let~\posA and~\posB be \SY{posets}, and suppose that~$\mapa\colon\posA \toinPos \posB$ is a monotone map.
...
\begin{proof}
    ...
    \end{equation}
    First, we see that they send the identity \SY{monotone maps}~$\mapidat{\posA}$ to the unit~$\dpidat{\posA}$ for any poset~\posA, because
    \begin{equation}
        \begin{aligned}
...
\begin{example}
    The identity \SY{design problem}~$\catidat\posA\colon \F{\posgenA} \profto \R{\posgenA}$ is the \SY{companion} (and the conjoint) of the identity map~$\catidat\posA'\colon \posgenA \toinPos \posgenA$.
