...
\begin{definition}[Category of design problems \DP]
    ...
    \begin{enumerate}
        ...
        \item \emph{Identity morphism}: The \SY{identity morphism}~$\funid_\posgenA \colon \funposA \profto \resposA$ is given by \cref{def:dp-identity}.
...
\begin{proof}
    ...
    We need to show
    \begin{equation}
        \dpitodpmor(\adpa \fthenof\DPI \adpb)=\dpitodpmor(\adpa)\fthenof\DP \dpitodpmor(\adpb).
    \end{equation}
    ...
    \begin{equation}
        \label{eq:dpitodp_a}
        \begin{aligned}
            \dpitodpmor(\adpa \fthenof\DPI \adpb)(\funposAel\F{\opel},\resposCel) & =
    ...
    \begin{equation}
        \label{eq:dpitodp_b}
        \begin{aligned}
             & (\dpitodpmor(\adpa) \fthenof\DP \dpitodpmor(\adpb))(\funposAel\F{\opel},\resposCel) \\
    ...
    \begin{itemize}
        \item If~$\dpitodpmor(\adpa \fthenof\DPI \adpb)(\funposAel\F{\opel},\resposCel)=\true$, there exist~$\impn{1}\setin \impspn{1},\impn{2}\setin \impspn{2}$ for which
              \begin{equation}
                  \begin{aligned}
              ...
              \end{equation}
              implying~$(\dpitodpmor(\adpa) \fthenof\DP \dpitodpmor(\adpb))(\funposAel\F{\opel},\resposCel)=\true$.
        \item The case
              \begin{equation}
                  \prftree{\dpitodpmor(\adpa \fthenof\DPI \adpb)(\funposAel\F{\opel},\resposCel)=\false}{(\dpitodpmor(\adpa) \fthenof\DP \dpitodpmor(\adpb))(\funposAel\F{\opel},\resposCel)=\false}
              \end{equation}
              follows analogously.
        \item The other direction is easier to show, since clearly
              \begin{equation}
                  \prftree{(\dpitodpmor(\adpa) \fthenof\DP \dpitodpmor(\adpb))(\funposAel\F{\opel},\resposCel)=\true}{\dpitodpmor(\adpa \fthenof\DPI \adpb)(\funposAel\F{\opel},\resposCel)=\true}
              \end{equation}
              and
              \begin{equation}
                  \prftree{(\dpitodpmor(\adpa) \fthenof\DP \dpitodpmor(\adpb))(\funposAel\F{\opel},\resposCel)=\false}{\dpitodpmor(\adpa \fthenof\DPI \adpb)(\funposAel\F{\opel},\resposCel)=\false}
              \end{equation}
...
\begin{definition}
    ...
    \begin{enumerate}
        \item Identity on the objects:~$\dpitodpob(\posA)=\posA$.
        \item Given~$\adp\colon \funsp\op \Ptimes \ressp \toinPos \Bool$, the action on morphisms is given by
              \begin{equation}
...
\begin{proof}
    ...
    We need to show
    \begin{equation}
        \dptodpimor(\adpa \fthenof\DP \adpb)=\dptodpimor(\adpa)\fthenof\DPI \dptodpimor(\adpb).
    \end{equation}
    Let's start with the left term.
    One has
    \begin{equation}
        \dptodpimor(\adpa \fthenof\DP \adpb)=\tup{\funposA,\resposC,\impsp,\prov,\req},
    \end{equation}
    where
    \begin{equation}
        \begin{aligned}
            \impsp & =\makeset{\tup{\funposAel,\resposCel}\setin \funposA \cartprod \resposC \colon (\adpa \fthenof\DP \adpb)(\funposAel,\resposCel)}, \\
    ...
    \begin{equation}
        \dptodpimor(\adpa) \fthenof\DPI \dptodpimor(\adpb)=\tup{\funposA,\resposC,\impsp\colI{'},\prov\F{'},\req\R{'}},
    \end{equation}
    where
    \begin{equation}
        \begin{aligned}
