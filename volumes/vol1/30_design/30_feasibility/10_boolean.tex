...
\todotext{here $K$ does not have a macro}
...
Note that the feasibility set $K_\adp$ of a design problem $\adp \colon \funsp \op \times \ressp\toinPos \Bool$ is a binary relation $K_\adp \subseteq \funsp\op \times \ressp$. We saw in \cref{re:rel-three-fun-descriptions} that there is a one-to-one correspondence between functions $\mapa : \setA \times \setB \to \Bool$ and binary relations $\relA \subseteq \setA \times \setB$. An analogous correspondence holds in the context of design problems:

\begin{exercise}\label{ex:adp-uppersets}
...
\end{exercise}
\begin{solution}
    \todotext{Write solution to exercise \cref{ex:adp-uppersets}}
\end{solution}
...
\begin{example}
    ...
    \begin{equation*}
        \adp \colon \F{\posgenA} \op \times  \R{\posgenB} \toinPos \Bool,
    \end{equation*}
