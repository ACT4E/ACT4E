...
\begin{remark}
    ...
    is empty or not.
\end{remark}
\todotext{@Gioele: this example is weak now, but it can be made much better:
    1) In the table, use the colors for functionality, implementation, resources.
    2) Order them like this: functionality on the left, implementation, resources in the middle.
    3) Show the resulting DP as another table, where we have forgotten about the implementation.
    This shows the difference between a DP and a DPI.
}
\begin{example}
...
\begin{definition}[Feasible set of a design problem]
    ...
    \begin{equation}
        ...
        \feasibleset{\adp} = \makeset{ \tup{\F{\fun^*},\res} \setin \funsp \op\cartprod \ressp  \mid %
            \adp(\fun\Fop, \res) = \true %
        }.
    \end{equation}
\end{definition}

Note that the feasibility set~$\feasibleset{\adp}$ of a design problem~$\adp \colon \funsp \op \cartprod \ressp\toinPos \Bool$ is a binary relation~$\feasibleset{\adp} \setsubseteq \funsp\op \cartprod \ressp$.
We saw in \cref{rem:rel-three-fun-descriptions} that there is a one-to-one correspondence between functions~$\mapb \colon \setA \cartprod \setB \to \Bool$ and binary relations~$\relA \setA \mto \setB$.

An analogous correspondence holds in the context of design problems:

\begin{exercise}
    \label{ex:adp-uppersets}
    Prove that there is a one-to-one correspondence between design problems~$\adp \colon \funsp \op \cartprod \ressp\toinPos \Bool$ and upper sets~$\feasibleset{\adp} \setsubseteq \funsp\op \cartprod \ressp$.
\end{exercise}
\begin{solution}
    \missingsolution
    \todotextjira{408}{@JL: Write solution to exercise \cref{ex:adp-uppersets}}
\end{solution}

Recall that when working with a relation~$\relA \setsubseteq \setA \cartprod \setB$ between sets, if the sets in question were finite, then we could conveniently draw the relation~$\relA$ using arrows to connect elements of~$\setA$ to those elements of~$\setB$ to which they are related via~$\relA$.
...
\begin{marginfigure}
    ...
    \todographics{@Gioele: label posets , relation}
\end{marginfigure}
...
\begin{example}
    ...
    \end{equation}
    \todographics{The dashed arrows do not describe a set but a relation.
        Add another figure showing the set of tuples?
    }
\end{example}
...
\begin{solution}
    \missingsolution
    \todotextjira{412}{@JL: Write solution to exercise \cref{ex:adp-monotone}}
\end{solution}
...
\toexclude{
    ...
    The theory of design problems can be easily generalized to pre-orders.
