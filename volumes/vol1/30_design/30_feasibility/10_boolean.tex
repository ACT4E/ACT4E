...
\begin{remark}
    ...
    \begin{equation}
        ...
        \defmapperiod{\adp}{\funsp\op \cartprod \ressp}{\toinPos}{\Bool}{\tup{\fun\Fop,\res}}{\exists \imp \setin \impsp \colon (\fun \posleqof\funsp \prov(\imp)) \booland (\req(\imp)\posleqof\ressp \res)}
    ...
    \begin{equation}
        ...
        \{ \imp \setin \impsp \colon (\fun \posleqof\funsp \prov(\imp)) \booland (\req(\imp)\posleqof\ressp \res)\}
    \end{equation}
    is empty or not.
\end{remark}
...
\begin{definition}[Feasible set of a design problem]
    ...
    \begin{equation}
        ...
        \feasibleset{\adp} = \{ \tup{\F{\fun^*},\res} \setin \funsp \op\cartprod \ressp  \mid %
        \adp(\fun\Fop, \res) = \true %
        \}.
    \end{equation}
\end{definition}
...
We saw in \cref{rem:rel-three-fun-descriptions} that there is a one-to-one correspondence between functions~$\mapb \colon \setA \cartprod \setB \to \Bool$ and binary relations~$\relA \setsubseteq \setA \cartprod \setB$.
...
\begin{solution}
    \missingsolution
    \todotextjira{408}{@J: Write solution to exercise \cref{ex:adp-uppersets}}
\end{solution}
...
\begin{example}
    ...
    \begin{equation}
        \posAset = \{ \setAeln{1}, \setAeln{2}, \setAeln{3} \}
        \qqand
        \posBset =  \{ \setBeln{1}, \setBeln{2}\},
    \end{equation}
    and ordered as shown in the figure.

    The feasibility set is marked with the dashed arrows; it is the set
    \begin{equation*}
        \feasibleset{\adp} = \{\tup{\setAeln{1}, \setBeln{1}}, \tup{\setAeln{1}, \setBeln{2}}, \tup{\setAeln{2}, \setBeln{2}}, \tup{\setAeln{3}, \setBeln{2}} \}.
    \end{equation*}
\end{example}
...
\begin{solution}
    \missingsolution
    \todotextjira{412}{@J: Write solution to exercise \cref{ex:adp-monotone}}
\end{solution}
