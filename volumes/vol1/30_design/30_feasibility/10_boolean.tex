...
\begin{exercise}
    ...
    Prove that there is a one-to-one correspondence between \SY{design problems}~$\adp \colon \funsp \op \Ctimes \ressp\toinPos \Bool$ and \SY{upper sets}~$\feasibleset{\adp} \setsubseteq \funsp\op \Ctimes \ressp$.
\end{exercise}
...
\toexclude{
    ...
    \end{enumerate}
    This means that, given a \SY{pre-order}, we can consider the \SY{poset} of its isomorphism classes, by means of the following equivalence relation:
    \begin{equation}
