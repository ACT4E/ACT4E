...
\begin{remark}
In cases where the posets involved are (small) finite posets, design problems can be visualized nicely using directed graphs,  and then composition of design problems can be calculated conveniently by following certain paths in the ``concatenation'' of such graphs. For examples and explanations of this, see Chapter 4 in the book ``An invitation to applied category theory'' by Fong and Spivak. 
\end{remark}

\todotext{Implement material explaining and illustrating the above remark.}
