...
\begin{equation}
    \adpa \colon \F{\posgenA} \op \Ptimes  \R{\posgenB} \toinPos \Bool,
\end{equation}
...
\begin{equation}
    \adpb \colon \F{\posgenB} \op \Ptimes  \R{\posgenC} \toinPos \Bool,
\end{equation}
...
Namely, a pair~$\tup{\setAel, \setCel}$ is in the feasibility set of~$\adp \fthen \adpb$ if and only if we can trace a path from~$\setAel$ to~$\setCel$ by only moving upwards in the \SY{posets}~\posA,~\posB, and~\posC, or crossing from one \SY{poset} to another following dashed arrows in the direction they are pointing.
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \begin{aligned}
            \pars{\adpab \fthen \adpc} (\funposAopel,\resposDel)
