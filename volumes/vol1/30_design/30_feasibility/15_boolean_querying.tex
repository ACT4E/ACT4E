...
\end{equation}
%
There are analogous transformations for a \SY{design problem}~$\adp\colon \funsp\op \Ptimes \ressp\toinPos \Bool$.
...
Which \SY{poset} structure can we choose on~$\setOfUppersets (\ressp)$ and~$\setOfUppersets (\funsp\op)$, respectively, so that~$\widehat{\adp}$ and~$\widecheck{\adp}$ are monotone?

\begin{gradedexercise}[\exname{CurryingDesignProblems}]
    \label{ex:CurryingDesignProblems}
    Let~$\adp\colon \funsp\op \Ptimes \ressp\toinPos \Bool$ be a design problem.
    ...
    \begin{enumerate}
        \item Show that~$\widehat{\adp} \colon \funsp\op \mto \setOfUppersets (\ressp)$ and~$\widecheck{\adp} \colon \ressp \mto \setOfUppersets (\funsp\op)$ are \SY{monotone maps} when we consider~$\setOfUppersets (\ressp)$ and~$\setOfUppersets (\funsp\op)$ to have the partial order corresponding to the inclusion of subsets.
        \item Show that the \SY{poset} $\tupp{\setOfUppersets\funsp\op, \setsubseteq}$ and the \SY{poset} $\tupp{\setOfLowersets\funsp, \setsubseteq}$ are isomorphic.
