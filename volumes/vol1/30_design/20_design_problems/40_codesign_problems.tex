...
\begin{example}[Non-convexity]
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}
    The feasible set~$\feasibleset{}\setsubseteq\funsp\Ctimes \ressp$ is the set of functionality and resources that satisfy the constraints~$\res\setin\ftor(\fun)$ and~$\res\posleq\fun$:
    %
    \begin{equation}
        \feasibleset{}=\makesett{ \tup{\fun,\res}\setin\funsp\Ctimes \ressp\colon (\res\setin\ftor(\fun))\wedge(\res\posleq\fun)} .
        \label{eq:feasible}
    \end{equation}
    %
