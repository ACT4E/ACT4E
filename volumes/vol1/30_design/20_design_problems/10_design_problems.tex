...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            \label{eq:imp_comp}
            \impsp \setsubseteq \makeTupcat{ \impsp_{1}, \impsp_{2}}
        \end{equation}
        \begin{equation}
            \label{eq:dp_mob_4}
...
\todotextjira{295}{Integrate \cref{exa:dpi_elmotor} with \cref{exa:motor}}
...
\begin{figure}[h]
    ...
    \caption{}
    \label{fig:gearbox}
    \todographics{@Gioele: can we put the reduction ratio and efficieny ratio as implementations in the drawing? }
\end{figure}
...
\end{example}
\todotext{@Gioele: Can you write down a model for propeller thrust? Double check that it is monotone.}
\todotextjira{296}{@Andrea: What are the implementations?}
\begin{figure}[h]
    \centering
...
\end{example}
\todographicsjira{296}{@Gioele: Put here an additional crank-rocker mechanism figure so that people who don't know that can get an idea.
...
\begin{figure}[tbh]
    ...
    \caption{}
    \label{fig:packing}
    \todographics{@Gioele: can you put `content shapes' in green, and `container shape' in red?}
\end{figure}
