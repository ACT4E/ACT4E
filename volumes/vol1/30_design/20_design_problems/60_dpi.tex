...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            \label{eq:adp12pf}
            (\adpa\dpthen\adpb): \FB{A}\op \cartprod  \RB{C} \to_{\Pos} \Bool
        \end{equation}
        %
        \begin{equation}
            \label{eq:adp12us}
            (\adpa\dpthen\adpb): \FB{A}  \to\uppersets \RB{C}
        \end{equation}
        %
        \begin{equation}
            \label{eq:adpM12us}
            (\adpa\dpthen\adpb):(\FB{A} \cartprod \FB{C})  \to\uppersets (\RB{C} \cartprod \RB{D})
        \end{equation}
        %
        \begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \label{eq:assoc_dpi}
        (\adpan{1}\fthen \adpan{2})\fthen \adpan{3}=\adpan{1}\fthen (\adpan{2}\fthen \adpan{3}).
    \end{equation}
    We know that the first part of the left term of \cref{eq:assoc_dpi} gives
    \begin{equation}
        (\adpan{1} \fthen \adpan{2})
        =
    ...
    \begin{equation}
        \label{eq:dpi_ass_1}
        (\adpan{1} \fthen \adpan{2})\fthen \adpan{3}=\tupp{ \funspa,\resspd,\impspn{1,3},\provn{1,3},\reqn{1,3}},
    \end{equation}
    where
    \begin{align}
...
The sum of two design problems with implementation is a design problem with the implementation space~$\impsp=\impsp_{1}\setdisunion\impsp_{2}$, and it represents the exclusive choice between two possible alternative families of designs.
