...
\linkvideo{spring2021-design:design:abstract-view-dp} % An abstract view of design problems
...
Nevertheless, functionality and resources can express most of
...
The implementation space~\impsp is the set over which we are optimizing; an implementation~$\imp\in\impsp$ is a particular point in that set~(\cref{fig:impspace}).
...
is as follows.
We will assume that, given one implementation, we can evaluate it
...
We would say that the property
    of being an electronic circuit is compositional.
\end{example}

\todojira{150}{Would be nice to also give an example of something that is *not* compositional}

\todojira{151}{Add watch example?}
