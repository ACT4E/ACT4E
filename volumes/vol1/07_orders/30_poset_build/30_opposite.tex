...
\begin{definition}[Opposite of a poset]
    ...
    The \maindef{opposite of a poset}~$\posA=\tupp{\posAset, \posAleq}$ is the \SY{poset} denoted~$\posA\op=\tupp{\posAset, \posAleqop}$.
    ...
    \end{equation}
\end{definition}
For a given~$\posela \setin \posAset$, we will sometimes write~$\posela^*$ do denote its corresponding copy in~$\posA\op$, in order to emphasize that~$\posela$ and~$\posela^*$ belong to distinct \SY{posets}.
