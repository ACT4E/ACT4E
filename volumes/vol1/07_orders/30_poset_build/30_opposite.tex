...
\begin{definition}[Opposite of a poset]
    ...
    \end{equation}
\end{definition}
For a given~$\posela \setin \posAset$, we will sometimes write~$\posela^*$ do denote its corresponding copy in~$\posA\op$, in order to emphasize that~$\posela$ and~$\posela^*$ belong to distinct posets.
...
\begin{example}[Credit and debt]
    ...
    From this set we can define two posets,~$\posA^{+} = \tup{\posAset, \Rleq}$ and~$\posA^{-} = \tup{\posAset, \Rgeq}$, that are the opposite of each other.
