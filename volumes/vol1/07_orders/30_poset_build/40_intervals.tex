...
\todojira{458}{POST-ALPHUBEL:Expand.
...
Suppose that we want to track the value of a quantity~$x\setin [0,100]$, without having \emph{a priori} information about~$x$.
We are equipped with sensors, which periodically measure the quantity~$x$ with some variable precision.
At time~$t\setin \nonNegReals $ they produce an \emph{observation}~$y_t\colon x_t\setin [l_t,u_t]$.
Also, note that the quantity fluctuates randomly, and we bound its ``velocity'' to be~$\dot{x}_t\setin [-1,1]$ (except at boundaries).
At the beginning, our information state~$\bar{i}_0$ could be that~$x\setin [0,100]$.
At time 0, we get an observation~$y_0$, that says~$x\setin [21,24]$.
...
\begin{equation*}
    x\setin \left( \interv{20}{25} \setintersection \interv{21}{24}\right)\equiv x\setin \interv{21}{24}.
\end{equation*}
