...
\begin{example}[Rounding functions]
    ...
    \begin{equation*}
        \defmapperiod{\rtntte}{\tup{\reals,\leq}}{\to}{\tup{\natnumbers,\leq}}{x}{\begin{cases}
                \funfloor(x), & x< (\funfloor(x)+\funceil(x))/2                                              \\
                \funceil(x),  & x> (\funfloor(x)+\funceil(x))/2                                              \\
                \funceil(x),  & (x= (\funfloor(x)+\funceil(x))/2 )\booland (\funceil(x) \text{ mod } 2 = 0)  \\
                \funfloor(x), & (x= (\funfloor(x)+\funceil(x))/2 )\booland (\funfloor(x) \text{ mod } 2 = 0)
            \end{cases}}
    \end{equation*}
