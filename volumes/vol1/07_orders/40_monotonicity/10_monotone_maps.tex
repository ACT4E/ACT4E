...
\begin{definition}[Monotone map]
    \label{def:monotone}
    A \emph{\iindex{monotone map}} between two \SY{posets}~$\posA=\tupp{\posAset, \posAleq}$ and~$\posB=\tupp{\posBset, \posBleq}$ is a map~$\mapa \colon \posAset \sto \posBset$ that preserves the ordering, in the sense that
...
\begin{example}[Cardinality map]\label{exa:cardinality}
    ...
    \end{equation}
    is monotone when considered as a map from the \SY{poset} $\powerposet\setA$ to the \SY{poset} $\tup{\natnumbers,\Nleq}$.
    % \begin{equation}
    %     \defmap{
    %         \cardmap
    %     }{
