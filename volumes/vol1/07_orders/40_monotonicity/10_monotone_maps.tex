...
\begin{definition}[Monotone map]
    \label{def:monotone}
    A \maindef{monotone map} between two \SY{posets}~$\posA=\tupp{\posAset, \posAleq}$ and~$\posB=\tupp{\posBset, \posBleq}$ is a map~$\mapa \colon \posAset \sto \posBset$ that preserves the ordering, in the sense that
