...
\begin{example}[Rounding functions]
    ...
    \end{equation}
    The functions $\funceil$ and $\funfloor$ are monotone, since~$\ela\leq \elc$ implies both~$\funceil(\ela)\leq \funceil(\elc)$ and~$\funfloor(\ela)\leq\funfloor(\elc)$.

    There are also other rounding functions that are commonly used by computers.
