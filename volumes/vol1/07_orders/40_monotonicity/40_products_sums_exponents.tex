...
\section{Products, sums, exponents}
...
\todotext{Make this its own section; is a different type of thing than products and sums of monotone functions and it forshadows natural transformations}

Given \SY{posets} \posA and \posB, the set of \SY{monotone maps} $\posA \mto \posB$ may be turned into a \SY{poset} itself: we can order the monotone functions ``point-wise''.
...
We will use the ``exponent'' notation $\posB^\posA$ for the poset obtained by ordering monotone functions from $\posA$ to $\posB$ in this way. 
