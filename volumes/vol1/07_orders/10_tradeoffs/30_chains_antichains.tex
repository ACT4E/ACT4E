...
\begin{example}[Chains and antichains in a power set]
    ...
    \begin{equation}
        \makeset{\Emptyset,\makeset{\posela},\makeset{\posela,\poselb},\makeset{\posela,\poselb,\poselc}}
        \qqand
...
\begin{figure*}
    ...
    \todographics{@Gioele: 1) Don't make the chains being both maximal chains. Maybe on
    e is $\makeset{x}, \makeset{xy}$. 2) Dont' make the antichains both be maximal. Have one be only two or one element. 
    Note: these changes will need change in the text as well.}
    \todographics{
    ...
    Also it doesn't mesh well with the fact that $\Emptyset$ is brown. Maybe we can circle them? }
\end{figure*}
...
\begin{example}
    ...
    \begin{marginfigure}
        ...
        \todographics{@Gioele: don't make either axis go to zero. }
    \end{marginfigure}
    %
