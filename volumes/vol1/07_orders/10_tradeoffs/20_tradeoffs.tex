...
\begin{quiz}[CommutativePosetDiagram]
Consider the semicategory of posets $\CatSymbol{PoSet}$ and the two objects $(\mathbb{N},\leq),(\{\mathrm{true},\mathrm{false}\},\implies)\in\Ob_{\CatSymbol{PoSet}}$. Then for $n,m\in\mathbb{N}$ such that $n\leq m$ define the morphisms of posets\footnote{i.e. a morphism of posets $\stylemorph{f}:(P_{1},\leq_{1})\to(P_{2},\leq_{2})$ such that if $x\leq_{1}y$ then $\stylemorph{f}(x)\leq_{2}\stylemorph{f}(y)$.} $\stylemorph{f}:(\{\mathrm{true},\mathrm{false}\},\implies)\to(\mathbb{N},\leq)$ and $\stylemorph{g}:(\mathbb{N},\leq)\to(\{\mathrm{true},\mathrm{false}\},\implies)$ by
$$\stylemorph{f}(\mathrm{false})=n$$
$$\stylemorph{f}(\mathrm{true})=m$$
and
$$\stylemorph{g}(k)=\begin{cases}
\mathrm{false} & , \text{ if } k\leq n \\
\mathrm{true} & , \text{otherwise}$$
\end{cases}.$$
Consider the diagram
$$\begin{tikzcd}
{(\{\mathrm{true},\mathrm{false}\},\implies)} \arrow[r,"\stylemorph{f}"] & {(\mathbb{N},\leq)} \arrow[r,"\stylemorph{g}"] & {(\{\mathrm{true},\mathrm{false}\},\implies)}
\end{tikzcd}$$
The the following is true ... (choose the coorect statements)
\begin{enumerate}[label=(\alph*)]
\item This diagram commutes.
\item This diagram does not commute.
\item $\stylemorph{f},\stylemorph{g}$ are not morphisms of posets.
\end{enumerate}
\end{quiz}
...
\begin{quiz}[\exname{CommutativePosetDiagramII}]
