...
\todotext{Explain how this is an economical way to visualize a relation, by showing an alternative}
...
\begin{figure}[h!]
    ...
    \todographics{@Gioele: CHF as standard currency?}
    \caption{The cost of a battery can be represented as a poset.}
    \label{fig:hassebattery}
\end{figure}

\todotext{Explain how quantitative is not numeric. Example of $\text{cheap}\leq\text{midrange}\leq\text{expensive}$}


\todotext{Bad example for Hasse diagram because there are no implicit arrows.}
\begin{example}
...
\begin{example}[Booleans]
    ...
    \end{equation*}
    \todotext{AC: Called ``Imp''? ``implication''?}
...
\begin{marginfigure}
    ...
    \caption{}
    \label{fig:posdef}
    \todographics{In the first picture the formula is partially obscured}
\end{marginfigure}
