...
A \SY{pre-order} is a set together with a relation that is both \SY{reflexive} (\cref{def:endo_reflexive_irreflexive}) and \SY{transitive}~(\cref{def:endo_transitive}).
...
\end{marginfigure}
By imposing \emph{\SY{totality}} (\cref{def:endo_total}), we obtain a total order.
