...
\todotext{Explain here the difference between the three by showing some graph representation of relations.}
\vfill
\clearpage
...
\begin{figure*}[h]
    ...
    \todographics{In the first panel, the label should $\posleqof\posA$, not $\posA$.}
\end{figure*}
...
\begin{example}[Booleans]
    ...
    \begin{marginfigure}
        ...
        \label{fig:boolean}
    \end{marginfigure}
    The booleans \index{\Bool} is a poset with carrier set~$\{\true,\false\}$ and the order relation given by~$\false \posleqof\Bool \true$ (\cref{fig:boolean}).
...
\begin{marginfigure}
    \centering
    \todographicsjira{559}{@Gioele: Inconsistent way to describe quadratic form between figure and text.}
    \subfloat[
...
\begin{example}
    \begin{definition}[Positive semi-definite matrix]
        A symmetric matrix~$\mat{M}\in \reals^{n\times n}$ is \emph{positive semi-definite} if~$\vec{x}^\intercal \mat{M}\vec{x}\geq 0$ for all non-zero~$\vec{x}\in \reals^n$.
    ...
    \begin{equation*}
        \vec{x}^\intercal \mat{A}\vec{x}=1,\quad \vec{x}\in \reals^n.
    \end{equation*}
...
\devel{
    Note~$b_1 \posleqof\Bool b_2$ iff~$b_1 \Imp b_2$, that is,
