...
\begin{ctdefinition}[Top and bottom]
    \label{def:top}
    \label{def:bot}
    If there is a least upper bound for the entire \SY{poset}~\posA, it is called the \emph{top} ($\postop$).
...
\begin{ctdefinition}[Minimal elements]
    \label{def:Min}
    $\Min \colon \posPA \to \posAA$ is the map that sends a subset~$\subA$ of a \SY{poset} to the minimal elements of that subset. % (those elements~$\setAel \setin \subA$ such that~$\setAel \posAleq \setBel$ for all~$\setBel \setin \subA$).
    In formulas:
    \begin{equation}
