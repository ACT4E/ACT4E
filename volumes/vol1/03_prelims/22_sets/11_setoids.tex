...
We write~$a \colon T$ to say that~$a$ has type~$T$.
For example, one could say~$42 \colon \aword{int}$ where~$\aword{int}$ is the type of integers.
...
In type theory the concepts of \emph{sets} and \emph{belonging} are not primitive and one should build them from scratch, enjoying much more flexibility.
...
\begin{ctdefinition}[Setoid]
    ...
    \begin{itemize}
        ...
        \item $\aword{equal}\colon T\times T\to\Bool$ is the equality;
...
\begin{itemize}
    \item \emph{Elements are opaque}: We never specify any interface for elements.
          ...
          However, this might be counter-intuitiv for a beginner, as it goes against the beginner's advice in object-oriented languages, where a \classname{Dog} is an \classname{Animal} that \funcname{barks}.
