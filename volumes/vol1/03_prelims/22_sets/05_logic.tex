...
To make mathematical proofs, we start with some assumptions (statements that we take, for the sake of our argument, to be true), and then we apply rules of reasoning, often called \emph{inference rules}, which allow us to deduce new statements from these.
...
A logical operation that only takes one argument is \emph{negation}: if~$P$ is a statement, its negation
\begin{equation}
    \lnot P
\end{equation}
...
Conventions about how strongly different logical operations ``bind'' allow us to use fewer parentheses.
...
\end{equation}
depending on whether one wishes to take~$\Imp$ as a primitive connective or to define it as a compound connective via \cref{eq:imp-formula}.
