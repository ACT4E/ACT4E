...
We describe them through the interfaces of \classname{Loader}, \classname{Saver}, and (union of the two) \classname{Serializer}. The two type parameters \TypeVar{E} and \TypeVar{R} correspond to the Python type that they serialize and to the resulting serialization format.
...
The type \classname{FiniteSet\_desc} is a type that describe the serialization format in \cref{fig:set1}. We don't show it, but you can click through and see the definition in the development environment.
...
There is already a skeleton implemented in the template in the file \files{act4e_solutions/sets_representation.py}, which contains the following:
...
First, you need to add your implementation of \classname{FiniteSet}. A possible skeleton is in \cref{lst:act4e_book_examples.empty}.
