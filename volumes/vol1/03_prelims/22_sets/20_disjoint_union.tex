...
Given sets~$\setA$ and~$\setB$, their disjoint union, denoted~$\setA \setdisunion \setB$ is the set
\begin{equation*}
...
\end{equation*}
In the special case where~$\setA = \emptyset$, then~$\emptyset \setdisunion \setB = \{ \tup{\elb,2} \mid \elb \in \setB \}$, and similarly~$\setA \setdisunion \emptyset =  \{ \tup{\ela,1} \mid \ela \in \setA \}$.
