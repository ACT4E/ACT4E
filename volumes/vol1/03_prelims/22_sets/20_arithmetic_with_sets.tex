...
\begin{align}\label{eq:arithmetic-equations}
    0 \cdot x &= 0,  \\ 
    1 \cdot x &= x, \\ 
    x \cdot y & = y \cdot x, \\ 
    x + y & = y + x, \\ 
...
\begin{align}\label{eq:arithmetic-with-sets}
...
\end{equation}
which holds for any set $\setA$ (and also $\setA \cartprod \Emptyset  = \Emptyset$\label{eq:emptyset-is-zero-again}). Note that this equation follows from \cref{def:cartesian-product}, or from the more precise formulation given in \cref{rem:formal-cartesian}.
...
In \cref{sec:special-sets} we defined the singleton set $\singleton = \makeset{\singletonel}$ in order to have -- for practical reasons of exposition and readability -- a ``standard, go-to, default one-element set''.  This set (and any other singleton set) behaves like the natural number ``$1$'' in the sense that
\begin{equation}\label{eq:singleton-set-is-neutral}
...
\begin{equation}\label{eq:leftunitor-sets}
 \defmapcomma{
           \leftunitor
        }{
...
\end{equation}
and
\begin{equation}\label{eq:rightunitor-sets}
 \defmapperiod{
           \rightunitor
        }{
...
This function is "canonical" because, once again, it's "shape" or "recipe" works for $\emph{any}$ two sets $\setA$ and $\setB$. To explicate this in more detail, consider, for example, the particular sets $\setA = \makeset{0, 1, 2}$ and $\setB = \makeset{\star, \dagger}$. Then, besides the canonical function $\braiding \colon \setA \cartprod \setB \to \setB \cartprod \setA$, there are also \emph{other} (non-canonical) isomorphisms possible, such as for instance the function $\setA \cartprod \setB \to \setB \cartprod \setA$ which maps like this: 
...
The name ``$\curry$'' stands for ``Curry'', which is the last name of the mathematician and logician Haskell Curry (\cref{fig:haskell-curry}) who did not discover this operation, but likely made it well-known. It is after his first name that the functional programming language Haskell is named. 

The operation given by the function \cref{eq:left-curry-function} is often called ``currying''. When needed for clarity, we call this operation \emph{right currying}, in order to distinguish it from the analogous operation of \emph{left currying} given by the function 
...
\end{equation}
We denote the functions \cref{eq:left-curry-function} and \cref{eq:left-curry-function} both by the same name ``$\curry$'' because we trust that it will be clear from context which of the two functions is being used in any given situation.
