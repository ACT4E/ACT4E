...
\begin{align}
    0 \cdot x &= 0, \\ \label{eq:arithmetic-zero}
    1 \cdot x &= x, \\ \label{eq:arithmetic-one}
    x \cdot y & = y \cdot x,  \\ \label{eq:arithmetic-mult-commutativity}
    x + y & = y + x, \\ \label{eq:arithmetic-add-commutativity}
    x \cdot (y \cdot z) & = (x \cdot y) \cdot z, \\ \label{eq:arithmetic-mult-associativity}
    x + (y + z) & = (x + y) + z, \\ \label{eq:arithmetic-add-associativity}
    (x \cdot y) + (x \cdot z)  & = x \cdot (y + z), \\ \label{eq:arithmetic-distributive}
    z^{x \cdot y}                    & = \pars{z^y}^x, \\ \label{eq:power-product}
    (x \cdot y)^z &= x^z \cdot y^z, \\ \label{eq:product-power}
    x^{(y + z)}           & = x^y \cdot x^z, \\ \label{eq:power-sum}
    (x + y)^z &= \sum_{k = 0}^z {{z}\choose{k}} \ x^k \cdot y^{z - k}. \label{eq:sum-power}
\end{align}
...
\begin{align}
    \Emptyset \cartprod \setA & \isomorphic \Emptyset,  \\ \label{eq:arithmetic-sets-zero}
    \singleton \cartprod \setA  & \isomorphic \setA,  \\ \label{eq:arithmetic-sets-one}
    \setA \cartprod \setB & \isomorphic \setB \cartprod \setA,  \\ \label{eq:arithmetic-sets-mult-commutativity}
    \setA \setdisunion \setB & \isomorphic \setA \setdisunion \setB,   \\ \label{eq:arithmetic-sets-add-commutativity}
    (\setA \cartprod \setB) \cartprod \setC & \isomorphic \setA \cartprod (\setB \cartprod \setC), \\ \label{eq:arithmetic-sets-mult-associativity}
    (\setA \setdisunion \setB) \setdisunion \setC & \isomorphic \setA \setdisunion (\setB \setdisunion \setC), \\ \label{eq:arithmetic-sets-add-associativity}
    (\setA \cartprod \setB) \setdisunion (\setA \cartprod \setC)  & \isomorphic \setA \cartprod (\setB \setdisunion \setC), \\ \label{eq:arithmetic-sets-distributive}
    \setC^{\setA \cartprod \setB}                    & \isomorphic \pars{\setC^\setB}^\setA, \\ \label{eq:sets-power-product}
    (\setA \cartprod \setB)^{\setC} & \isomorphic  \setA^\setC \cartprod \setB^\setC, \\ \label{eq:sets-product-power}
    \setA^{\setB \setdisunion \setC}           & \isomorphic \setA^\setB \cartprod \setA^\setC, \\ \label{eq:sets-power-sum}
    (\setA \setdisunion \setB)^{\setC} & \isomorphic \stylesets{\sum_{\subA \setin \powerset \setC}} \setA^\subA \cartprod \setB^{\setC \backslash \subA} \label{eq:sets-sum-power}
\end{align}
...
\begin{equation}\label{eq:sets-associator-def}
