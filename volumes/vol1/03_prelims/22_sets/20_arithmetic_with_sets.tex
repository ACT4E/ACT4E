...
Now let's work on finding a canonical isomorphism
\begin{equation}
...
\end{equation}
The image $\mora(\phi)$ should be an element of $\makeset{\subA} \cartprod (\setA^{\subA} \cartprod \setB^{\setC \backslash \subA} )$ for some $\subA \setin \powerset \setC$. We will use $\subA = \subB$. What remains for us to do, then, is to define an element of $\setA^{\subB}$ and an element of $\setB^{\setC \backslash \subB}$, using $\phi$. 
