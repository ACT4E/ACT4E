...
We define a \classname{SetProduct} to be a \Setoid that also has the structure of the \SY{cartesian product} of sets.
This means that:
\begin{itemize}
...
\begin{itemize}
    \item The generic type variable \TypeVar{C} represents the type of the components; the generic type variable \TypeVar{E} refers to the type of the product.
          Note that we are not constraining the internal representation of the product.
          (A possible choice could be \pystr{E = List[C]}).
    \item The method \funcname{components} returns the ordered list of the \SY{setoid} factors; these must be \SY{setoids} of type \TypeVar{C}.
...
There is also a specialization, which corresponds to a product of finite sets; the only change is that the components are finite sets rather than \SY{setoids}.
