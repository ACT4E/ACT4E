...
%
\begin{equation*}
    \ela \setin\bigsetunion \setofsetsA   \Leftrightarrow \ \exists \setA \setin\setofsetsA \colon \ela \setin\setA.
\end{equation*}
%
Similarly we can define the intersection~$\bigsetintersection \setofsetsA$ of all the sets which are elements of~$\setofsetsA$:
%
\begin{equation*}
    \ela \setin\bigsetintersection \setofsetsA  \Leftrightarrow \  \forall \setA \setin\setofsetsA \colon \ela \setin\setA.
\end{equation*}
...
\begin{equation*}
    \setA \setunion \setB = \bigsetunion \{ \setA, \setB \}  \text{ and } \setA \setintersection \setB = \bigsetintersection \{ \setA, \setB \}.
\end{equation*}
...
To spell this out, let~$\{ \setA_i \}_{i \setin\stylesets{I}}$ be a collection of sets (with index set~$\stylesets{I}$).
We can consider the set of sets
\begin{equation*}
    \{  \setA_i \mid i \setin\stylesets{I} \},\footnote{For those familiar with ZFC set theory: the axiom of replacement ensures that this is a set.
    }
\end{equation*}
and in particular also its union~$\setA \definedas \bigsetunion \{  \setA_i \mid i \setin\stylesets{I} \}$.
Now, our formal definition of a family~$\{ \setA_i \}_{i \setin\stylesets{I}}$ of sets is that it is a function
\begin{equation*}
    \mapa\colon \stylesets{I} \to \powerset \setA
\end{equation*}
such that~$\mapa(i) = \setA_i$ for all~$i \setin\stylesets{I}$.

\todojira{609}{POST-ALPHUBEL: include some sort of a non-arbitrary (and useful??) example to make more concrete and illustrate?}

\begin{remark}
    To simplify notation, for unions of families of sets we will usually write~$\bigsetunion_{i \setin I} \setA_i $ instead of~$\bigsetunion \{  \setA_i \mid i \setin\stylesets{I} \}$, and similarly for intersections.
\end{remark}
...
What about if we start with a family~$\{ \setA_i \}_{i \setin\stylesets{I}}$ of sets -- can we still take their cartesian product or disjoint union?
The answer is yes.
...
We define the cartesian product of a family~$\{ \setA_i \}_{i \setin\stylesets{I}}$ as
\begin{equation}
    \underset{i \setin\stylesets{I}}{\stylesets{\prod}}  \setA_i  \definedas \{ \mapa \colon \stylesets{I} \to \bigsetunion_{i \setin\stylesets{I} } \setA_i \mid \mapa(i) \setin\setA_i \}.
\end{equation}

And for the disjoint union of a family~$\{ \setA_i \}_{i \setin\stylesets{I}}$ we set
\begin{equation}
    \underset{i \setin\stylesets{I}}{\stylesets{\sum}} \setA_i \definedas \bigsetunion_{i \setin\stylesets{I}} \setA_i \cartprod \{ i \}.
\end{equation}

\todojira{659}{Make sure we really need the above.}
