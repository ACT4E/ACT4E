...
\begin{equation}
    ...
    \mapa \mthen \mapb \colon \setA \to \setC,
\end{equation}
...
\end{equation}
Because such a function ``does nothing'', it behaves neutrally with respect to the composition of functions: given~$\mapa \colon \setA \to \setB$ and~$\mapb \colon \setB \to \setC$, we have~$\mapa \mthen \catid_\setB = \mapa$ and~$\catid_\setB \mthen \mapb = \mapb$.
...
\begin{solution}
    ...
    \begin{equation}
        \mapa \mthen \mapb=\mapa \mthen \mapc=\catid_\setA,
    \end{equation}
    and
    \begin{equation}
        \mapb \mthen \mapa=\mapc \mthen \mapa=\catid_\setB.
    \end{equation}
    Now, we can write
    \begin{equation}
        \begin{aligned}
            \mapc & =\catid_\setB \mthen \mapc \\
                  & =(\mapb \mthen \mapa)\mthen \mapc \\
                  & =\mapb\mthen (\mapa \mthen \mapc) \\
                  & =\mapb \mthen \catid_\setA \\
                  & =\mapb,
        \end{aligned}
    \end{equation}
...
\begin{solution}
    ...
    By definition,~$\mapb(\elb)=\ela$, and hence~$(\mapa\mthen \mapb)(\ela)=\mapb(\mapa(\ela))=\mapb(\elb)=\ela$, implying~$\mapa\mthen \mapb=\catid_\setA$.
    Similarly, take~$\elb\setin\setB$ and~$\ela=\mapb(\elb)$.
    Then, by definition we have~$\mapa(\ela)=\elb$, and hence~$(\mapb\mthen \mapa)(\elb)=\mapa(\mapb(\elb))=\mapa(\ela)=\elb$, implying~$\mapb\mthen \mapa=\catid_\setB$.
...
The latter means, by definition, that there exists~$\mapb \colon \setB \to \setA$ such that~$\mapa \mthen \mapb = \catid_\setA$ and~$\mapb \mthen \mapa = \catid_\setB$.
...
Observe that functions~$\singletonel \to \setA$ are in 1-to-1 correspondence with the elements of~$\setA$.
Indeed, a function~$\mapa\colon \singletonel \to \setA$ will have to map ``$\singletonel$'' to some element~$\mapa(\singletonel) \setin\setA$, and since~$\singletonel$ has no other elements, that is all that~$\mapa$ does.
So~$\mapa$ ``picks out'' an element of a~$\setA$.
We can work with functions~$\singletonel \to \setA$ in place of elements of~$\setA$.
...
If we use, instead of~$\ela \setin\setA$, the function~$\mapa \colon \singletonel \to \setA$ to which it corresponds, then the element~$\mapb(\ela) \setin\setB$ corresponds to the function~$\mapa \mthen \mapb\colon \singletonel \to \setB$.
