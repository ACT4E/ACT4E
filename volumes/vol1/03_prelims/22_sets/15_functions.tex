...
\end{equation}
holds.
A function which is both \SY{injective} and \SY{surjective} is called \emph{\iindex{bijective}}.
...
\begin{marginfigure}
    ...
    \todographics{\alphubel: @Gioele: slightly shrink these figures so that they fit in the margins.}
\end{marginfigure}
...
\begin{solution}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    Therefore,~$\ela=\ela'$ and~$\mapa$ is \SY{injective} (and therefore bijective).

    Now, let a map~$\mapa\colon \setB \sto \setA$ be bijective.
    One can define~$\mapb\colon \setB \sto \setA$ in the following way.
    Take a~$\elb\setin\setB$, and since~$\mapa$ is \SY{surjective} (it is bijective), there exists a~$\ela\setin\setA$ such that~$\mapa(\ela)=\elb$.
    Let~$\mapb(\elb)=\ela$.
