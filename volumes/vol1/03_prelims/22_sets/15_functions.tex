...
\todotext{\AC{We should have defined the symbol $\reals$ before.}}
...
The property \cref{eq:function-determinism} describes the ``mathematical determinism'' that functions are supposed to have: for any~$\ela \in \setA$ there exists \emph{exactly one} element~$\elb \in \setB$ that is ``the result'' of the function~$\mapa$ applied to~$\ela$.
...
\begin{equation*}
    \mapa(\setA) \definedas \{ \elb \in \setB \mid \exists \ela \in \setA \colon \mapa(\ela) = \elb \}.
\end{equation*}
More generally, given a subset~$\subA \subseteq \setA$, its image under~$\mapa$ is
\begin{equation*}
    \mapa(\subA) \definedas \{ \elb \in \setB \mid \exists \ela \in \subA \colon \mapa(\ela) = \elb \}.
\end{equation*}
Given a subset~$\subB \subseteq \setB$, its \emph{preimage} under~$\mapa$ is
\begin{equation*}
    \mapa^{-1}(\subB) \definedas \{ \ela \in \setA \mid  \mapa(\ela) \in \subB \}.
\end{equation*}
