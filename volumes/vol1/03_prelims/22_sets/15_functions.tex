...
\begin{equation}
    ...
    \end{cases}
\end{equation}
It can be checked that this sets up a 1-to-1 correspondence between functions~$\setB \sto \setTwo$ and subsets of~\setB.
...
Let~$\singleton$ denote the set~$\makeset{\singletonel}$ which has a single element, namely the symbol ``$\singletonel$'' (we could just as well have chosen anything else).
Observe that functions~$\singleton \sto \setA$ are in 1-to-1 correspondence with the elements of~\setA.
Indeed, a function~$\mapa\colon \singleton \sto \setA$ will have to map ``$\singletonel$'' to some element~$\mapa(\singletonel) \setin\setA$, and since~$\singleton$ has no other elements, that is all that~$\mapa$ does.
