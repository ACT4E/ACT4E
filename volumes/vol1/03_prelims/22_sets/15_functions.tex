...
Observe that functions~$\stylesets{1} \sto \setA$ are in 1-to-1 correspondence with the elements of~$\setA$.
Indeed, a function~$\mapa\colon \stylesets{1} \sto \setA$ will have to map ``$\singletonel$'' to some element~$\mapa(\singletonel) \setin\setA$, and since~$\stylesets{1}$ has no other elements, that is all that~$\mapa$ does.
So~$\mapa$ ``picks out'' an element of a~$\setA$.
We can work with functions~$\stylesets{1} \sto \setA$ in place of elements of~$\setA$.
...
If we use, instead of~$\ela \setin\setA$, the function~$\mapa \colon \stylesets{1} \sto \setA$ to which it corresponds, then the element~$\mapb(\ela) \setin\setB$ corresponds to the function~$\mapa \mthen \mapb\colon \stylesets{1} \sto \setB$.
