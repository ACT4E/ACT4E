...
The definition \cref{def:cartesian-product} is for a \emph{binary} operation of cartesian product: one where there are two factors. We can make an analogous ``n-ary'' definition for any finite number of factors.
...
\begin{ctdefinition}[Disjoint union of two sets]
    \label{def:disjoint-union-of-sets}
    Given sets~\setA and~\setB, their \SY{disjoint union}, or sum, is the set
    \begin{equation}
...
Consider the sets~$\setA=\makeset{\sbretzel, \stea}$ and~$\setB=\makeset{\sfondue, \smilk}$. Their \SY{disjoint union} can be represented as:
%
