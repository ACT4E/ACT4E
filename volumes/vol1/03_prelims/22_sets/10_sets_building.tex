...
A ``tuple'' is  a finite sequence or listing of ``things'' where the order matters and repetitions are allowed.
...
Our notation to indicate a tuple is via these kind of brackets: ~$\tup{\ela, \elb, \elc}$ denotes a 3-tuple, or triple, for example. The items inside the brackets will be called entries or components.

In general, given~$n \setin\natnumbers$, a tuple of length $n$ is called an $n$-tuple. In particular there is a tuple of length zero, which we call the empty tuple and denote by $\tup{}$. 


Typically, we will use tuples in situations where we also specify, for each entry of the tuple, a set for which that entry is an element or a type of thing of which that entry is an instance.

For example, if~$\setA = \makeset{ \sapple, \scarrot, \sgrapes}$ and~$\setB = \makeset{ \stea, \swater }$, then we will sometimes want to specify that~$\tupp{\sapple, \swater}$ is a 2-tuple where~$\sapple \setin\setA$ and~$\swater \setin\setB$.  Or, we might be considering pairs $\tup{\styleobj{V}, \stylemorph{A}}$ where $\styleobj{V}$ is a real vector space and $\stylemorph{A}: \styleobj{V} \rightarrow \styleobj{V}$ is a real linear map. 


\subsection{Lists}
\label{sec:lists}

We will use the concept of \emph{list} when we want to talk about a tuple of things which are all elements of a single specified set or are all things of a specified type. 

For example, we use $\maketypedlist{3, 7, 8}{\natnumbers}$ to denote a list of natural numbers, and $\maketypedlist{3, 7, 8}{\reals}$ to denote a list of real numbers. Often it will be clear which type of things we are dealing with, in which case we drop the subscript in our notation and simply write $\makelist{3, 7, 8}$. 

Of course, the type of things we consider need not be numbers. For example, we might work with lists whose entries are from the set  $\setA = \makeset{\sapple, \sbanana, \scarrot, \sbretzel, \sgrapes}$. Such a list is $\makelist{\sgrapes, \sapple, \scarrot, \sapple}$, for instance. 

Similar to tuples, lists can have any length $n \in \natnumbers$, including zero. The zero-length list will be called the \emph{empty list} and denoted $\makelist{}$.  
...
%In fact we have a notion of $n$-fold cartesian product for any $n \in \natnumbers$.
