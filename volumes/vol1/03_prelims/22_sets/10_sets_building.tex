...
We \hypertarget{target:tupconcat}{concatenate tuples} using the symbol $\tupconcat$:
\begin{equation}
...
We \hypertarget{target:listconcat}{concatenate lists} using the symbol $\listconcat$:
\begin{equation}
...
\begin{remark}
    Later we will see that the \SY{cartesian product} of sets is a special case of a very general construction in category theory, called the \SY{categorical product}, and that the \SY{disjoint union} of sets is a special case of a ``dual'' construction, called coproduct.
\end{remark}
