...
\begin{equation*}
    \setA \setdisunion \setB \coloneqq (\setA \cartprod \{ 1 \}) \cup (\setB \cartprod \{2\}).
\end{equation*}
...
The sets~$\{ 1 \}$ and~$\{2\}$ here simply provide labels which ``force'' the sets~$\setA \cartprod \{ 1 \}$ and~$\setB \cartprod \{2\}$ to be disjoint (even if~$\setA$ and~$\setB$ have elements in common).
...
By definition, this is~$(\setA \cartprod \{ 1 \}) \cup (\setA \cartprod \{2\})$, so if~$\setA = \{ \sapple, \sbanana, \scarrot\}$, then
\begin{equation*}
...
For example, if~$\setA = \emptyset$, then~$\emptyset \setdisunion \setB = \setB \cartprod \{2\}$, and similarly~$\setA \setdisunion \emptyset =  \{ \disunionA{\ela} \mid \ela \in \setA \}$.
