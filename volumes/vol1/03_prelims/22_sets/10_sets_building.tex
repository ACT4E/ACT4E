...
If we fix $\setA$, there are a number of operations that take elements $\powerset \setA$ and produce other elements of $\powerset \setA$. 

For one, given $\subA \in \powerset \setA$ and $\subB \in \powerset \setA$, their union $\subA \cup \subB$ and intersection $\subA \cap \subB$ are again elements of $\powerset \setA$. 

Given $\subA \in \powerset \setA$, the \emph{complement} of $\subA$ with respect to $\setA$ is
\begin{equation}
\setA \backslash \subA := \{ \ela \in \setA \mid \ela \notin \subA \}.
\end{equation}
When the set $\setA$ is clear, the notation $\subA^c$ instead of $\setA \backslash \subA$ is also common. 
