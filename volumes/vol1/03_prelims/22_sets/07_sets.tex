...
Sets with finite cardinality are called \emph{finite sets}. If a set is not finite, there are different possible ``infinite sizes''. We will delve into distinguishing different infinite cardinalities only later if we need to.

Our notation for the size of a set $\setA$ will be $\vert \setA \vert$. For now, $\vert \setA \vert$ can be either zero (for the emtpy set), a natural number, or infinite. 
