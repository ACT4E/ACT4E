...
\begin{remark}
    ...
    \begin{equation*}
        \natnumbers = \{0, 1, 2, 3, 4, \ldots \}.
    \end{equation*}
    Similarly, the set~$\wnumbers$ of integers (or ``whole numbers'') is
    \begin{equation*}
        \wnumbers = \{\ldots, -3, -2, -1, 0, 1, 2, 3, \ldots \}.
    \end{equation*}
    ...
    \todotext{\AC{The last two sentences seem out of place in this section concerned with simple notations}}
\end{remark}
...
\begin{equation*}
    \stea \in \makeset{ \sapple, \scarrot, \sgrapes, \stea }
\end{equation*}
...
%
\begin{equation*}
    \{ \ela \in \setB \mid \ela \text{ is a fruit} \} = \{ \sapple, \sgrapes \}.
\end{equation*}
...
\footnote{For this use, respectively,~$Q(x) = (x \in A) \booland (P(x))$ and~$Q(x) = (x \in A) \Imp P(x)$.}
