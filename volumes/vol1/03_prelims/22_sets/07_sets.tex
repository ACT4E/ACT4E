...
\devel{
    ...
    \begin{equation}
        \natnumbers = \{1, 2, 3, 4, ... \}
    \end{equation}
    of \emph{natural numbers}.
    In some cultures, the convention is that the set $\natnumbers$ also contains the number zero; however for us it does not.
...
$\natnumbers$ denotes the set
\begin{equation}
    \natnumbers = \{0, 1, 2, 3, 4, ... \}
\end{equation}
of \emph{natural numbers}.
In some cultures, the convention is that ``the set of natural numbers''  does not contain the number zero; we follow the ISO 80000-2 standard~\cite{ISO:2009:IQU} that includes it.
...
\todojira{604}{What? we always meant \natnumbers to include zero.}
$\wnumbers$ denotes the set
\begin{equation}
    \wnumbers = \{... -3, -2, -1, 0, 1, 2, 3, ... \}
\end{equation}
of integers (or whole numbers).
An important subset of $\wnumbers$ is the set
\begin{equation}
    \natnumbers = \{1, 2, 3, 4, .
    .. \}
\end{equation}
of \emph{natural numbers}.
In some cultures, the convention is that the set $\natnumbers$ also contains the number zero; however for us it does not.
If we wish to include zero, we will use the notation $\natnumbers_0$ or $\wnumbers_{\geq 0}$.

\todojira{604}{@J: fill this in more}
...
\todojira{605}{@J: perhaps move this subsection to the section on foundations}
