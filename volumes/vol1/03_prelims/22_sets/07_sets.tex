...
\\The \hypertarget{target:setin}{symbol to indicate membership} is ``$\setin$'': for example,
\begin{equation}
...
the set~\natnumbers of \hypertarget{target:natnumbers}{natural numbers}\footnote{
...
\end{equation}
%
the set~$\wnumbers$ of \hypertarget{target:wnumbers}{integers} (or ``whole numbers'')
\begin{equation}
    \wnumbers = \makeset{ 0, +1, -1, +2, -2, +3, -3, \ldots},
\end{equation}
%
or the \hypertarget{target:ratnumbers}{rational numbers}~$\ratnumbers$, \hypertarget{target:reals}{real numbers}~\reals, and complex numbers $\cnumbers$.
...
\label{sec:booleans}
The \hypertarget{target:set-of-booleans}{\maindef{set of booleans}} is defined as:
\begin{equation}
