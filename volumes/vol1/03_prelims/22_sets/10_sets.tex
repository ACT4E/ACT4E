...
\todotext{@J: move this to somewhere after we've discuss functions, so that we can discuss arithmetic analogies, e.g. involving powers of sets}
...
If a set is not finite, there are different possible ``infinites sizes''. However, we will usually not need to trouble with these distinctions; it suffices for now for us to distinguish between finite sets and sets with infinite cardinality.  
