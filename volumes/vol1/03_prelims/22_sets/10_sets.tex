...
\{ \setA, \setB, \setC \}
\end{equation}
whose elements are $\setA$, $\setB$, and $\setC$. One special case that sometimes gives rise to confusion is the fact that $\emptyset$ and $\{ \emptyset \}$ are different sets. The former is the emptyset (is has no elements), while the latter is a set which has precisely one element, and that element happens to be the set~$\emptyset$. 
...
From one perspective, the criterion for knowing when two sets are equal reduces to knowing when elements (or ``things'') are equal. For instance, if $\setA$ and $\setB$ are sets, then to check if the sets $\{ \setA \}$ and $\{ \setB \}$ are equal, we need to check if their respectively elements, namely $\setA$ and $\setB$, are equal. From another perspective, the question of equality of sets can be expressed in terms of membership: $\setA$ and $\setB$ are equal if and only if the statements $\ela \in \setA$ and $\ela \in \setB$ are logically equivalent for every instantiation of the variable $\ela$ with the elements of $\setA$ and $\setB$.  
