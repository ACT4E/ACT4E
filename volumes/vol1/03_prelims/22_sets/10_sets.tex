...
In addition to the ``naming the elements'' way of specifying a set and building new sets from old using \text{e.g.} unions, many times sets are specified with the help of a logical ``statement'' or ``sentence'' which serves as a condition which characterizes its elements.
