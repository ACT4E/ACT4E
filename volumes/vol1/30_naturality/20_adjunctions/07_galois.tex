...
Here is a (very simplified) example in the context of ``private means of transportation''.
...
\subsection{Induced monotone functions}
...
This corresponds to reading off in the above table where there are crosses ``$\times$'' in the row labeled by $\ela$.
For example
...
More generally, given a subset $\setA \setsubseteq \things$, we can consider the largest set $\incidence_\sharp(\setA)$ of attributes which all elements of $\setA$ have in common.
For example
...
\end{equation}
because ``\text{classic car}'' and ``\text{classic bike}'' have \emph{no} attributes from the set $\attributes$ in common.
In general we have
...
Observe that the larger $\setA$ is, the smaller $\incidence_\sharp(\setA)$ will be.
...
\end{equation}
Another way of saying this is to say that $\incidence_\sharp$ is a monotone function of posets
...
\end{equation}
Or, equivalently, we can say that $\incidence_\sharp$ is a monotone function
...
Thus we have a monotone function
...
\end{equation}
We will also want to use its opposite, the monotone function
...
One consequence of \cref{eq:concept-adjunction} is that the monotone functions
...
\begin{ctdefinition}\label{def:closure-operator}
    Let $\posA = \tup{\posAset, \posleq}$ be poset.
    A \emph{closure operator} on $\posA$ is

    \constit
    \begin{enumerate}
        \item a monotone function $\mora \colon \posA \mto \posA$;
    \end{enumerate}

    \condit
    \begin{enumerate}
        \item \text{Extensivity:} \ $\ela \posleq \mora(\ela) \quad \forall \ela \setin \posAset$;
        \item \text{Idempotence:} \ $\mora (\mora(\ela)) = \mora(\ela) \quad \forall \ela \setin \posAset$.
    \end{enumerate}
\end{ctdefinition}

\begin{ctdefinition}\label{def:interior-operator}
    Let $\posA = \tup{\posAset, \posleq}$ be poset.
    ...
    \condit
    \begin{enumerate}
        \item \text{Intensivity:} \ $\mora(\ela) \posleq \ela \quad \forall \ela \setin \posAset$;
        \item \text{Idempotence:} \ $\mora (\mora(\ela)) = \mora(\ela) \quad \forall \ela \setin \posAset$.
    \end{enumerate}
\end{ctdefinition}
...
\begin{proof}
    ...
    Applying the monotone function $\incidence_\sharp \mthen \incidence_\flat$ to the relation $\setA \setsubseteq (\incidence_\sharp \mthen \incidence_\flat)(\setA)$, we have
    \begin{equation}
...
Closure and interior operators arise in various contexts in mathematics.
...
\begin{ctdefinition}\label{def:closure-closed-elements}
    Let $\posA = \tup{\posAset, \posleq}$ be a poset, $\mora \colon \posA \mto \posA$ a monotone function, and $\ela \setin \posAset$ an arbitrary element of $\posA$.
...
\begin{ctdefinition}\label{def:cfa-concept}
...
\begin{ctdefinition}[Monotone Galois Connection]\label{def:monotone-galois-connection}
    A (monotone) \maindef{Galois connection} between \SY{posets}~\posA and~\posB is a pair of \SY{monotone functions}
    ...
    %
    \equationsag{globular_galois_connection}{eq:globular-galois-connection}
    %
\end{ctdefinition}

\begin{lemma}\label{lem:alternative-def-Galois-connection}
    Monotone functions $\mapa\colon \posA\to \posB$ and~$\mapb\colon \posB\to \posA$ form a Galois connection if and only if the following hold:
    \begin{enumerate}
...
\todotext{write here the intuition / perspective on galois connections in terms of optimization, where on poset contains parameters, and the other variables}

An interesting and useful intuition about Galois connections is to think about them in terms of ``constrained optimization''. Let us explain this perspective. Suppose we are given a Galois connection
\begin{center}
...
\end{tikzcd}
\end{center}
Let us think of the elements of one of the posets -- say $\posA$ -- as ``parameters'', and lets us think of the elements of the other poset -- in this case $\posB$ -- as ``variables''. Now let's fix a parameter $\posAel \in \posA$. We then think of the inequality
\begin{equation}
\posAel \posAleq \mapb(\posBel)
\end{equation}
as a condition on variables $\posBel \in \posB$, a condition that may be satisfied or not. 

The optimization setting then is this: we are interested in the set of variables
\begin{equation}
\makeset{ \posBel \in \posB \mid \posAel \posAleq \mapb(\posBel)}
\end{equation}
that satisfy this condition, and furthermore we wish to find the minimal elements of this set. 

In this situation, the value $\mora(\posAel))$ of the left adjoint $\mora$ at the parameter $\posAel$ is then precisely such a minimal element. That is precisely what the definition of Galois connection says. In other words, given the ``constraint function'' $\morb$, its left adjoint $\mora$ is such that, for \emph{every} parameter $\posAel$, the value $\mora(\posAel)$ is a solution to the optimization problem posed in terms of $\posAel$ and $\morb$. 

\subsection{Examples}

\begin{example}
Let $\mora \colon \setA \mto \setB$ be any function between sets, and let's fix it for the entirety of this example. There is a pair (!) of Galois connections induced from $\mora$. They translate between the posets $\tup{\powerset \setA, \subseteq}$ and $\tup{\powerset \setB, \subseteq}$. The overall situation is that we have monotone functions

\begin{center}
\begin{tikzcd}[every arrow/.append style={arrowfunctorstyle}]
    {\powerset \setA}
     &  &  & & & & {\powerset \setB}
    \arrow[""{name=-1, anchor=center, inner sep=0}, "{\stylemorph{\exists}_\mora}", bend left=50, from=1-1, to=1-7]
    \arrow[""{name=0, anchor=center, inner sep=6}, "{\stylemorph{\mora^*}}"', from=1-7, to=1-1]
    \arrow[""{name=1, anchor=center, inner sep=0}, "{\stylemorph{\forall}_\mora}"', bend right=50, from=1-1, to=1-7]
    \arrow["\dashv"{anchor=center, rotate=-90}, draw=none, from=0, to=1]
    \arrow["\dashv"{anchor=center, rotate=-90}, draw=none, from=-1, to=0]
\end{tikzcd}
\end{center}

with $\stylemorph{\exists}_\mora \dashv \stylemorph{\mora^*}$ and $\stylemorph{\mora^*} \dashv \stylemorph{\forall}_\mora$. 

How are the three monotone functions denoted above defined? 

First, $\stylemorph{\mora^*}$ is the function
\begin{equation}
\defmap{\stylemorph{\mora^*}}{\powerset \setB}{\mto}{\powerset \setA}{\subB}{ \makeset{ \ela \setin \setA \mid \mora(\ela) \setin \subB} }
\end{equation}
which computes the preimage under $\mora$ of any subset $\subB \subseteq \setB$. 

Next, the functions $\stylemorph{\exists}_\mora$ and $\stylemorph{\forall}_\mora$ are defined, respectively, as 
\begin{equation}
\defmap{\stylemorph{\exists}_\mora}{\powerset \setA}{\mto}{\powerset \setB}{\subA}{ \makeset{ \elb \setin \setB \mid  \exists \ela \setin \stylemorph{\mora^*}(\makeset{\elb}) : \ela \setin \subA} }
\end{equation}
and
\begin{equation}
\defmap{\stylemorph{\forall}_\mora}{\powerset \setA}{\mto}{\powerset \setB}{\subA}{ \makeset{ \elb \setin \setB \mid \forall \ela \setin \stylemorph{\mora^*}(\makeset{\elb}) : \ela \setin \subA} }
\end{equation}
Note how these two functions are defined in precisely the same way, except for in which quantifier, ``$\exists$'' or ``$\forall$'', is used. 

\end{example}

\todotext{Fill this section}

\todotext{@J: add the forall / exists adjunction between powersets, e.g. as described well and concisely in spivak's CT for the sciences book}


\subsection{Antitone Galois connections}
...
\begin{remark}\label{rem:not-focusing-on-antitone-Galois-connections}
    The underlying function of an antitone map $\mapa\colon \posA\to \posB$ defines a monotone function $\mapa\colon \posA\to \posB\op$ (or a monotone function $\mapa \colon \posA \op \to \posB$).
...
Because of the above remark, and because we prefer to work with monotone functions (since they are morphisms of posets), we will mainly focus on (monotone) Galois connections.
...
\begin{lemma}\label{lem:closure-interior-operators-from-Galois-connection}
    If $\mapa \colon \posA \to \posB$ and~$\mapb \colon \posB \to \posA$ form a Galois connection $\mora \vdash \morb$, then
...
\devel{
    ...
    \begin{lemma}
        ...
        $\delta$ and $\gamma$ are \SY{monotone functions}.
    \end{lemma}
    \begin{proof}
        We first prove that~$\delta$ is a \SY{monotone function}.
        ...
        \begin{equation}
            ...
            \end{aligned}
        \end{equation}
        meaning that~$A\posleqof{\posLA}
            B \Imp \delta(A)\posleqof{\posUB} \delta(B)$.
        We now prove that~$\gamma$ is a \SY{monotone function}.
        ...
        \begin{equation}
            ...
            \end{aligned}
        \end{equation}
        meaning that~$C\posleqof{\posUB}
            D \Imp \gamma(C)\posleqof{\posLA} \gamma(D)$.
    \end{proof}
    ...
    \begin{proof}
        ...
        \end{equation}
        meaning that~$A\posleqof{\posLA}
            B\Imp \alpha(A) \posgeq_{\posUB} \alpha(B)$.
        ...
        \end{equation}
        meaning that~$C\posleqof{\posUB}
            D \Imp \beta(C)\posgeq_{\posUB} \beta(D)$.
    \end{proof}
    ...
    \end{lemma}
    \begin{proof}
        In~\cref{lem:deltagammamonotone} we proved that~$\delta$ and~$\gamma$ are \SY{monotone functions}.
