...
\begin{tabular}{rc}
    ...
    \str{\setA \cartprod \setB} &$\setA \cartprod \setB$  \\
    \str{\mapa \funcprod \mapb} &$\mapa \funcprod \mapb$ \\
    \str{\prodMapob \Obja \Objb} &$\prodMapob \Obja \Objb$ \\
    \str{\prodMapmor \mora \morb } &$\prodMapmor \mora \morb $\\
    \str{\posA \Ptimes \posB} &$\posA \Ptimes \posB $ \\
    \str{\CatC \Ctimes \CatD} &$\CatC \Ctimes \CatD $  \\
\end{tabular}
...
We have a command \str{\pars} that is equivalent to \str{\left(...\right)}. 
...
The code for \str{\makeset} with divide the contents in the following chunks:
