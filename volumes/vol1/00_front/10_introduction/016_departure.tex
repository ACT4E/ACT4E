...
\begin{itemize}
    ...
    \item In the discussion of semigroups, we use ``$\mtimes$'' rather than ``$\after$'' as the \SY{semigroup} composition operation.
...
In our trajectory, \SY{natural transformations} appear first associated to monoidal categories.
The role they play there is associated to very technical checks.
Nothing exciting!
We made the decision to provide a version of monoidal categories that are strict, so that there is no need for \SY{natural transformations}.
...
We provide the traditional exposition of monoidal categories in \cref{chap:generalization} after the introduction of \SY{natural transformations} in \cref{chap:naturality}.
