...
Let's define two interfaces: \classname{Setoid}, for a generic set, and \classname{FiniteSet}, for a setoid that has a finite set of elements. They are defined in the package \str{act4e_interfaces} which is in the repository \str{ACT4E/ACT4E-interfaces}.

This is the interface for \classname{Setoid}:

\classsource{Setoid}

The class \classname{Setoid}  derives from \classname{ABC}, which means \emph{abstract base class}; it tells the interpreter that some of the methods of the class will have to be implemented by the subclasses.
...
For \classname{Setoid}, we ask that the implementer implements the \funcname{contains} function, and they can optionally implement \funcname{equal}.

This is the semantics of the methods:

\begin{compactitem}
  \item The method \funcname{contains} checks if an element is part of this set.
  \item The method \funcname{equal} checks if two elements are the same. The default way to do this is to use Python's equality operator (which calls the \pystr|__equal__| method of the class of the first object); later we will see cases in which we might want to choose something different
\end{compactitem}
...
This is the interface for \classname{FiniteSet}:
...
The class inherits from \classname{Setoid} and add some methods:
...
You will have to implement the \classname{FiniteSet} class as part of the exercise.
