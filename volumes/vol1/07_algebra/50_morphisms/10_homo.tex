...
\todotext{The question of whether to use the term 'morphism' or the term 'homomorphism' in this chapter is deferred to a later point. Can ostensibly be decided quite later in the book process.}

\todotext{This section needs more examples}

A \emph{semigroup morphism} is a map between \SY{semigroups} that ``preserves the structure'' of composition.
...
\begin{marginfigure}
    ...
    \label{fig:semigroup-morphism-diagram}
\end{marginfigure}
...
\begin{ctdefinition}[Identity morphism]
    ...
    \begin{equation}
        ...
        \funid_\sgrpAset (\sgrpela \mtimesof{\sgrpAset} \sgrpelb) \ = \ \sgrpela \mtimesof{\sgrpAset} \sgrpelb
        \ = \
        \funid_\sgrpAset (\sgrpela )\mtimesof{\sgrpAset}\funid_\sgrpAset(\sgrpelb).
    \end{equation}
...
\todotext{We need to define the adjective 'isomorphic'}


\todotext{Before the lemma below, we first have to define/explain what we mean by the word composition here.}
...
\todotext{The example below is doing too much at once. Separate log and exp out into distinct examples, then discuss that they are mutually inverse as a third separate message.}
...
\begin{example}[Transition function, continuation of \cref{sub:transition-functions}]
    Consider the map
    \begin{equation}
...
\todotext{JL: I commented out below an example that seems strange and should be clarified or removed}
...
\todotext{@AC: simplify and improve this section}

\todotext{In the example below, the 'list functor' is being used it seems -- shall we explain that this particular example connects to the 'list construction' that we already discussed?}

\begin{example}[ASCII code]
    ASCII encoding takes any alphanumeric characters and symbols into a number between 0 and 127 (\cref{fig:asciifigure}).
...
\begin{figure}[h]
    ...
    \label{fig:asciifigure}
\end{figure}

\subsection{Morse coding}
...
%
\begin{solution}
    The answer is \textbf{no} because the encoding is context-dependent; we do not know if a single letter is followed by a space or another letter.
    ...
    \end{equation}
    However, this cannot work, because in the second instance of $M$ we would need to output a letter separator, while in the first case, we do not.
    ...
    \begin{equation}
        ...
        \tup{X, \wordsp}.
    \end{equation}
    Based on this representation, we can define context-independent rules that make this map a morphism.
\end{solution}
...
\todotext{put definition below into the constituents and conditions format}
...
\todotext{add negative examples here first, then interesting examples and canonical examples (inluding the 'constant 1' morphism').}
...
\begin{gradedexercise}[\exname{TraceAndDeterminant}]
    ...
    We also note that~$\tup{\reals, {{\cdot}}, 1}$ and~$\tup{\reals, {{+}}, 0}$ are two different monoid structures on~$\reals$.
...
\todotext{rewrite definition below to say that its just a monoid morphisms between monoids that happen to be groups.}
...
\todotext{in the remarks below, remove part about neutral element}
...
\todotext{in the example below, say it works for any finite dimension}
...
\todotext{in the example below, treat using 2x2 matrices and expand the exposition}

\begin{example}[Square matrices with full rank]
    \label{exa:square-full}
    Fix an integer~$n \geq 1$ and consider the set of square matrices with full rank~$\mat{A} \setin \reals^{\ntimesn}$, which is to say $\mdet(\mat{A}) \neq 0$.
...
\begin{ctdefinition}[Identity morphism]
    ...
    Given a \SY{group}~$\grpA = \tup{\grpAset, \gtimes, \idgrp, \ginv}$, the \SY{identity function} $\grpAset \to \grpAset$ induces a \SY{morphism of groups}~$\funid_\grpA\colon \grpA \fto \grpA$.
\end{ctdefinition}
