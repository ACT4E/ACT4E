...
\begin{ctdefinition}[Representable Functor]\label{def:representable-functor}
    Let $\CatC$ be a category. A functor $\funa\colon\CatC \fto \Set$ is said to be \SY{representable}, if there exists an object $\Objc\setin\Ob_\CatC$ and a natural isomophism between $\funa$ and the functor $\Hom_\CatC[\Objc, -]$.
...
\begin{example}
    ...
    Analogously, the ways to map $\Hom_\CatC(\Objb, \Objd)$ to $\Hom_\CatC(\Obja, \Objd)$ is given by the set $\makeset{\morbn{1} \mapsto \moran{1}, \morbn{1} \mapsto \moran{2}, \morbn{2} \mapsto \moran{1}, \morbn{2} \mapsto \moran{2}}$. These hom-sets are isomorphic.
    \end{example}
