...
\devel{
    \begin{example}
        ...
        \todotext{@J: Finish}
        This example is ``dual'' to \cref{exa:meet-as-prod-cont}. We considered \XXX and we drew an arrow~$\ela \to \elb$ iff~$\ela \leq \elb$.
...
\begin{example}[Adapted from~\cite{spivak2014category}]
    \label{def:ex_graph}
    \index{\Graph}
    We can define a category of graphs~\Graph. Objects of this category are graphs~$\graph=\tupp{\vertices,\arcs,\source,\target}$.
