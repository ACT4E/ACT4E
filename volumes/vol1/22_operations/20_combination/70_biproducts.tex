...
\devel{
    ...
    \todotextjira{93}{@J: Write intro text.
Fill in with example of direct sum of vector spaces, and other examples... }
    
    
    \begin{example}
        We can define a category \iindex{$\Vect_\reals$}, composed of:
        \begin{compactitem}
            \item \emph{Objects}: real vector spaces;
        ...
        \end{compactitem}
        It is a good exercise to prove that $\Vect_\reals$ really forms a category.
In the following, we want to look at the coproduct in $\Vect_\reals$.
It is given by the \emph{direct sum} of vector spaces.
Recall that given vector spaces~$V$ and~$W$, their direct sum is the set
        \begin{equation*}
...
For addition, this means that given~$\tup{v_1,w_1},\tup{v_2,w_2}\in V\oplus W$, their sum in~$V \oplus W$ is
        \begin{equation*}
...
Let's now look at the universal property in this case, by considering any vector space $U\in \Ob_\Vect$, and linear maps~$S\colon V\to U$,~$T\colon W\to U$.
