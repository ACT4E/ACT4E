...
Let $\setA$ be a set. For any subsets $\subA, \subB \subseteq \setA$, let us draw an arrow $\subA \to \subB$ iff $\subA \subseteq \subB$.  Then ``taking the intersection'' is an example of the categorical product; see \cref{fig:exa_prod_intersection}. 
  \begin{marginfigure}
  \begin{center}
  ...
  \end{marginfigure}
 For instance, let $\setA = \{ 1, 2, 3, 4 \}$, $\subA = \{ 1, 2, 3 \}$, and $\subB = \{2, 3, 4 \}$. Then $\subA \cap \subB = \{2, 3 \}$, and $\{ 1, 2, 3 \} \supseteq \{2, 3 \} \subseteq \{2, 3, 4 \}$. 
\end{example}
...
We will see that the cartesian product exemplifies the categorical product when we are working within the category \Set, but that in other  categories, the categorical product can show itself quite differently!
...
Let~$\Participants$ denote the set of participants. \JL{Since the water emoji is hard to see, perhaps we could use the "apple juice box" emoji in its place?}
