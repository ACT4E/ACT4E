...
\begin{equation*}
  \begin{aligned}
    \phi_{f,g}\colon \snack{Participants}&\to \snack{Snacks}\times \snack{Drinks}\\
    p&\mapsto \tup{f(p),g(p)}.
  \end{aligned}
\end{equation*}
...
The whole situation can be summarized thus: given a set~$\snack{Participants}$ and functions~$f\colon \snack{Participants} \to \snack{Snacks}$ and~$g\colon \snack{Participants} \to \snack{Drinks}$ as in \cref{fig:snacks_2}, there is a unique function~$\phi_{f,g} \colon \snack{Participants} \to \snack{Snacks} \times \snack{Drinks}$ such that the diagram \cref{fig:snacks_3} commutes.
...
\book{
  ...
  \begin{ctdefinition}[Product category]
    ...
    \begin{compactenum}
      ...
      \begin{equation}
          \tup{f,g}\then \tup{f',g'}=\tup{f\then f',g\then g'}.
      \end{equation}
    \end{compactenum}
  \end{ctdefinition}
