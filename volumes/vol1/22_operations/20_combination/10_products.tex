...
First, we recall the notion of \emph{cartesian product of sets} (introduced in \cref{sec:cartesian-product}) via an example.
...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            ...
            \{ 1, 2, 3 \} \setsupseteq \{2, 3 \} \setsubseteq \{2, 3, 4 \}
        \end{equation}
        %
        \begin{equation}
...
\begin{example}
    ...
    For any $x_1,x_2\setin \reals$ let us draw an arrow~$x_1\to x_2$ iff~$x_1\Rleq x_2$.
...
\begin{example}
    ...
    Let~$\setA$ be a set.
    For any subsets $\subA_1, \subA_2 \setsubseteq \setA$, let us draw an arrow $\subA_1 \to \subA_2$ iff $\subA_1 \setsubseteq \subA_2$.
    ...
    Then $\subA_1 \setintersection \subA_2 = \{2, 3 \}$, and $\{ 1, 2, 3 \} \setsupseteq \{2, 3 \} \setsubseteq \{2, 3, 4 \}$.
\end{example}
...
\begin{example}
    ...
    For any $x_1,x_2\setin \reals$, we drew an arrow~$x_1\to x_2$ iff~$x_1\Rleq x_2$.
    ...
    It says that if $t \setin \reals$ is such that $t \leq x_1$ and $t \leq x_2$, then $t \leq \min \{ x_1, x_2 \}$.
...
\begin{example}
    ...
    If we choose~$\setC = \{ 2 \}$, we see that~$\setC \setsubseteq \subA_1$ and~$\setC \setsubseteq \subA_2$, and that also~$\setC \setsubseteq \subA_1 \setintersection \subA_2$ (as it must, according to the universal property).
...
\begin{lemma}
    ...
    View this poset as a category (this means there is a single morphism~$\subA_1 \mto \subA_2$ if and only if~$\subA_1 \setsubseteq \subA_2$).
    For any two objects~$\subA_1, \subA_2 \setin \powerset \setA$, their categorical product exists and is given by~$\subA_1 \setintersection \subA_2 \setin \powerset \setA$.
\end{lemma}
