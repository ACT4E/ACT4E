...
\todojira{87}{Cartesian product appearing before somewhere else. Let's not duplicate definitions. (in intro to sets and functions, but also somewhere else, find, and add references).}
...
\begin{example}
    ...
    \begin{equation}
        ...
        \setA \cartprod \setB = \{ \disunionA{\ariesicon}, \disunionB{ \ariesicon}, \tup{3, \ariesicon}, \tup{4, \ariesicon}, \disunionA{\cancericon}, \disunionB{ \cancericon}, \tup{3, \cancericon}, \tup{4, \cancericon} \}.
    \end{equation}
...
\devel{
    \todotext{finish \cref{exa:meet-as-prod}}
    \begin{example}
        \label{exa:meet-as-prod}
        Let $...$ \XXX be a lattice. For any element $\ela, \elb \in ...$, \XXX let us draw an arrow $\ela \to \elb$ iff $\ela \leq \elb$. Then the ``meet'' (the operation $\wedge$) of two elements is an example of the categorical product; see \cref{fig:exa_prod_meet}.
        \begin{marginfigure}
            \centering
...
Let~$\Participants$ denote the set of participants. \todo{\JL{Since the water emoji is hard to see, perhaps we could use the "apple juice box" emoji in its place?}}
...
Alternatively, snacks could be pre-packaged in such a way as to allow all possible combinations of food and drink choices. This corresponds to~$\Snacks\cartprod \Drinks$.
...
It turns out that this state of affairs \emph{characterizes} the cartesian product $\Snacks\cartprod \Drinks$. We think of the diagram
\begin{equation}
...
\todotext{Need to use the set notation for Z, X, etc. below}
\begin{example}[Product code]
    ...
    However,~$Z$ is not precisely the cartesian product of $X$ and $Y$ (which we will call $X\cartprod Y$).
    The elements of~$Z$ are 12-digit codes, while elements of~$X\cartprod Y$ are pairs~$\tup{x,y}$ where~$x$ is a 8-digit code and~$y$ is a 4-digit code. Since both~$Z$ and~$X\cartprod Y$ satisfy the definition of categorical product, they must, by \cref{prod unique up to iso}, be isomorphic.

    \begin{center}
        \includesag{050_digits_3}
    \end{center}
    ...
    \begin{equation}
        ...
        \phi_{\text{first 8},\text{last 4}}\colon Z \to X\cartprod Y
    \end{equation}
    maps for instance
    \begin{equation}
...
\devel{
    \begin{example}
        ...
        This is a continuation of \cref{exa:meet-as-prod-cont}. We considered \todo{FINISH} and we drew an arrow~$\ela \to \elb$ iff~$\ela \leq \elb$.
        So the category at play here is the one corresponding to the poset underlying \XXX .
