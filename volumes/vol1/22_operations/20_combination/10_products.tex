...
\todojira{87}{Cartesian product appearing before somewhere else.
Let's not duplicate definitions. (in intro to sets and functions, but also somewhere else, find, and add references).}
...
\devel{
    \todotext{@Gioele: finish \cref{exa:meet-as-prod}}
    \begin{example}
        \label{exa:meet-as-prod}
        Let $...$ \XXX be a lattice.
        For any element $\ela, \elb \in ...$, \XXX let us draw an arrow $\ela \to \elb$ iff $\ela \leq \elb$.
        Then the ``meet'' (the operation $\wedge$) of two elements is an example of the categorical product; see \cref{fig:exa_prod_meet}.
        \begin{marginfigure}
            \centering
...
Let~$\Participants$ denote the set of participants. \todotext{@Gioele: \JL{Since the water emoji is hard to see, perhaps we could use the "apple juice box" emoji in its place?}}
...
\todotext{@J: Need to use the set notation for Z, X, etc. below}
...
\devel{
    \begin{example}
        ...
        This is a continuation of \cref{exa:meet-as-prod-cont}.
        We considered \XXX \todotext{@Gioele: Finish \cref{exa:meet-as-prod-cont} } and we drew an arrow~$\ela \to \elb$ iff~$\ela \leq \elb$.
