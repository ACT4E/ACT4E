...
To formulate this version, all we need are morphisms, their composition, the notion of \SY{identity morphisms}, and the notion of equality of morphisms (for equations such as~``$\mapa \mthen \mapb= \catid_\ela$'').
...
\begin{ctdefinition}[Isomorphism]
    ...
    We say that~$\mora$ is an \emph{\iindex{isomorphism}} if there exists a morphism~$\morb\colon \Objb \mto \Obja$ such that~$\morab = \catidat\Obja$ and~$\morb \mthen \mora = \catidat\Objb$.
\end{ctdefinition}
...
\end{definition}
\begin{exercise}
    Show that strictly \SY{monotone maps}.$\mapa\colon \reals \sto \reals$ are invertible.
\end{exercise}
\begin{solution}
