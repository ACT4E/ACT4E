...
\begin{example}[Semantic coherence]
    ...
    This can be modeled by a function~$f\colon M \to \{ \text{\Si, No} \}$ which maps a given dish~$m \setin M$ to the statement ``\Si'' (yes, I'd eat it) or ``No'' (no, I wouldn't eat it).
    ...
    \begin{equation*}
        \{ m \setin M \mid f(m) = \text{\Si}  \qqand  g(m) = \text{Ja} \}.
    \end{equation*}
    ...
    \begin{equation*}
        \{ m \setin M \mid \tilde f(m) = \varheartsuit \qqand \tilde g(m) = \varheartsuit \}.
    \end{equation*}
...
\begin{solution}
    ...
    Suppose that~$\mapa$ is strictly monotone, but there esist~$\setAel,\setBel\setin \reals$ such that~$\mapa(\setAel)=\mapa(\setBel)$ but~$\setAel\neq \setBel$.
