...
\begin{example}
    ...
    \begin{equation*}
        \defmapcomma{\tup{a,b}}{\reals}{\sto}{\reals}{x}{ax-d}
    \end{equation*}
...
\devel{
    ...
    \begin{definition}[Strict monotone functions]
        ...
        A function~$\mapa\colon \reals \to \reals$ is \emph{strictly monotone} if for all~$\setAel,\setBel\in \reals$:
        \begin{equation*}
    ...
    \begin{solution}
        \todo{finish}
        Because of \cref{def:strict-monotone-function}, given~$\setAel,\setBel\in \reals$, if~$\mapa(\setAel)=\mapa(\setBel)$, then we must have~$\setAel=\setBel$.
        Therefore, there exists a function~$\mapb$ such that~$\mapa \then \mapb=\catid$.
        ..
    \end{solution}
