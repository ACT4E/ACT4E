...
\begin{ctdefinition}[Adjunction, Version 1]
    ...
    \begin{enumerate}
        \item A \SY{functor}~$\funl\colon \CatC \fto \CatD$ (the \emph{left adjoint});
        \item A \SY{functor}~$\funr\colon \CatD \fto \CatC$ (the \emph{right adjoint});
        \item A \SY{natural isomorphism}~$\adjtau: \HomSet\CatD{\funl -}{-} \nto \HomSet\CatC{-}{\funr}$
    \end{enumerate}
    We use the notation~$\funl \adjunction  \funr$ to indicate that~$\funl$ and~$\funr$ form an \SY{adjunction}, with $\funl$ the left adjoint and $\funr$ the right adjoint.
\end{ctdefinition}

\begin{remark}
    Note that~$\adjtau$ is a \SY{natural isomorphism} between \SY{functors} of the form
    \begin{equation}
