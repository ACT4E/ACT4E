...
\begin{definition}
Let $\posA = \tup{\posAset, \posleq}$ be poset and  $\mora \colon \posA \mto \posA$ a closure operator. An element $\ela \setin \posAset$ is called \emph{closed} if $\mora(\ela) = \ela$. 
\end{definition}
...
Returning to formal concept analysis, let's look at what closed elements for the closure operator $
\incidence_* \mthen \incidence^* \colon \tup{\powerset \things, \subseteq} \mto \tup{\powerset \things, \subseteq}$ are in terms of our simple example. 
