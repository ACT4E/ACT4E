...
\begin{ctdefinition}[Monotone Galois Connection]\label{def:monotone-galois-connection}
    A \emph{\iindex{monotone Galois connection}} between \SY{posets}~\posA and~\posB is a pair of \SY{monotone maps}.$\mapa\colon \posA\to \posB$ and~$\mapb\colon \posB\to \posA$ such that for all~$\posAel\setin \posA$,~$\posBel\setin \posB$:
    \begin{equation}
