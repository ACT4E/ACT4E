...
Given an \SY{adjunction}~$\funa \dashv \funb$ from a category \CatC to a category \CatD, we have, by \cref{def:adj-iso} a \SY{natural isomorphism}~$\tau$ with components
\begin{equation}
...
Given the unit~$\nunit$ and counit~$\ncounit$, we can construct the components~$\tau_{\Obja,\Objb}$ of the \SY{natural transformation}~$\tau$ as follows.
...
\begin{exercise}
    ...
    \end{equation}
    between \SY{functors}~$\CatC\op \Ctimes \CatD \fto \Set $.
\end{exercise}
