...
Since \SY{mappings between functors} may be rather abstract, we will first start with a concrete example. Consider two categories of 
images: those that are not square $\CatC$ and those that are square $\CatD$.

A picture $\Obja \setin \ObC$ can be transformed in two ways: we can first apply a mask on it to render it monochromatic (black and white)
and crop it to a square, or first crop it and then apply a mask.

Notice that the mask being applied in the first case differs from the one used in the second, since the dimensions are different. Thus, 
we see that there is a mask for every different picture, which brings it to its black and white version. If we consider the diagram of transformations
(masks / cropping) onto images, then the diagram is \SY{commutative} (see \ref{def:commutative-diagram}). This means that going either way -- mask then crop or crop then mask --
is equivalent. This is the main idea of natural transformations.
...
$\funa_1, \funb_1:\CatC \fto \CatD$ and $\funa_2, \funb_2:\CatD \fto \CatE$ relating them. We can thus have the following four diagrams:
\begin{equation}
        \begin{tikzcd}[every arrow/.append style={arrowfunctorstyle}]
            \CatA \ar[r, bend left=40, "\funa_1"]\ar[r, bend right=40, "\funb_1"'] &
            \CatB \ar[l, phantom, "\stylenat{\Downarrow} \ntrafoa_1"]
...
\begin{equation}
    ...
    \end{tikzcd}
\end{equation}
Otherwise, we apply horizontal composition first, yielding
\begin{equation}
    \begin{tikzcd}[every arrow/.append style={arrowfunctorstyle}]
        \CatC &  &  &  & \CatD
