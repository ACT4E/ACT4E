...
To define natural transformations, the general situation we will start from is when we have two \SY{functors}~$\funa \colon \CatC \fto \CatD$  and~$\funb \colon \CatC \fto \CatD$, sharing the same source and target, respectively.
In the previous example above,~$\funa$ and~$\funb$ were database instances.

A natural transformation from~$\funa$ to~$\funb$ is then a kind of ``map'' that relates the two \SY{functors}.
...
(This does not mean, however, that there is at most one natural transformation between any two given \SY{functors}- on the contrary, there might be many!)
...
\begin{figure}[h!]
    ...
    \todographics{@Gioele: perfectionism: the double line is hacked with a white rectangle that does not render well on non-white background.}
\end{figure}
...
\todojira{632}{\alphubel: @JL: write up example here about natural trafos between monotone maps}.
...
\begin{example}
    ...
    \end{equation}
    These form the components of a natural transformation from the identity \SY{functor} on \Vect to the double dual \SY{functor}.
...
\begin{example}
    ...
    As a reminder, the \SY{functor} maps each set to its \SY{powerset}, and each map to a map distributed over subsets of the \SY{powerset}.
...
\begin{gradedexercise}[\exname{NatTrafosGraphs}]
    ...
    There, we defined a category~$\CatStyle{G}$ which has precisely two objects and four morphisms, see \cref{fig:graph-cat-again} (the two  \SY{identity morphisms} are not drawn).
    ...
    Now consider two \SY{functors}~$\funaA, \funaB \colon \CatStyle{G} \fto \Set$.
...
\begin{gradedexercise}[\exname{UpperSetsNatTrafos}]
    ...
    There we fixed a poset~\posA, viewed it as a category~$\CatStyle{P}$, and saw that \SY{functors}~$\CatStyle{P} \fto \Bool$ encode upper sets in~\posA.
    Suppose we have two \SY{functors}~$\funaA, \funaB \colon \CatStyle{P} \fto \Bool$.
