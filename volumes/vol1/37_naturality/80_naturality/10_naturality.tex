...
\begin{gradedexercise}[\exname{NatTrafosGraphs}]
    ...
    There, we defined a category~$\Cat{G}$ which has precisely two objects and four morphisms, see \cref{fig:graph-cat-again} (the two  \SY{identity morphisms} are not drawn).
    ...
    Now consider two \SY{functors}~$\funaA, \funaB \colon \Cat{G} \fto \Set$.
...
\begin{gradedexercise}[\exname{UpperSetsNatTrafos}]
    ...
    There we fixed a poset~\posA, viewed it as a category~$\Cat{P}$, and saw that \SY{functors}~$\Cat{P} \fto \Bool$ encode upper sets in~\posA.
    Suppose we have two \SY{functors}~$\funaA, \funaB \colon \Cat{P} \fto \Bool$.
