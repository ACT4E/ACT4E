...
\begin{example}
    \label{ex:Vect}
    Consider the category~$\Vect_{\reals}$ whose objects are real \SY{vector spaces} and whose morphisms are linear maps.
    ...
    \end{equation}
    These form the components of a \SY{natural transformation} from the \SY{identity functor} on \Vect to the double dual \SY{functor}.
...
\begin{gradedexercise}[\exname{UpperSetsNatTrafos}]
    ...
    There we fixed a poset~\posA, viewed it as a category~$\CatStyle{P}$, and saw that \SY{functors}~$\CatStyle{P} \fto \Bool$ encode \SY{upper sets} in~\posA.
