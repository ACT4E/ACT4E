...
%\item
%(8 points)
%Let $\adp \colon \F{\posgenA} \Ptimes \F{\posgenC} \profto \R{\posgenB} \Ctimes \R{\posgenC}$ be a morphism in $\DP$. Show that the design problem $\F{\posgenA}  \profto \R{\posgenB}$ given by the following composition
%\begin{equation}
%\rightunitor^{-1}_{\posgenA} \mthen (\catid_\posgenA \mtimescatmor \coev_\posgenC) \mthen (\adp \mtimescatmor \catid_\posgenC) \mthen (\catid_\posgenB \mtimescatmor \braiding_{\posgenA, \posgenA\op}) \mthen (\catid_\posgenB \mtimescatmor \ev_\posgenC) \mthen \rightunitor_\posgenB
%\end{equation}
