...
\todotextjira{199}{Write something about how we already saw string diagrams, and that (co)evaluation maps allow for bending the wires around to change the their direction, and that we can encode dual objects by adding a ``flow'' direction to the wires.
    Then motivate the notion of trace in a monoidal category by asking what we get when we bend a wire around and let the outout of a morphism flow back and be it's input -- ``closing the loop''}
...
\begin{ctdefinition}[Trace of an endomorphism]
    ...
    Let~$\Obja \setin \ObC$ be dualizable and let~$\mora \setin \Hom(\Obja, \Obja)$.
    The \emph{trace} of~$\mora$ is the morphism~$\Tr(\mora) \setin \Hom(\idmoncat, \idmoncat)$ defined by
    \begin{equation}
        \includesag{endo_trace}
    \end{equation}
    %\begin{equation}
    %\label{eq:endomorphism-gen-trace}
    %\idmoncat \overset{\coev_\Obja}{\mto} \Obja \mtimescatat \Obja^\vee  \overset{\mora \mtimescat \catid_{\Obja^\vee}}{\mto} \Obja \mtimescat \Obja^\vee  \overset{\braiding}{\mto}  \Obja^\vee \mtimescat \Obja \overset{\ev_\Obja}{\mto} \idmoncat.
    %\end{equation}
\end{ctdefinition}
...
\begin{gradedexercise}[\exname{LinearAlgebraTrace}]
    ...
    \todotextjira{516}{@JL: Didn't we call this already something different?}
...
\begin{widepar}
    \begin{ctdefinition}[Trace of a generalized endomorphism]
        ...
        %\Objb \overset{\catid_\Objb \mtimescat \coev_\Obja}{\mto} \Objb \mtimescat \Obja \mtimescat \Obja^\vee   \overset{\mora \mtimescat \catid_{\Obja^\vee}}{\mto} \Objc \mtimescat \Obja \mtimescat \Obja^\vee  \overset{\catid_\Objc \mtimescat \braiding}{\mto}  \Objc \mtimescat \Obja^\vee \mtimescat \Obja \overset{\catid_\Objc \mtimescat \ev_\Obja}{\mto} \Objc.
        %\end{equation}
        \begin{equation}
...
\todojira{361}{@JL: The above needs to be fixed to account for coherence morphisms in the non-strict case, and also for the definition of trace below}

%\devel{
%\begin{forslides}
%
%\begin{equation}\label{eq:slides-trace-1}
%\idmoncat \overset{\coev_\Obja}{\mto} \Obja \mtimescat \Obja^\vee  \overset{\mora \mtimescat \catid_{\Obja^\vee}}{\mto} \Obja \mtimescat \Obja^\vee  \overset{\braiding}{\mto}  \Obja^\vee \mtimescat \Obja \overset{\ev_\Obja}{\mto} \idmoncat.
%\end{equation}
%
...
\begin{ctdefinition}[Traced monoidal category]
    ...
    \begin{enumerate}
        ...
        \item \emph{Naturality in $\Objb$:} For any morphisms $\mora\colon \Obja\mtimescat \Objc \mto \Objb\mtimescat \Objc$ and $\morb : \Objb \mto \Objb'$,
              \begin{equation}
                  \Tr_{\Obja, \Objb'}^\Objc ( \mora \mthen (\morb \mtimescat \catid_\Objc) ) =  \Tr_{\Obja, \Objb}^\Objc (\mora) \then \morb
              \end{equation}
