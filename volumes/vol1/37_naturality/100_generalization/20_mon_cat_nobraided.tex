...
\begin{remark}
    ...
    The ``obvious'' or ``canonical'' choice then (given that we are considering \emph{any} \SY{monoidal category}) is that $\funb$ is the \SY{identity functor} and that $\funa$ is the \SY{functor} which acts on morphisms by mapping $\mora : \Obja \mto \Objb$ to
    %
    \begin{equation}
