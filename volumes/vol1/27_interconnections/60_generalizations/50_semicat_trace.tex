...
\section{Feedback and semicategories}

\begin{ctdefinition}[Semitraced symmetric stacking semicategory]
    \label{def:semitraced-assoc-stack-scat}
    We say that a symmetric stacking \SY{semicategory}~$\tup{\CatC, \mtimescatob, \mtimescatmor}$ is \emph{semitraced} if it is equipped with a family of \emph{semitrace functions}
    \begin{equation}
...
\subsection{Feedback for Moore machines}

\begin{example}
    Consider the semicategory \Moore of Moore machines.
    We show that when equipped with stacking and trace operations as previously defined, it fulfills the semitrace axioms for a strict monoidal stacking semicategory.

    \textbf{Compatibility with object stacking:}
    ...
    \textbf{Compatibility with morphism stacking:}
    ...
    \textbf{Compatibility with the monoidal unit:}
...
\devel{
\subsection{Feedback for proper LTI systems}
...
\begin{example}
    Consider the subsemicategory of \LTI only allowing proper LTI systems as morphisms, with the semitrace defined via the formula in \cref{def:trace-lti-prop}.
    We show here that the semitrace axioms for a strict monoidal stacking semicategory are satisfied.
...
\begin{example}
    \LTI with the trace as previously defined, does not satisfy all the trace axioms.
