...
Alternatively, in \cref{fig:double-stacked} we depict the stacking of the string diagrams for $\moran{1}$ and $\moran{2}$, respectively, by stacking their diagrams vertically and drawing a box around them.
The outer box denotes $\moran{1} \mtimescatmor \moran{2}$.
...
\end{equation}
However, as we have seen in REF, \todotext{Add cross reference} if we wish to have associativity on the nose we can work instead with an equivalent subcategory $\SetL$ of $\Set$ for which we can define a version of the cartesian product which \emph{is} associative. An analogous approach is possible for various other examples that we will consider as well. 

Later, in the chapter REF  \todotext{Add cross reference} on monoidal categories we will also discuss a different approach to the ``failure of associativity'' of the cartesian product on $\Set$ (and other similar examples). We have seen in REF  \todotext{Add cross reference} that the cartesian product is ``almost associative'' in that there is always a canonical isomorphism
\begin{equation}
