...
\begin{gradedexercise}[\exname{TracingRelations}]
    ...
    $$\Tr_{\Obja,\Objb}^\Objc \colon \HomSet{\Rel}{\Obja \cartprod \Objc}{\Objb \cartprod \Objc}\to \HomSet{\Rel}{\Obja}{\Objb}$$
    which is defined, for a morphism $\relA \setin\HomSet{\Rel}{\Obja \cartprod \Objc}{\Objb\cartprod \Objc}$, by
    \begin{equation}
        \Tr_{\Obja,\Objb}^\Objc(\relA) = \makeset{\tup{\ela, \elb} \setin\Obja \cartprod \Objb \mid \exists \elc \setin\Objc \colon \tup{\tup{\ela, \elc}, \tup{\elb, \elc}} \setin\relA}.
    \end{equation}
    ...
    \begin{enumerate}
        ...
        \item \emph{Vanishing II}:
              ...
              For any relation~$\relA \colon (\Obja \cartprod \Objc) \cartprod \Objd \to (\Objb \cartprod \Objc) \cartprod \Objd$ in $\Rel$,
              \begin{equation}
                  \Tr_{\Obja,\Objb}^\Objc\pars{
                      \Tr_{\Obja \cartprod \Objc, \Objb \cartprod \Objc}^\Objd(\relA)} = \Tr_{\Obja,\Objb}^{\Objc\cartprod \Objd}\pars{\associator_{\Obja, \Objc, \Objd} \mthen \relA \mthen \associator^{-1}_{\Objb, \Objc, \Objd}}.
              \end{equation}

        \item \emph{Superposing}:

              For any relation~$\relA \colon \Obja\cartprod \Objc \to \Objb\cartprod \Objc$ in $\Rel$,
              \begin{equation}
                  \Tr_{\Obje\cartprod \Obja,\Obje\cartprod \Objb}^{\Objc}\pars{\associator_{\Obje, \Obja, \Objc} \mthen \catidat\Obje \cartprod \relA \mthen \associator^{-1}_{\Obje, \Objb, \Objc}} = \catidat\Obje \cartprod \Tr_{\Obja,\Objb}^\Objc(\relA).
              \end{equation}
    \end{enumerate}
\end{gradedexercise}
...
\begin{ctdefinition}[Trace of a design problem]
    \label{def:dp-trace}
    Given a \SY{design problem}~$\adpa\colon \funposA \Ptimes \funposC \profto \resposB \Ptimes \resposC$, its \emph{trace}
    \begin{equation}
...
\todotext{J: some brackets and associators are missing in the proof below; perhaps fine to leave so but then we should make a comment}
