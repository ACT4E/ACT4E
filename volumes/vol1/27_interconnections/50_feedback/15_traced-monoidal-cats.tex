...
The following definition of a trace operator is identical with the definition of a feedback operator, except for two important differences:
\begin{enumerate}
    \item the dinaturality axiom for the trace operator is only required to hold for all morphisms, not just isomorphisms;
    \item we add the so-called yanking axiom, which encodes a compatibility condition between the trace and the symmetric braiding. 
\end{enumerate}
...
The diagrammatic equation in \cref{fig:trace-yanking} illustrates the Yanking axiom. 
\begin{figure}[h!]
    \centering
