...
Note that we use~$\sgrpmorA\colon \sgrpAset \sto \sgrpBset$ when we want to highlight the function between sets, and we use~$\sgrpmorA: \sgrpA \fto \sgrpB$ when we want to highlight the relation between semigroup structures.
...
\begin{ctdefinition}[Identity morphism]
    ...
    The identity function~$\mapidat\sgrpAset\colon \sgrpAset \sto \sgrpAset$ is always a \SY{morphism of semigroups}.
...
\begin{ctdefinition}[Semigroup isomorphism]
    ...
    A \SY{morphism of semigroups} ~$\sgrpmorA\colon \sgrpA \mto \sgrpB$ is called a \emph{semigroup isomorphism} if there exists a \SY{morphism of semigroups} ~$\sgrpmorB\colon \sgrpB \mto \sgrpA$ such that
    \begin{equation}
...
\begin{example}[Logarithms and exponentials]
    ...
    The reals with addition~$\tup{\reals, +}$ is a \SY{semigroup}.
    ...
    \end{equation}
    Because~$\mlog$ is a bijection and~$\mexp$ is its inverse, it already follows that~$\mexp$ is a morphism in the opposite direction.
...
\begin{example}[Transition function, continuation of \cref{exa:transition-functions}]
    ...
    Re-reading~\cref{eq:transition-property}, we can see that it is a morphism between the semigroup~$\tup{\nonNegReals,+}$ and the semigroup of endomorphisms of~$\reals^n$.
\end{example}
...
\begin{gradedexercise}[\exname{CharacterizeSemigroupIsos}]
    ...
    Prove that~$\sgrpmorA$ is an isomorphism of \SY{semigroups} if and only if the function~$\sgrpmorA \colon \sgrpAset \sto \sgrpBset$ is \SY{bijective}.
\end{gradedexercise}
...
\begin{example}[ASCII code]
    ...
    We can see ASCII encoding as a semigroup morphism of~$\listsof\alphanums$ to the free semigroup on the integers~$\listsof\asciichar$:
    \begin{equation}
        \label{eq:ASCII-semi}
...
We have defined \SY{semigroup morphism}.
A monoid morphism has the same properties, and one additional one: the constraint that it be compatible with the identity elements.
...
\begin{example}
    The set~$\monoidAset = \makeset{ \minusone, 0, \plusone }$, together with multiplication of whole numbers and with~$1$ as neutral element, forms a \SY{monoid}.
...
\begin{ctdefinition}[Monoid isomorphism]
    ...
    A \SY{morphism of monoids}~$\funa\colon \monoidA \fto \monoidB$ is called a \emph{monoid isomorphism} if there is a \SY{morphism of monoids}~$\funb\colon \monoidB \fto \monoidA$ such that
    \begin{equation}
...
\begin{gradedexercise}[\exname{MorphismMonoidIsomorphism}]
    Prove: a \SY{morphism of monoids}~$\funa\colon \monoidA \fto \monoidB$ is an isomorphism of monoids if and only if the function~$\funa\colon \monoidAset \sto \monoidBset$ is \SY{bijective}.
\end{gradedexercise}
...
\begin{gradedexercise}[\exname{TraceAndDeterminant}]
    ...
    These form a \SY{monoid}~$\tupp{\realsnn\com \cdot, \idmat}$, with matrix multiplication as composition, and the identity matrix as neutral element.
    They also form a \SY{monoid} $\tupp{\realsnn, +, 0}$ with matrix addition as composition, and the zero matrix as neutral element.
...
\begin{example}[Square matrices with full rank]
    ...
    This set, equipped with the usual matrix multiplication as the binary operation ($\mat{A}\mtimes \mat{B}\definedas \mat{AB}$), the identity matrix~$\idmat$ as the neutral element, and matrix inverse as the inverse ($\ginv(\mat{A})\definedas \mat{A}^{-1}$), forms a \SY{group}.
    ...
    \end{enumerate}
    This makes~$\mdet$ a \SY{group morphism} from the \SY{group of invertible square matrices} to the reals with multiplication.
\end{example}

As before for \SY{semigroups} and \SY{monoids}, the identity map on the underlying set of a group is a \SY{group homomorphism}.

\begin{ctdefinition}[Identity morphism]
    \label{def:identity-grp-mor}
    Given a \SY{group}~$\grpA =  \tup{\grpAset, \gtimes, \idgrp, \ginv}$, the identity function $\grpAset \to \grpAset$ induces a \SY{morphism of groups}~$\funid_\monoidA\colon \monoidA \fto \monoidA$.
\end{ctdefinition}

\begin{ctdefinition}[Group isomorphism]
    \label{def:grp-iso}
    A \SY{morphism of monoids}~$\funa\colon \grpA \fto \grpB$ is called a \emph{group isomorphism} if there is a \SY{morphism of groups}~$\funb\colon \grpB \fto \grpA$ such that
    \begin{equation}
