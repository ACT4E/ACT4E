...
\begin{ctdefinition}[Identity \whomo]
    ...
    The identity function~$\mapidat\sgrpAset\colon \sgrpAset \sto \sgrpAset$ is always a morphism of semigroups.
...
\begin{example}[ASCII code to binary]
    ...
    Therefore, we can see ASCII as a \whomo between~$\listsof\alphanums$ and binary lists~$\listsof{\makeset{0,1}}$.
\end{example}
\showslides{
    \begin{forslides}
        \begin{equation}
...
\begin{example}
    The set~$\monoidAset = \makeset{ \shortminus 1, 0, 1 }$, together with multiplication of whole numbers and with~$1$ as neutral element, forms a \SY{monoid}.
...
\begin{ctdefinition}[Identity \whomo]
    ...
    Similar to the case of \SY{semigroups}, the identity function induces a \whomo of monoids~$\funid_\monoidA\colon \monoidA \fto \monoidA$.
\end{ctdefinition}
...
\begin{gradedexercise}[\exname{MorphismMonoidIsomorphism}]
    Prove: a \SY{morphism of monoids}~$\funa\colon \monoidA \fto \monoidB$ is an isomorphism of monoids if and only if the function~$\funa\colon \monoidAset \sto \monoidBset$ is bijective.
\end{gradedexercise}
...
\todotextjira{21}{\bernina: @JL: Show the inverse operation being compatible with group structure, commuting with \whomos.
