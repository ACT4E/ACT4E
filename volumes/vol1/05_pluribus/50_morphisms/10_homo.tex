...
\devel{
    \begin{example}[Phonetic languages]
        \todotextjira{17}{Nice example: the map from sequence of characters to sequences of sounds is monoidal in certain languages (Korean, Japanese, almost in Italian.) and also invertible.}
    \end{example}
...
\begin{gradedexercise}[\exname{TraceAndDeterminant}]
    ...
    These form a monoid~$\tupp{\reals^{n \cartprod n}, \cdot, \idmat}$, with matrix multiplication as composition, and the identity matrix as neutral element.
