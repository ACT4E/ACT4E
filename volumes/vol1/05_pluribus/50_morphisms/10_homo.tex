...
\showslides{%
    \begin{forslides}
        ...
        \begin{equation}
            ...
            \prftree{f: \setA \to \setB}{x \setin \setA}{f(x) \setin \setB}
        \end{equation}
        %
        \begin{equation}
            \label{eq:function-axiom2}
            \prftree{f: \setA \to \setB}{x_1 \setin \setA}{x_2 \setin \setA}{x_1 = x_2}{f(x_1) = f(x_2) }
        \end{equation}
        %
        \begin{equation}
...
\begin{example}[ASCII code to binary]
    ...
    Therefore, we can see ASCII as a \whomo between~$\alphanums^*$ and binary lists~$\makeset{0,1}^*$.
\end{example}
\showslides{
    \begin{forslides}
        \begin{equation}
...
\begin{exercise}
    ...
    \begin{equation}
        ...
        \asciienc: \alphanums^* \to \makeset{0,1}^*
    \end{equation}
...
\begin{solution}
    ...
    \begin{equation}
        ...
        \asciienc^{-1}\colon  \makeset{0,1}^* \to \alphanums^*.
    \end{equation}
    ...
    ~$\asciienc\then \mapb = \monid_{\alphanums^*}$, but it is not a right inverse.
...
\begin{example}[Morse code]
    ...
    Define now the extended alphabet~$\setAprime$ to be the union of~$\setA$ and the set~$\makeset{\lettersp, \wordsp}$, where~$\lettersp$ is inter-letter space, and~$\wordsp$ is inter-word space.
...
\showslides{
    \begin{forslides}
        ...
        \begin{equation}
            \label{eq:morsemap}
            \morsemap \colon \mathcal{A} \to  \makeset{\morsedot, \morsedash}^*
        \end{equation}
        \begin{equation}
            \label{eq:morsemorph}
            \morsemorph \colon \mathcal{A}^* \to   \makeset{\morsedot, \morsedash}^*
        \end{equation}
        \begin{equation}
        ...
        \begin{equation}
            ...
            \right)^*
            \to   \makeset{ \morsedot,  \morsedash,  \morsedsp,  \morselsp, \morsewsp }^*
        \end{equation}
    \end{forslides}
}%\devel
\begin{exercise}[Morse Morphism]
    ...
    \begin{equation}
        ...
        \setA = (\texttt{A} \ \text{to}\ \texttt{Z}) \setunion (0 \ \text{to}\ 9) \setunion \makeset{\wordsp}
    \end{equation}
    %
    to the alphabet
    %
...
\devel{
    \todotextjira{17}{@Andrea:2: POST-ALPHUBEL Nice example: the map from sequence of characters to sequences of sounds is monoidal in certain languages (Korean, Japanese, almost in Italian.) and also invertible.}
...
\begin{example}
    The set~$\monoidAset = \makeset{ -1, 0, 1 }$, together with multiplication of whole numbers and with~$1$ as neutral element, forms a monoid.
...
\todotextjira{728}{@JL: include examples of monoid isomorphims and discuss how this is a notion of sameness}
...
\todotextjira{21}{POST-ALPHUBEL @JL: Show the inverse operation being compatible with group structure, commuting with \whomos.
...
\todotextjira{729}{@JL: include examples of group isomorphisms}
