...
\begin{solution}
    ...
    The function~$\mapb$ is a left inverse for~$\asciienc$, in the sense that~$\asciienc\then \mapb = \idmon_{\listsof\alphanums}$, but it is not a right inverse.
...
\begin{exercise}[Morse Morphism]
    ...
    \begin{equation}
        (\texttt{A} \ \text{to}\ \texttt{Z}) \setunion (0 \ \text{to}\ 9) \setunion \makeset{\lettersp, \wordsp}
    \end{equation}
