...
\begin{ctdefinition}
    [Monoid action operation]\label{def:monoid-cov-action-prelim}
...
\begin{ctdefinition}[Monoid action map]
    \label{def:monoid-cov-action}
    A \maindef{monoid action} of a \SY{monoid}~$\monoidA$ onto a set~\setA is a \SY{monoid homomorphism}
    \begin{equation}
        \label{eq:act-monoid}
...
For defining a group covariant action, we must adapt the definition of a monoid action.
...
\subsection{Homomorphisms of actions}
