...
\begin{remark}
    Our above definition of a monoid left action operation corresponds to what is often called a ``left action''.
    Such action operations are often denoted using infix notation of the kind
    \begin{equation}
...
There is a way to reformulate the notions of left and right monoid action operations which leads to a compact definition which we will generalize later.
To derive this definition, we'll start with the definition \cref{def:monoid-right-action} of a right action operation, and then apply ``left currying''.
...
To see that this is the case, consider first the second condition in the definition of a right action operation, given by \cref{eq:monoiad-ract-2cond}.
...
Now let's take a look at the first condition in the definition of a right action operation.
...
What about left action operations?

\begin{gradedexercise}[\exname{LeftActionCurry}]
    Show that having a left action operation of a monoid $\monoidA$ on a set $\setA$ is equivalent to having a monoid homomorphism
    \begin{equation}
