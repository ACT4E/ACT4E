...
\begin{definition}[Magma]
 ...
 \todotext{AC: Wow -- this is a really complicated example to start the book with... }
\begin{example}
  \label{ex:cross_prod}
...
\begin{exercise}
  Show that the magma defined in \cref{ex:cross_prod} does not form a semigroup.
\end{exercise}


\begin{example}\label{string-sgrp}
...
\begin{gradedexercise}[Variations on concatenation] 
  \label{ex:alphabet}
...
\todotext{JL: write out (and include graphic) of a simple multiplication table as an example}
...
\todographics{Make a sidefigure showing the transitions. }

\todographics{Use colored symbols for (X, T). }
\begin{example}
...
\todographics{Make a side figure with composition diagrams. 
(We are going to use dynamical systems diagram much more this time around.)}
...
\end{center}
\caption{Composition of discrete-time linear systems. \label{fig:comp_dyn_syst}}
\end{figure}
