...
\begin{example}
Pair-wise average on $\reals$ is not a monoid, because it is not associative.
...
which has the input of the first system and the output of the second system. Let's consider the first system
\begin{equation*}
      \begin{aligned}
        x_{k+1} &= \mat{A} x_k  + \mat{B} u_k \\
        y_{k}   &= \mat{C} x_k  \\
      \end{aligned}
    \end{equation*}
and the second system (which has~$y$ as input)
\begin{equation*}
      \begin{aligned}
      ...
      \end{aligned}
    \end{equation*}
We can write their composition compactly as a discrete linear system with input~$u$ and output~$w$.
\begin{equation*}
\begin{aligned}
\begin{bmatrix}
x_{k+1}\\
