...
\todographics{@Gioele: This figure is a good candidate to be typeset vertically instead. Make vertical marginfigure}
...
\end{marginfigure}
\todo{J: Is it intentional that there is no mention of a start state in the above? These systems are special cases of Moore machines, but for the latter we define them as coming equipped with a start state.}
...
\todo{J: This equality is maybe not formally true on the nose... but maybe yes... i guess we should try to make a more precise definition? If we want to see these as a special case of Moore machines (which seems desireable), then we won't have equality on the nose unless we use the trick AC suggested, using \textbf{SetL}.}
