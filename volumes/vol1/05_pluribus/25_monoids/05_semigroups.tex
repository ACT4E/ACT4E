...
\begin{marginfigure}
    \centering
    \includesag{semigroup_commutative_diagram}
    \caption{Semigroup Commutative Diagram}
    \label{fig:semigroup-commutative-diagram}
\end{marginfigure}
...
\begin{remark}
    Given a set~$\sgrpAset$, there will in general be many choices of composition operation which make~$\sgrpAset$ into a \SY{semigroup}.
...
\todotext{Let's lead first with non-examples}

\begin{example}
    Pairwise average on~\reals,
    \begin{equation}
        \label{eq:pairwise}
...
\todotext{For the 'positive' examples, let's focus on examples that are either especially chosen as important for the semigroup topic or that are examples of a semigroup that is not a monoid. Furthermore, we landed on the idea of loosely organizing some of the examples into different `swimlanes' -- one lane which is basic examples involving math that students are already familiar with; another lane that is characterized by the theme of 'transformations'; and another lane/example here that uses the non-reflexive graph of aquiantance at a party, and considers the semigroup generated by its iterates.}
...
\paragraph{The list construction}

\todotext{Expain briefly that it is a general construction}
...
\paragraph{Endorelations}

\todotext{Expain briefly that it is a general construction? Also: double check what properties an endorelation needs to satisfy to not generate a monoid... (non-reflexive?)}

\todotext{Given example related to the relation 'friend of' at a party}
