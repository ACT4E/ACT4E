...
\begin{example}
    ...
    \end{equation}
    %
    Let~$\sgrpA$ be the set of non-empty strings of elements of~$A$.
    For example,
    %
    \begin{equation}
...
\begin{definition}[Discrete-time linear systems]
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    If~$\mat{x}$ has dimension~$n\geq1$,~$u$ dimension~$m\geq1$ and~$\mat{y}$ dimension~$p\geq1$, then~$\mat{A}$ has dimension~$n \cdot n$,~$\mat{B}$ has dimension~$n \cdot m$, and~$\mat{C}$ has dimension~$p \cdot n$.
\end{definition}
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
\todotext{@Gioele: here and in all the figures the systems $d_1$, $d_2$ should be 
morphisms-colored. Also make a macro for them. I would not use ``d'' because 
it will be the default letter for a DP. Maybe ``S'' for ``system''?}
...
To do so, we need a third system,~$d_3$, with input~$\mat{v}$ and output~$\mat{w}$:
\begin{equation*}
    \label{eq:dlin-4}
    \begin{aligned}
...
\begin{figure}[tbh]
    ...
    \todographics{@Gioele: (polish) Can we add some space around the double lines?}
