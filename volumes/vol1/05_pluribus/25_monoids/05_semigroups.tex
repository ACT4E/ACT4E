...
A \emph{semigroup} is a \SY{magma} for which composition is associative.

\linkvideo{spring2021-semi-mon-gro:mag-semi}
\begin{ctdefinition}[Semigroup]
    \label{def:semigroup}
    A \emph{\iindex{semigroup}}~$\sgrpA$ is defined by:
    \begin{body}
        \constit
        \begin{enumerate}
            \item A set~$\sgrpAset$;
...
\begin{remark}
    ...
    Also note that any semigroup is in particular a \SY{magma}~$\sgrpA = \tup{\sgrpAset, \mtimes}$, and so it also has an `underlying \SY{magma}'.
\end{remark}

\begin{definition}[Commutative semigroup]\label{def:commutative-semigroup}
    A semigroup~$\sgrpA = \tup{\sgrpAset, \mtimes}$ is called \emph{commutative} (or: Abelian) if its underlying \SY{magma} is commutative.
\end{definition}
...
\begin{example}
    ...
    \end{equation}
    It is readily seen that concatenation satisfies the associative law, so~$\sgrpA$, together with this multiplication, forms a \SY{semigroup}.
