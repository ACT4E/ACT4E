...
\begin{example}[Booleans]\label{exa:booleans-table}
  \label{ex:bool_semigroup}
  Consider the set~$\stylesets{B}=\{\false, \true\}$, and~$\tup{\stylesets{B},\wedge}$, where the operation~$\wedge$ is defined via the following composition table:
  \begin{center}
    \begin{tabular}{c|cc}
      $\wedge$ & $\false$ & $\true$  \\
      \hline
...
\begin{gradedexercise}[\exname{CompositionTable}] \label{ex:CompositionTable}
...
\begin{gradedexercise}[\exname{VariationsOnConcatenation}]\label{ex:VariationsOnConcatenation}
  \label{ex:alphabet}
