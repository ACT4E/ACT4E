...
\begin{example}[Orthogonal matrices]
    ...
    Any orthogonal matrix~$\mat{A}$ has the property~$\det(\mat{A})\in \{-1,1\}$. The subset of orthogonal~$n\times n$ matrices with determinant 1 forms the so-called \emph{special} orthogonal group~$\mgSOn$.
\end{example}
