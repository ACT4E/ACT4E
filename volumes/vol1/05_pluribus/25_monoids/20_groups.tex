...
\todotextjira{392}{POST-ALPHUBEL @J: Add more to intro}
...
\begin{example}
    The monoid~$\tup{\stylesets{B},\booland, \true}$ from \cref{ex:bool_monoid} cannot become a group, because there cannot by an inverse for~$\false$.
...
\begin{example}
    \label{exa:grps-order-four}
    Consider the set $\setA = \makeset{ \ela, \elb, \elc, \eld}$. We will show two different ways to equip this set with a group structure. For both we will choose $\ela$ to be the neutral element. 
    
    One way is to define composition and inverses is as in \cref{tab:comp-table-Z4}. This group is called the cyclic group of order four. It appears `in nature' for example if we consider the set $\makeset{0, 1, 2, 3 }$ equipped with addition modulo 4 as composition operation and $0$ as the neutral element. 
    \begin{margintable}
...
Another way is to define composition and inverses as in  \cref{tab:comp-table-Klein4}. This group is know as the Klein four group, named after the mathematician Felix Klein. 
...
\todotextjira{12}{POST-ALPHUBEL @J: Insert examples of groups of order 1, 2, and 3 respectively}



\todotextjira{13}{POST-ALPHUBEL @J: insert example showing multiplication tables of the two possible group structures on a set of four elements}
