...
\begin{gradedexercise}[\str{GroupWithThreeElements}]
...
\begin{gradedexercise}[\str{GroupInverseProperties}]
...
\begin{enumerate}
\item By the inverse laws for~$\ginv$ we have~$\ginv(\idgrp) \mtimes \idgrp = \idgrp$, and by the neutrality laws for~$\idgrp$, the left-hand side of this equation is equal to~$\ginv(\idgrp)$.
Thus we find that~$\ginv(\idgrp) = \idgrp$.
...
\begin{equation*}
  \begin{aligned}
(\ginv(\grpelb) \mtimes \ginv(\grpela)) \then (\grpela \mtimes \grpelb) &= \ginv(\grpelb) \mtimes ( \ginv(\grpela) \then \grpela ) \mtimes \grpelb \\
&= \ginv(\grpelb) \mtimes \idgrp \mtimes \grpelb \\
&= \ginv(\grpelb) \mtimes  \grpelb \\
&= \idgrp.
\end{aligned}
\end{equation*}
