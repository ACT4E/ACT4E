...
\begin{exercise}[Neutral elements of monoids are unique] \\
  Prove \cref{neut-el-unique}.
\end{exercise}
\begin{solution}[Neutral elements of monoids are unique]
...
\begin{example}[Free monoid]
  By extending the structure introduced in \cref{string-semigroup} with the empty string~$\tup{}$ as the neutral element, we get a monoid.
  This often referred to as the \emph{free monoid} on the set~$\sgrpA$.
\end{example}
