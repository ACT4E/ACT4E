...
\todojira{454}{POST-ALPHUBEL: Add more to submonoids subsection}
...
Let $\setA$ be a set.  We denote by $\listsof{\setA}$ set of all lists of elements of $\setA$. 
\end{definition}

\begin{example}
    \label{exa:string-monoid}
For any set $\setA$, the set $\listsof{\setA}$ of lists of elements of $\setA$ can naturally be equipped with a monoid structure: composition is concatenation (just like in \cref{string-sgrp}), and the neutral element is the empty list $\maketypedlist{}{\setA}$. This monoid is known as the \emph{free monoid} on the set $\setA$. 
\end{example}
...
\todotextjira{554}{@Andrea: spell out some simple sufficient constraint (smooth?)}
