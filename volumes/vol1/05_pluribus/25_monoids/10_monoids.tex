...
\begin{ctdefinition}[Monoid]
    \label{def:monoid}
    A \emph{\iindex{monoid}}~$\monoidA$  is given by:
    \begin{body}
        \constit
        \begin{enumerate}
...
\todotext{Split this section in its own file}
...
That is~$\monid_{\reals^n}$, the identity on~$\reals^n$.
To show that~$\stylemaps{T_0} = \monid_{\reals^n}$ is an identity, we can fix any~$\delta$ and substituting in \cref{eq:transition-property} we have
\begin{equation}
    \begin{aligned}
