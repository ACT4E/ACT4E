...
\begin{definition}[Graph homomorphism]
    ...
    Given graphs~$\graph=\tup{\vertices,\arcs,\source,\target}$ and~$\graph'=\tup{\vertices',\arcs',\source',\target'}$, a graph homomorphism~$\funa \colon \graph \fto \graph'$ is given by maps~$\funaob\colon \vertices \fto \vertices'$ and~$\funamor\colon \arcs \fto \arcs'$, such that the following diagrams commute:
    \begin{equation}
