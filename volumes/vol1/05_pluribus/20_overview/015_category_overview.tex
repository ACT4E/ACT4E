...
\begin{marginfigure}
    \centering
    \subfloat[\label{fig:subtractive} Subtractive composition]{
...
\todographics{@Gioele: possible to use the same stripe pattern instead of solid gray?}
...
\begin{widepar}
    \begin{center}
        \includesag{colors-mixing}%{eq:colors-mixing}
    \end{center}
\end{widepar}
...
\begin{widepar}
    \begin{center}
        \includesag{colors-additive}%{eq:colors-additive}
    \end{center}
\end{widepar}
...
\todographics{In principle I would like to treat all the diagrams as equations.
However, for these four: colors-mixing, colors-additive, colors-additive-rgb, colors-subtractive, I cannot do it because they use tabular.}
...
\todographics{@Gioele: We should use different placeholders than dotted boxes - colors too hard to distinguish. Maybe square, circle, rectangle? (with A,B,C noted next to them?)}
...
\todographics{@Gioele: here we are mentioning A, B, C but these are not present in the image}
...
\begin{table*}[p]
    ...
    \todographics{Add some padding to the fbox. Now it's too close to material.}
...
\todographics{@Gioele: weird halo effect here.}
...
%
\equationsag{rope_knot_2}{eq:rope-compose}
%
%
