...
%  \end{equation}
%  is satisfied for all $\sgrpelAa, \sgrpelAb \in \sgrpAset$.
...
%
%\begin{equation}\label{eq:funcomposition-tree}
%\prftree{f: \setA & \to \setB}{g: \setB \to \setC}
%\begin{aligned}
%f \then g : \setA & \to  \setC \\
%            a & \mapsto g(f(a))
%     \end{aligned}
...
\begin{forslides}
%\begin{table}
%\caption{}\label{tab:morse_together}
%\begin{tabular}{rl}
%\mst A&	\morsedot \morsedash\\
%\mst B&	\morsedash \morsedot \morsedot \morsedot\\
%\mst C&	\morsedash \morsedot \morsedash \morsedot\\
...
For example, take the string
    \begin{equation}\label{eq:max-iammax}
    \mst {I} \wordsp \mst{A} \mst{M}\wordsp \mst{MAX}
    \end{equation}
We can decompose it as follows
\begin{equation} \label{eq:max-iammax-decomp}
...
\begin{equation}\label{eq:max-iammax-decomp-morph}
...
\begin{ctdefinition}[Monoid \whomo]
  ...
  A morphism  $\sgrpmorA: \monoidA \fto \monoidB$ between  monoids
  \begin{equation}
