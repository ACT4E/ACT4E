...
\begin{ctdefinition}[Semigroup \whomo]
  ...
  \end{equation}
\end{ctdefinition}
Note that we use~$\sgrpmorA\colon \sgrpAset \to \sgrpBset$ when we want to highlight
the function between sets, and we use~$\sgrpmorA: \sgrpA \fto \sgrpB$
when we want to highlight the relation between semigroup structures. We think of \cref{eq:sgrp-mor-comp} as a way of saying that the function~$\sgrpmorA\colon \sgrpAset \sto \sgrpBset$  is \emph{compatible} with the multiplication operations on~$\sgrpA$ and~$\sgrpB$, respectively.
...
\begin{gradedexercise}[\str{IsoViaTables}]
...
An explicit isomorphism between the first and the third semigroup is the function~$\mapa \colon \setA \sto \setA$ with~$\mapa(\ela) = \elb$ and~$\mapa(\elb)=\ela$.

To see that the first and second semigroup are not isomorphic, suppose there were a function~$\mapa \colon \setA \sto \setA$ which defined such an isomorphism.
In particular, by \cref{ex:semi-morph}, the map~$\mapa$ would necessarily be bijective.
Since~$\mapa$ has only two elements, there the only two bijective functions from~$ \setA \sto \setA$: the identity function, or the function described above that swaps the elements of~$\setA$.
It can be explicitly checked that neither of these maps defines an isomorphism.

For example, if we consider the identity function~$\id \colon \setA \sto \setA$, this is not an isomorphism because for instance
\begin{equation*}
  \begin{aligned}
\id( \ela \mtimes_1 \elb) &= \id (\ela)\\
&= \ela \neq \elb\\
&= \ela \mtimes_2 \elb\\
&= \id( \ela) \mtimes_2 \id (\elb).
\end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
\mapa( \ela \mtimes_1 \ela) &= \mapa (\ela)\\
&= \elb \neq \ela\\
&= \elb \mtimes_2 \elb\\
&= \mapa ( \ela) \mtimes_2 \mapa (\ela).
\end{aligned}
\end{equation*}
\end{solution}}

\begin{gradedexercise}[\str{SemigroupUpToIso}]
...
\instructors{
  \todotext{Write solution of \cref{ex:non-isomorphic} - in the other paper. (JL: not sure what "other paper" means, but this exercise might actually be quite difficult and involved... I haven't solved it yet.)}
}

\begin{gradedexercise}[\str{CharacterizeSemigroupIsos}]
  \label{ex:semi-morph}
...
\begin{gradedexercise}[\str{TraceAndDeterminant}]
  \label{ex:tr-and-det}
