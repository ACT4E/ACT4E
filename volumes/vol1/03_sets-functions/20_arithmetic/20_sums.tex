...
\begin{equation}
\setA \setdisunion \setB = (\makeset{1} \cartprod \setA) \setunion (\makeset{2} \cartprod \setB)
\end{equation}
and
\begin{equation}
\setB \setdisunion \setA = (\makeset{1} \cartprod \setB) \setunion (\makeset{2} \cartprod \setA).
\end{equation}
...
Analogously, we have 
\begin{equation}
    (\setA \setdisunion \setB) \setdisunion \setC \simeq \setA \setdisunion (\setB \setdisunion \setC)
\end{equation}
and this may be proven via a canonical isomorphism
\begin{equation}\label{eq:sets-sum-associator}
    \associator_{\setA, \setB, \setC}\colon (\setA \setdisunion \setB) \setdisunion \setC \to \setA \setdisunion (\setB \setdisunion \setC).
\end{equation}

Because the domain of $\associator_{\setA, \setB, \setC}$ is a two-fold nesting of sums, to define $\associator_{\setA, \setB, \setC}$ we distinguish three sub-cases: 
\begin{equation}\label{eq:sets-sum-associator-def}
    \begin{aligned}
            \associator_{\setA, \setB, \setC} \colon & (\setA \setdisunion \setB) \setdisunion \setC & \to & \ \setA \setdisunion (\setB \setdisunion \setC), & \ \\
            & \tup{1, \tup{1, \ela}} & \mapsto &\  \tup{1, \ela} & \quad \tup{1, \tup{1, \ela}} \setin \makeset{1} \cartprod (\makeset{1} \cartprod \setA) \\
            & \tup{1, \tup{2, \elb}} & \mapsto &\  \tup{2, \tup{1, \elb}} & \quad \tup{1, \tup{2, \elb}} \setin \makeset{1} \cartprod (\makeset{2} \cartprod \setB) \\
            & \tup{2, \elc} & \mapsto &\  \tup{2, \tup{2, \elc}} & \quad \tup{2, \elc} \setin \makeset{2} \cartprod \setC \\
    \end{aligned}
\end{equation}

\begin{exercise}\label{ex:associator-cart-prod-and-disjoint-union}
    Write down an inverse function to $\associator_{\setA, \setB, \setC}$, and argue why it is indeed an inverse. 
\end{exercise}

\begin{solution}
    The inverse function simply re-nests the labels in the other direction:
    \begin{equation}\label{eq:sets-sum-associator-inverse-def}
    \begin{aligned}
            \associator^{-1}_{\setA, \setB, \setC} \colon & \setA \setdisunion (\setB \setdisunion \setC) & \to & \ (\setA \setdisunion \setB) \setdisunion \setC, & \ \\
            & \tup{1, \ela} & \mapsto &\  \tup{1, \tup{1, \ela}} & \ \tup{1, \ela} \setin \makeset{1} \cartprod \setA \\
            & \tup{2, \tup{1, \elb}} & \mapsto &\  \tup{1, \tup{2, \elb}} & \ \tup{2, \tup{1, \elb}} \setin \makeset{2} \cartprod (\makeset{1} \cartprod \setB) \\
            & \tup{2, \tup{2, \elc}} & \mapsto &\  \tup{2, \elc} & \ \tup{2, \tup{2, \elc}} \setin \makeset{2} \cartprod (\makeset{2} \cartprod \setC) \\
    \end{aligned}
    \end{equation}
    Comparing with \cref{eq:sets-sum-associator-def}, we see that the three cases of $\associator^{-1}_{\setA, \setB, \setC}$ are exactly the three cases of $\associator_{\setA, \setB, \setC}$ read from right to left. Hence applying $\associator_{\setA, \setB, \setC}$ and then $\associator^{-1}_{\setA, \setB, \setC}$ returns every element of $(\setA \setdisunion \setB) \setdisunion \setC$ to itself, and likewise in the other order, so that
    \begin{equation}
        \associator_{\setA, \setB, \setC} \mthen \associator^{-1}_{\setA, \setB, \setC} = \catidat{(\setA \setdisunion \setB) \setdisunion \setC}
        \qqand
        \associator^{-1}_{\setA, \setB, \setC} \mthen \associator_{\setA, \setB, \setC} = \catidat{\setA \setdisunion (\setB \setdisunion \setC)}.
    \end{equation}
    In particular, $\associator_{\setA, \setB, \setC}$ is thus an isomorphism.
\end{solution}
...
\end{equation}
which we call \emph{left unitor} and \emph{right unitor}, respectively.

\subsection{From sums to products}

In arithmetic, if we sum a number $x$ with itself some finite number  of times, then 
\begin{equation}
x + \dots + x = n \cdot x,
\end{equation}
where $n$ is the number of summands in the left-hand sum. 

Analogously, if we sum a set $\setA$ with itself some finite number of times, we have
\begin{equation}
\setA \setdisunion \cdots \setdisunion \setA \simeq \stylesets{N} \cartprod \setA,
\end{equation}
where $\stylesets{N}$ denotes here the set $\makeset{1, \dots ,n}$, with $n \setin \natnumbers$ being the number of summands. 

For example, for $\stylesets{2} = \makeset{1,2}$ we have 
\begin{equation}
\setA \setdisunion \setA \simeq \stylesets{2} \cartprod \setA
\end{equation}
and for $\stylesets{3} = \makeset{1,2, 3}$ we have 
\begin{equation}
\setA \setdisunion \setA \setdisunion \setA \simeq \stylesets{3} \cartprod \setA
\end{equation}
and so on. 

By \cref{def:n-ary-disjoint-union-of-sets}, for the special case $n =1$, we also have that the one-fold sum of $\setA$ is isomorphic to $\singleton \cartprod \setA$ for $\singleton = \makeset{1}$ (however, there is no infix notation for a one-fold sum).
