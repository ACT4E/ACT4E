...
\newcommand{\dpisp}{\ensuremarkath{\vmath{DPI}}}
\newcommand{\cdpisp}{\ensuremarkath{\vmath{CDPI}}}

\newcommand{\dprobsp}{\ensuremarkath{\vmath{DP}}}
...
\newcommand{\cdprobsp}{\ensuremarkath{\vmath{CDP}}}
...
\paragraph{Robotic systems as the prototype of complex systems}
...
is needed. Extremarkely interesting trade-offs arise when considering
...
\paragraph{Contribution: A Principled Theory of Co-Design}
...
\paragraph{Chains and antichains}
...
\end{equation}
\end{lemma}
Note in (\cref{eq:up_order}) the use of~``$\supseteq$'' instead
...
using the order on the upper sets~(\cref{antichains_upsets}).
\begin{lemma}
\label{lemma:antichains-are-poset}$\antichains\posA$ is a poset with
...
\paragraph{Monotonicity and fixed points\label{sec:Monotonicity-and-fixed}}
...
(\DCPO) if each of its directed subsets has a supremarkum (least of
...
(\cref{ceil}).
\end{remark}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_ceil.pdf}
...
\label{lemma:CPO-fix-point-2}If~$\posA$ is a \CPO and~$f:\posA\rightarrow\posA$
...
\label{lemma:kleene-1}Assume $\posA$ is a \CPO, and~$f:\posA\rightarrow\posA$
is \scottcontinuous. Then the least fixed point of~$f$ is the supremarkum
...
between these three spaces~(\cref{setup}).
\begin{definition}
...
\end{itemize}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_setup.pdf}
\label{setup}
\end{definition}
...
to each motor the resources it needs~(\cref{motor}).
\end{example}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_motor_evalexec.pdf}
\begin{example}[Chassis design]
\label{exa:chassis}Suppose we need to choose a chassis for a robot~(\cref{gmcdp_chassis_eval}).
...
functional requiremarkents would include maneuverability, the cargo volume,
...
\end{example}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_chassis_eval.pdf}
...
are the implementations that have minimal resources usage?''~(\cref{setup-1}).
\begin{problem}
\label{prob:problem1}
Given~$\funn \in \funsp$, find the implementations
...
\end{equation}
\end{problem}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_setup_query_f.pdf}
...
the resources usage, and then maximize the functionality provided~(\cref{setup_max_f}).
The formulation is entirely dual, in the sense that it is obtained
from \cref{objective} by swapping~$\Min$ with~$\Max$, $\funsp$~with~$\ressp$,
and $\exc$~with~$\eval$.
\begin{equation}
\begin{cases}
\with & \imp\in\impsp,\\
...
\end{equation}
\end{remark}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_setup_query_r.pdf}
...
to each functionality~$\funn$ the objective function of~\cref{problem1},
...
\end{definition}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_setup_h.pdf}
...
the achievable trade-off of \R{cost}, \R{mass}, and other resources~(\cref{motor-trade-offs}).
The antichains are depicted as continuous curves, but they could also
be composed by a finite set of points.

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_motor_tradeoffs.pdf}
\end{example}

By construction, $\ftor$ is monotone (\cref{monotone}), which
means that
\begin{equation*}
...
\cref{antichains-are-poset}. Monotonicity of~$\ftor$ means that
...
\subsubsection{Co-design problems}
...
red edges~(\cref{dp_graphical}).

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_dp_graphical.pdf}
...
in a rounded box~(\cref{connection}). 
...
and resources to be connected, and it holds that~$\pi_{\cdpiresindA}\ressp_{\cdpinA}=\pi_{\cdpifunindB}\funsp_{\cdpinB}$~(\cref{mcdps}). 

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi.pdf}
\end{definition}
...
of the functionality and resources of the nodes that remarkain unconnected.
...
\begin{example}
\label{exa:chassis_plus_motor}Consider the co-design of chassis (\cref{chassis})
plus motor (\cref{motor}). The design problem for a motor has \F{speed}
and \F{torque} as the provided functionality (what the motor must
provide), and \R{cost}, \R{mass}, \R{voltage}, and \R{current}
as the required resources~(\cref{motor}).

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_motor.pdf}

\noindent For the chassis (\cref{gmcdp_chassis}), the provided
...
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_chassis.pdf}

\noindent The two design problems can be connected at the edges for
torque and speed~(\cref{gmcdp_chassis_plus_motor_series}). The
semantics is that the motor needs to have\emph{ at least }the given
torque and speed. 

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_chassis_plus_motor_series.pdf}
...
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_weightsum.pdf}
...
graph~(\cref{gmcdp_chassis_plus_motor}). 

\noindent 
\begin{figure}[h]
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_chassis_plus_motor.pdf}\caption{\label{fig:gmcdp_chassis_plus_motor}}
\end{figure}

\noindent This formalism makes it easy to abstract away the details
in which we are not interested. Once a diagram like~\cref{gmcdp_chassis_plus_motor}
is obtained, we can draw a box around it and consider the abstracted
problem~(\cref{gmcdp_chassis_plus_motor-1}). 

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_chassis_plus_motor2.pdf}

\label{exa:finish}Let us finish assembling our robot. A motor needs
a motor control board. The functional requiremarkents are the (peak)
\F{output current} and the \F{output voltage range}~(\cref{mcb}).

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_mcb.pdf}
...
The resources could include \R{cost} and \R{mass} (\cref{example-ba}).

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_battery.pdf}
...
can be modeled by a trivial ``multiplication'' DPI (\cref{current_times_voltage}).

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_voltage_current.pdf}
...
\R{mass} and \R{cost}~(\cref{connect}).

\includegraphics[scale=0.29]{papers/arxiv_submission_v6/gmcdp_MCB_PSU_2.pdf}

\noindent Draw a box around the diagram, and call it ``MCB+PSU'';
then interconnect it with the ``chassis+motor'' diagram in~\cref{another}.

\noindent 
\begin{figure}[h]
\begin{centering}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_mobility_power.pdf}
\par\end{centering}
\caption{\label{fig:another}}
\end{figure}

We can further abstract away the diagram in~\cref{another} as
a ``mobility+power'' CDPI, as in \cref{shipping}. The formalism
...
\noindent 
\begin{figure}[h]
\centering{}\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_shipping.pdf}\caption{\label{fig:shipping}}
\end{figure}
...
\end{example}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/batteries_svorenova.pdf}
...
evaluated.
\end{example}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/batteries_zia.pdf}
...
plus other non-physical resources, such as \R{prior knowledge}~(\cref{robot-generic}).
...
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/batteries_okane.pdf}
...
for the entire CDPI (\cref{question}). 

\includegraphics[width=7cm]{papers/arxiv_submission_v6/gmcdptro_question.pdf}

The rest of the paper will provide a solution to~\cref{MCDP},
under the assumption that all the DPIs inside the CDPI are ``monotone'',
in the sense of~\cref{DPI-monotone}.
\begin{definition}
\label{def:DPI-monotone}A DPI $\left\langle \funsp,\ressp,\impsp,\exc,\eval\right\rangle $
is ``monotone\emph{''} if
\begin{enumerate}
\item The posets $\funsp,\ressp$ are complete partial orders (\cref{cpo}).
\item The map~$\ftor$ is \scottcontinuous (\cref{scott}).
\end{enumerate}
\end{definition}


Call ``Monotone Co-Design problems'' (MCDPs) the set of CDPIs for
which all subproblems respect the conditions in~\cref{DPI-monotone}.
 I will show two main results:

1) A \textbf{modeling result} (Theorem~\cref{thm:CDP-monotone})
says that the class of MCDPs is closed with respect to arbitrary interconnections.
Therefore, given a co-design diagram, such as the one in~\cref{shipping},
if we know that each design problem is an MCDP, we can conclude that
the diagram represents an MCDP as well.

2) An \textbf{algorithmic result }(Theorem~\cref{thm:CDP-solvig})
...
\cref{threeoperators} defines these composition operators. \cref{Decomposition}~shows
...
\begin{figure}[h]
\hfill{}\subfloat[\label{fig:Simple-DP}]{\centering{}\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_nonconvex1b.pdf}}\hfill{}\subfloat[\label{fig:nonconvex3}]{\centering{}\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_nonconvex3.pdf}}\hfill{}
...
\begin{example}
\label{exa:one}Consider the CDPI in \cref{Simple-DP}. The \underline{m}inimal
...
\end{cases}
\end{equation*}
The \underline{fea}sible set~$\Phi\subseteq\funsp\times\ressp$ is
...
\end{equation}
The \underline{p}rojection~$P$ of~$\Phi$ to the functionality space
is:
\begin{equation*}
...
\end{equation}
\cref{nonconvex3} shows an example of a continuous map~$\ftor$
...
problems. Later, \cref{Decomposing2} will prove that any co-design
...
\end{eqnarray*}
\end{definition}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_series3.pdf}
...
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_parallel2.pdf}
\end{definition}
...
the constraint~$\funn_{2}\posgeq\res$~(\cref{sloop}).
\end{definition}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_sloop2.pdf}
...
feedback operator $\dploopb$ as in \cref{loop_general}, which
can be rewritten in terms of $\dploop$, using the construction in~\cref{loop_general2}\emph{.}
...
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdp_coproduct.pdf}
\end{definition}
...
\end{proposition}
\begin{proof}
We show this constructively. We will temporarily remarkove all cycles
...
that, when remarkoved, remarkove all cycles from the graph (see, e.g.,~\cite{golovach15incremarkental}).
For example, the CDPI represented in~\cref{cdpi_comp1} has a minimal
AFS that contains the edge~$\text{c}\rightarrow\text{a}$~(\cref{cdpi_comp2}). 

\begin{figure}[h]
\subfloat[\label{fig:cdpi_comp1}]{\centering{}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi_comp1.pdf}}
\hfill{}\subfloat[\label{fig:cdpi_comp2}]{\centering{}\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi_comp2.pdf}}\hfill{}\subfloat[\label{fig:cdpi_comp2b}]{\begin{centering}
...
remarkove the~AFS~$F$ from~$\mathcal{E}$ to obtain the reduced edge
...
are three subgraphs (\cref{cdpi_comp3}). 

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi_comp3.pdf}
...
of node~$\text{b}$ and the identity $\triv(\idFunc)$ (\cref{cdpi_comp4}).

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi_comp4.pdf}
...
instance of the~$\dploopb$ operator is sufficient~(\cref{cdpi_comp5}).
In this example, the tree representation (\cref{cdpi_comp6}) is
\begin{equation*}
\dploopb(\dpseries(\dpseries(\text{a},\dppar(\idFunc,\text{b})),\text{c}).
\end{equation*}

\begin{figure}[h]
\subfloat[\label{fig:cdpi_comp5}]{\centering{}\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi_comp5.pdf}}\hfill{}\subfloat[\label{fig:cdpi_comp6}]{\centering{}\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_cdpi_comp6.pdf}}\hfill{}

\caption{Tree representation for the co-design diagram in~\cref{cdpi_comp1}.}
\end{figure}
...
like the one in~\cref{nested1} can be rewritten as~\cref{nested2}.
...
\begin{proof}
\cref{reduction} has shown that any interconnection of design
...
in the sense of~\cref{DPI-monotone} is preserved by each operator
separately. This is done below in~\cref{dppar-monotone}\textendash \cref{pro:loop-continuous}. 
\end{proof}

\begin{proposition}
\label{pro:dppar-monotone}If~$\dprob_{1}$ and~$\dprob_{2}$ are
monotone (\cref{DPI-monotone}), then also the composition~$\dppar(\dprob_{1},\dprob_{2})$
is monotone.
\end{proposition}
\begin{proof}
We need to refer to the definition of $\dppar$ in \cref{parallel}
and check the conditions in \cref{DPI-monotone}. If~$\funsp_{1},\funsp_{2},\ressp_{1},\ressp_{2}$
are CPOs, then~$\funsp_{1}\times\funsp_{2}$ and~$\ressp_{1}\times\ressp_{2}$
are CPOs as well.

From \cref{ftor} and (\cref{eq:dppar-exec}) we know $\ftor$ can
be written as
\begin{align*}
...
are~\cite[lemmama II.2.8]{gierz03continuous}.
\end{proof}

\begin{proposition}
\label{pro:series-monotone}If $\dprob_{1}$ and $\dprob_{2}$ are
monotone (\cref{DPI-monotone}), then also the composition $\dpseries(\dprob_{1},\dprob_{2})$
is monotone.
\end{proposition}
\begin{proof}
From the definition of $\dpseries$ (\cref{series-composition}),
...
\end{equation}
The situation is described by \cref{series_mono1-2}. The point~$\funn_{1}$
...
\includegraphics[scale=0.45]{papers/arxiv_submission_v6/gmcdp_monotone_proof_mono1.pdf}
...
\begin{proposition}
\label{pro:loop-continuous}If $\dprob$ is monotone (\cref{DPI-monotone}),
so is~$\dploop(\dprob)$.
\end{proposition}
\begin{proof}
The diagram in \cref{sloop} implies that the map~$\ftor_{\dploop(\dprob)}$
can be described as:
\begin{align}
...
\end{equation*}
Rewrite $\res\in\ftor_{\dprob}(\funn_{1},\funn_{2})$ in \cref{loopproblem}
as
\begin{equation}
\res\in\ftor_{\funn_{1}}(\funn_{2}).\label{eq:h2}
\end{equation}
Let~$\res$ be a feasible solution, but not necessarily minimal.
Because of \cref{antichain-write}, the constraint \cref{h2} can
be rewritten as 
\begin{equation}
...
Therefore, by \cref{antichain_inter}, we have
\begin{equation}
...
\cref{antichain_union})
\begin{equation}
\R{R}\posgeq_{\Aressp}\Min_{\resleq}\bigcup_{\res\in\R{R}}\ftor_{\funn_{1}}(\res)\ \cap\ \upit\res.\label{eq:recursive}
\end{equation}

{[}Converse: It is also true that if an antichain~$\R{R}$ satisfies~\cref{recursive}
then all~$\res\in\R{R}$ are feasible. The constraint \cref{recursive}
...
rewrite~\cref{conc} by using~$\res_{0}$ in place of~$\res_{1}$
...
satisfy~\cref{recursive}, and conversely, if an antichain~$\R{R}$
satisfies~\cref{recursive}, then it is an antichain of feasible
resources. 

Equation \cref{recursive} is a recursive constraint for~$\R{R}$,
of the kind 
\begin{equation*}
...
\end{equation}
\cref{dagger} shows that $\lfp(\Phi_{\funn_{1}})$ is \scottcontinuous
in~$\funn_{1}$. 
\end{proof}

\begin{lemma}
\label{lemma:antichain-write}Let~$A$ be an antichain in $\posA$.
Then
\begin{equation*}
...
\begin{lemma}
\label{lemma:antichain_inter}For $A,B\in\antichains\posA$, and $S\subseteq P$,
$A\posleq_{\Aressp}B$ implies $A\cap S\posleq_{\Aressp}B\cap S$. 
\end{lemma}

\begin{lemma}
\label{lemma:antichain_union}For $A,B,C,D\in\antichains\posA$, $A\posleq_{\Aressp}C$
and $B\posleq_{\Aressp}D$ implies $A\cup B\posleq_{\Aressp}C\cup D.$
\end{lemma}

\begin{lemma}
\label{lemma:dagger}Let~$f\colon\posA\times\posB\rightarrow\posB$
...
recursively using the rules in \cref{Correspondence}.
...
\begin{proof}
These expressions were derived in the proofs of~\cref{dppar-monotone}\textendash \cref{pro:loop-continuous}.
 The operators $\opseries,\oppar,\oploop,\opcoprod$ are defined
in \cref{opseries}\textendash \cref{def:opcoprod}. 
\end{proof}
...
\begin{figure}[h]
\subfloat[\label{fig:toydiagram}]{\centering{}\includegraphics[width=6.2cm]{papers/arxiv_submission_v6/gmcdptro_example_diagram.pdf}}\subfloat[\label{fig:toydiagram_tree}]{\begin{centering}
\includegraphics[scale=0.33]{papers/arxiv_submission_v6/gmcdptro_example_diagram_tree.pdf}
\par\end{centering}
}

\caption{Co-design diagram equivalent to \cref{example-1} and its tree representation.}
\end{figure}
...
\end{eqnarray*}
The tree decomposition (\cref{toydiagram_tree}) corresponds to
the expression
\begin{equation}
\dprob=\dploop(\dpseries(\dppar(\dprob_{2},\dprob_{2}),\dpseries(\dprob_{1},\dprob_{3}))).\label{eq:expression}
\end{equation}
Consulting \cref{Correspondence}, from~(\cref{eq:expression})
...
for~$\ftor$. By substituting in~(\cref{eq:h}) the definitions given
in~\cref{opseries}\textendash \cref{def:oploop}, we obtain that
...
\par\end{centering}
\caption{\label{fig:example24}Kleene ascent to solve the problem \cref{example-1}
...
\end{align*}
\cref{example24}~shows the sequence for $\F{c}=20$.
...
set~$\ressp\ \backslash\upit\R{R}_{k}$,) are infeasible. (In \cref{example24},
...
because there might be holes in the feasible set, as in \cref{one}.
...
\cref{complexity} above assumes that the MCDP is already in the
...
to the procedure in~\cref{Decomposition}, we need to find an arc
...
Of the three performance measures in~\cref{complexity}, the most
...
\begin{centering}
\medskip{}
\subfloat[\label{fig:battery_nicad}MCDPL code equivalent to equations (\cref{eq:mass})\textendash (\cref{eq:cost}).]{\begin{centering}
...
The choice of a battery can be modeled as a DPI (\cref{battery})
...
\cref{battery_nicad} shows the MCDPL code that describes the
model corresponding to (\cref{eq:mass})\textendash (\cref{eq:cost}).
The diagram in \cref{Co-design-diagram} is automatically generated
from the code. \cref{Tree-representation-of}~shows a tree representation
...
Each row of the table is used to describe a model as in \cref{battery_nicad}
by plugging in the specific values in lines 12\textendash 14.

Given the different models, we can defined their co-product (\cref{Co-product-of-battery})
using the MCDPL code in~\cref{batteries_code}. 
...
This semantics can be described by the co-design diagram in~\cref{diagram},
...
The MCDPL code is shown in~\cref{diagram_code}. Note the intuitive
...
of MCDPs. \cref{mainbattery} shows the optimal choice of the battery
...
there are. Some of the results are intuitive. For example, \cref{min_mass}
...
objectives. For example, \cref{min_cost} shows the case where the
objective is to minimize the \R{cost}, which, defined by~\cref{cost},
...
$\left\langle \R{\text{mass}},\R{\text{cost}}\right\rangle $~(\cref{min_mass_cost})
or~$\left\langle \R{\text{mass}},\R{\text{cost}},\R{\text{maintenance}}\right\rangle $
(\cref{min_mass_cost}), there are multiple non-dominated solutions.

\begin{figure}
\begin{centering}
\subfloat[\label{fig:drone}\label{fig:drone_dia}Co-design diagram corresponding
to (\cref{eq:drone_eq_first})\textendash (\cref{eq:drone_eq_last}).]{\centering{}\includegraphics[scale=0.38]{papers/arxiv_submission_v6/batteries_overall.pdf}}
\par\end{centering}
\begin{centering}
\subfloat[\label{fig:drone_code}MCDPL code for (\cref{eq:drone_eq_first})\textendash (\cref{eq:drone_eq_last}).
The ``instance'' statements refer to previously defined models for
batteries (\cref{batteries_code}) and actuation (not shown). ]{\begin{centering}
\begin{minipage}[t]{8.6cm}
\begin{center}
...
for a drone~(\cref{drone_dia}). We will see that the decision
...
The functionality of the drone's subsystem considered~(\cref{drone})
...
a quadratic function of lift~(\cref{actuation}). Any other monotone
map could be used.

\includegraphics[scale=0.33]{papers/arxiv_submission_v6/batteries_actuation_dp.pdf}
...
in \cref{drone_code}; it refers to the previously defined models
...
Looking at~\cref{drone}, we know that there is a monotone relation
...
110 nodes and 110 edges. It is possible to remarkove all cycles by remarkoving
only one edge (e.g., the $\R{\text{energy}}\leq\text{\F{\text{capacity}}}$
constraint), so the design complexity (\cref{design-complexity})
is equal to~$\posetwidth(\R{\overline{\mathbb{R}}_{+}})=1$. The
tree representation is shown in~\cref{drone_tree}. Because the
...
\end{equation*}
we need to project across 2D slices. \cref{drone-endurance-missions}~shows
the relation when the functionality varies in a chain in the space
\F{endurance}/\F{missions}, and~\cref{drone-endurance-payload}
shows the results for a chain in the space \F{endurance}/\F{payload}.

Finally, \cref{drone_choice} shows the optimal choices of battery
...
The decision boundaries are completely different from those in~\cref{mainbattery}.
...
defined as the co-product of all battery technologies (\cref{batteriesbig}).
...
actuation-energetics model described in \cref{dronebigfig}. Please
see the caption of~\cref{mainbattery} for an explanation of the
symbols. Notice how in most cases the decision boundaries are different
than those in~\cref{mainbattery}: this is an example in which
...
``axiom'' is to keep the design requiremarkents orthogonal (i.e., do
...
optimizing the rate of convergence. The order of evaluation in~\cref{ai}
