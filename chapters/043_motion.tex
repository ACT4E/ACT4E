...
\AC{Why not use $\otimes$ from the start as the monoid operation?}
\begin{definition}[Monoid]
A \index{\emph{monoid}} is a set~$M$ with a binary operation~$(*)\colon M\times M\to M$, and a \emph{neutral element} $1\in M$, which satisfy:
\begin{enumerate}
