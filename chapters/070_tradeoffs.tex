...
\begin{definition}[Partially ordered set]
  \label{def:poset}
  A \emph{partially-ordered set (\iindex{poset})} is a tuple~$\tup{P,\ordleq}$,
...
A \emph{\iindex{Hasse diagram}} is an economical (in terms of arrows) way to visualize a poset. In a Hasse diagram elements are points, and if~$\obja \ordleq \objb$ then~$\obja$ is drawn lower than~$\objb$ and with an ege connected to it, if no other point is in between. Hasse diagrams are directed graphs.
...
\begin{definition}[Preorder]
  \label{def:preorder}
  A \emph{\iindex{preorder}} is a tuple~$\tup{P, \leq}$,
  where~$P$ is a set (also called the \emph{carrier set}), together with a
...
\begin{example}[Credit and debt]
  ...
  \begin{equation*}
    \USD=\{\$0.00,\$0.01,\$0.02,\dots\}
  \end{equation*}
  of all US dollars monetary quantities approximated to the cent.
  From this set we can define two posets:~$\USD^{+} = \tup{\USD, \ordleq}$ and~$\USD^{-} = \tup{\USD, \ordgeq}$, that are the opposite of each other.
  If the context is that, given two quantities~$\$1$ and~$\$2$, we prefer~$\$1$ to~$\$2$ (for example because it is a cost to pay to acquire a component), then we are working in~$\USD^{+}$, otherwise we are working in~$\USD^{-}$ (for example because it represents the price at which we are selling our product).
...
An interval is an ordered pair of elements~$\tup{l,u}$ of~$P$, such that~$l\ordleq_P u$. Given a poset~$P$, one can define a \emph{poset of intervals} on~$P$. Intervals can be ordered by inclusion, e.g.:
  ...
  \end{equation*}
\end{definition}
\todo{put here the definition of poset of intervals}
