...
The category of sets \Set is defined by:
    ...
    \item \emph{Identity morphism}: given a set~$\obja$, its identity morphism~$\text{id}_\obja$ is the identity function~$\obja \to \obja, \ \text{id}_\obja(x) = x$.
...
\begin{example}
	...
	\item For each~$\obja \in \Ob_{\Set}$, the identity relation~$\id_\obja=\{\tup{x,x'}\in \obja\times \obja \mid x=x'\}$ corresponds to the identity function~$\id_\obja \colon \obja \to \obja$ in \Set.
