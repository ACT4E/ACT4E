...
\todo{Add figure using the graphical notations for tensor operations}
...
\paragraph{Morphisms} A morphism in \LTI is an arrow $l\to m$, for $l,m\in \mathbb{N}_0$. The arrow describes the transformation of an input sequence $u\colon T\to \reals^l$ into an output sequence $y\colon T\to \reals^m$. Such an arrow is given by a continuous time LTI system of the form
\begin{equation}
  \begin{aligned}
...
The identity morphism for $l\in \Ob(\LTI)$ is an arrow $l\to l$, parametrized by the system $\tup{0,0^{1\times l},0^{l\times 1},\mathbb{I}^{l\times l}}$.
