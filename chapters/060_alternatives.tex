...
\end{comment}
Recall that given sets $A$ and $B$, their disjoint union $A + B$ is a set that contains a distinct copy of $A$ and $B$ each. If an element is contained in both $A$ and $B$, then there will be two distinct copies of it in the disjoint union $A + B$.

\begin{definition}[Disjoint union of sets]
\label{def:disjoint-union}
    The \emph{disjoint union} (or sum) of two sets~$A$ and~$B$ is 
    \begin{equation}
...
Let~$\CatC$ be a category and let~$A, B \in \Ob_{\CatC}$ be objects. The \emph{coproduct} of~$A$ and~$B$ consists of the following constituent data, satisfying the following condition. \\
\underline{Data:}
\begin{compactenum}
...
\end{compactenum}
\underline{Condition:} \\
For any~$X \in \Ob_{\CatC}$ and any morphisms $f\colon  A \to X, g \colon B \to X$, there exists a \emph{unique} morphism~$(\coprodMap{f}{g}) \colon \coprodMap{A}{B} \to X$ such that~$f = \iota_A\then (\coprodMap{f}{g})$ and~$g = \iota_B \then (\coprodMap{f}{g})$. 
\end{ctdefinition} 
