...
When we talk about computing minimal sets of resources, we always mean computing an appropriate antichain, i.e. an element in $\antichains \posA$.
...
} \JT{They're not equivalent but on upper sets they're very close. Assume my definition, and let $\Min S = \O$, then $S = \O$, which does \emph{not} satisfy $S = \upit \Min S$. On the other hand, suppose $\Min S = K \neq \O$ for some set $K$. Then $S \neq \O$ trivially, and it remains to determine $\upit K$. But since $S$ is an upper set, every $s \in S$ satisfies $s \geq k$ for some $k \in K$, so $s \in \upit K$; on the other hand this is the same definition of a typical element $k \in \upit K$.
...
\begin{proof}
    ...
    Pick $A \in \antichains\posA$. Then $A$ generates an upper set $\upit A = \{ x \in P : a \leq x \text{ for some $a \in A$} \}$, and $\Min \upit A = A$. From the other direction, consider $\dcuppersets \posA$, the set of upper sets of $P$ which are downward-closed. Pick a non-empty $U \in \dcuppersets \posA$. Since $U$ is downward-closed, $\Min U$ is a non-empty antichain given by $\{ a \in \underline{U} : a \leq x \text{ for all $x \in \underline{U}$} \}$, and $\upit \Min \underline{U} = \underline{U}$.
\end{proof}
...
% Weaker version: If a design problem $\adp : F \tickar R$ is downward-closed, and $(f^*,r)$ is a minimal element of $K_\adp$, then $h_\adp$ is Scott-continuous at $f$, i.e. if $f = D^\upit$ is the supremum of some directed subset $D \subset F$, then $h_\adp(f) = h_\adp(D)^\upit$.
\end{lemma}
