...
\todo{make a command for the identity. Also note later we use I}

\begin{definition}[Monoid]\label{def:monoid}
    A \index{\emph{monoid}} is a set~$M$ with a binary operation~$\otimes \colon M\times M\to M$, and a \emph{neutral element}~$1\in M$, which satisfy:
    \begin{enumerate}
