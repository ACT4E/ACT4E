...
\begin{ctdefinition}[Functor]
  \label{def:functor}
  Given categories~\CatC and~\CatD, a \index{\emph{functor}}~$F\colon \CatC\to \CatD$ from~\CatC to ~\CatD is defined by the following data, satisfying the following conditions. \\
\underline{Data:}
  \begin{compactenum}[i)]
...
\begin{remark}
  A functor from a category to itself it is called an \index{\emph{endofunctor}}.
\end{remark}
