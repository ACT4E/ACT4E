...
\section{Products}\label{sec:combination-products}

We'll start off by recalling a familiar way of combining two sets, $\Obja$ and $\Objb$.


\begin{definition}[Cartesian product of sets]\label{def:cartesian-product}
    Given two sets~$\Obja,\Objb$, their \emph{\iindex{cartesian product}} is denoted~$\Obja \times  \Objb$ and defined as
    \begin{equation*}
...
Alternatively, snacks could be pre-packaged in such a way as to allow all possible combinations of food and drink choices. This corresponds to~$X\times Y$.
