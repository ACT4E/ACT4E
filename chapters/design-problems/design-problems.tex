...
\begin{example}
    ...
    \end{equation}
    We now have to think about \R{resources} and \F{functionalities}. Each motor \R{requires} some \R{weight} (in \unit[]{g}), \R{power} (in \unit[]{W}), and has some \R{cost} (in \stdcurr), and \F{provides} some \F{torque} (in~$\unit[]{kg\cdot cm}$). Thus, we can identify
    \begin{equation*}
    ...
    \end{equation}
    since no listed model can provide~$\unit[0.2]{kg\cdot cm}$ \F{torque} by requiring the set of \R{resources} $\tup{\unit[50.0]{g},\unit[2.0]{W},\unit[15.0]{\stdcurr}}$ or less.
...
\begin{example}[Motor design]
    ...
    could include the \R{cost {[}\stdcurr{]}}, the \R{mass {[}g{]}}, the
...
\end{figure}
\todographics{Redo figure with consistent style}
...
\todotext[inline]{Write short summary}
...
\node[dp={1}{5}] (dc) {\begin{tabular}{c} ``robotics''\end{tabular}};
\draw[runconn, runame={\unit[sensing resources]{}}] (dc_res1){};
\draw[runconn, runame={\unit[actuation resources]{}}] (dc_res2){};
\draw[runconn, runame={\unit[computation resources]{}}] (dc_res3){};
\draw[runconn, runame={\unit[prior knowledge]{}}] (dc_res4){};
\draw[runconn, runame={\unit[...]{}}] (dc_res5){};
\draw[funconn, funame={\unit[tasks]{}}] (dc_fun1){};
\end{tikzpicture}
