...
The visualization in Figure \ref{fig:example_rel} hints at the fact that we can think of a relation $R \subseteq X \times Y$ as a \emph{morphism} from $X$ to $Y$.
...
\begin{center}
\begin{tikzpicture}
\draw (-1.35,0) ellipse (0.75cm and 1.7cm);
\draw (1.35,0) ellipse (0.75cm and 1.7cm);
\node at (0,0) {
    \begin{tikzcd}[column sep=2cm,row sep=0.05]
    x_1&\\
    &z_1\\
    x_2\arrow{ur}\arrow{dr}&\\
    &z_2\\
    x_3&
    \end{tikzcd}};
\end{tikzpicture}
...
\item $\gamma : [0, T] \rightarrow L$ is a $C^1$ function with $\dot \gamma(0) = v_1$ and $\dot \gamma(T) = v_2$  (we take one-sided derivatives at the boundaries). 
\end{itemize}
...
\item Given morphisms $\tup{\gamma_1, T_1}: \tup{p_1, v_1} \rightarrow \tup{p_2, v_2}$ and $\tup{\gamma_2, T_2}: \tup{p_2, v_2} \rightarrow \tup{p_3, v_3}$, their composition is $\tup{\gamma, T}$ with $T = T_1 + T_2$ and
\begin{equation}
 \gamma(t) = \left \{ 
 \begin{array}{ccc}
