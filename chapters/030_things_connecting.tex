...
\begin{ctdefinition}[Category \Rel]
  The category \index{\Rel} of relations is given by:
  \begin{compactenum}
...
\begin{definition}
  ...
  Let~$\Obja$ and~$\Objb$ be sets. A \index{relation}~$R \subseteq \Obja \times \Objb$ is a \textbf{function} if it satisfies the following two conditions:
  \begin{enumerate}
...
\begin{definition}[Graph]
  A (directed) \index{\emph{graph}}~$G=\tup{V,A,s,t}$ consists of a set of vertices~$V$, a set of arrows~$A$, and two functions~$s,t\colon A\to V$, called the \emph{source} and \emph{target} functions, respectively. Given~$a\in A$ with~$s(a)=v$ and~$t(a)=w$, we say that~$a$ is an \emph{arrow} from~$v$ to~$w$.
\end{definition}
