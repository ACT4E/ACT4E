...
What is the relationship between relations and functions? One point of view is that functions are special kinds of relations. 
...
And what does this definition have to do with the ``usual'' way that we think about functions? 
...
$$
The passage from the function $f: X \rightarrow Y$ to the associated relation $R_f \subseteq X \times Y$ does not lose  The relation $R_f$ encodes the same information that is encoded in $f$ -- simply in a different form. 

In this course, we take Definition \ref{functions as relations} as our rigorous definition of a what a function is. Nevertheless, we'll often use functions ``in the usual way'', \text{e.g.} we'll write things like $y = f(x)$. 
