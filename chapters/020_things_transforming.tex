...
Furthermore, we might consider the idea of composition of arrows. If from~$Y$ we can get~$X$ and from~$Z$ we can get~$Y$, then from~$Z$ we can get~$X$. In our example, if the arrows \textsf{wheel} and \textsf{engine} exist, then also the arrow ``\textsf{engine then wheel}'' exists~(\cref{fig:e4}).
