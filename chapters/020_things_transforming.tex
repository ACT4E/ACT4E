...
Furthermore, we might consider the idea of composition of arrows. If from~$Y$ we can get~$X$ and from~$Z$ we can get~$Y$, then from~$Z$ we can get~$X$. In our example, if the arrows \textsf{wheel} and \textsf{engine} exist, then also the arrow ``\textsf{wheel then engine}'' exists~(\cref{fig:e4}).
...
\item \emph{Morphisms}: for every pair of objects~$X,Y\in \ObC$, there is a set~$\CatC(X,Y)$, elements of which are called
...
morphism of~$X$}.
\item \emph{Composition operations}: given any morphism~$f \in  \CatC(X,Y)$ and any morphism~$g \in \CatC(Y, Z)$, there exists a morphism~$f\then g$ in $\CatC(X,Z)$ which is the \emph{composition of~$f$ and~$g$}.
\end{compactenum}

Furthermore, constituents are required to satisfy the following conditions:
\begin{compactenum}
    \item \emph{Unitality}: for any morphism~$f\in\CatC(X,Y)$,
    \begin{equation}
        \id_X \then f= f = f \then \id_Y.
    \end{equation}
    \item \emph{Associativity}: for~$f\in \CatC(X,Y)$,~$g\in \CatC(Y,Z)$, and~$h\in \CatC(Z,W)$,
    \begin{equation}
...
We start with a set~$\Cat{C}$ of labels for all the currencies we wish to consider, \text{i.e.} 
\begin{equation*}
...
\begin{definition}[Category $\Cat{Curr}$]
...
\begin{figure}[h]
\begin{center}
    \begin{tikzcd}[column sep = 5cm, row sep = 2cm]
...
\gray{The category $\Cat{Curr}$ represents the set of all possible currency exchangers that could
...
The composition of morphisms is given by concatenation of paths, and for any object~$x \in V$, the associated identity morphism~$\id_x$ is the trivial path which starts and ends at $x$.-–≠≠––≠\end{definition}
\end{shaded*}
