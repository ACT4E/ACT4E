...
a ``design problem with implementation'', which is a tuple of ``\Fcol{functionality} space'', ``\Icol{implementation} space'', and ``\Rcol{resources} space'', together with two maps that describe the feasibility relations between these three spaces~(\cref{fig:setup}).
\begin{definition}
  \label{def:DPI}A \emph{\iindex{design problem with implementation}} (DPI) is a tuple
  \begin{equation}
...
This means that the functionality and resources spaces
