...
\item $\ressp$ is a poset, called \emph{\Rcol{resources} space};
...
by \Fcol{torque} and \Fcol{speed}. The \emph{resources} to consider
could include the \Rcol{cost {[}\${]}}, the \Rcol{mass {[}g{]}}, the
input \Rcol{voltage {[}V{]}}, and the input \Rcol{current {[}A{]}}.
...
be formalized as ``the ability to transport a certain \Fcol{payload
{[}g{]}}'' and ``at a given \Fcol{speed {[}m/s{]}}''. More refined
functional requirements would include maneuverability, the cargo volume,
etc. The resources to consider could be the \Rcol{cost {[}\${]}} of
the chassis; the total mass; and, for each motor to be placed in the
chassis, the required \Rcol{speed {[}rad/s{]}} and \Rcol{torque {[}Nm{]}}.
\end{example}
...
\begin{example}
The \Fcol{functionality} of a DC motor~(\figref{dc_motor})
is to provide a certain \Fcol{speed} and \Fcol{torque}, and the \Rcol{resources}
are \Rcol{current} and \Rcol{voltage}.
\end{example}
...
\begin{example}
A gearbox (\figref{gearbox}) provides a certain \Fcol{output
torque $\tau_o$} and \Fcol{speed $\tau_o$}, given a certain
\Rcol{input torque $\tau_i$} and \Rcol{speed $\omega_i$}. For
...
\begin{example}
\emph{Propellers}~(\figref{propeller}) generate \Fcol{thrust}
given a certain \Rcol{torque} and \Rcol{speed}.
\end{example}
...
\begin{example}
A \emph{crank-rocker} (\figref{crack}) converts \Rcol{rotational
motion} into a \Fcol{rocking motion}.
\end{example}
...
\Fcol{accuracy} or \Fcol{robustness} as functionality, and \Rcol{computation}
or \Rcol{sensing} as resources. Typically these bounds are known in
...
relation between the \Fcol{robustness} (1 - probability of failure),
the \Fcol{accuracy}, and the \Rcol{number of particles}~(\figref{gmapping}).
\end{example}
...
refined, are described by a smooth relation between the \Fcol{resolution}
and the \Rcol{latency} to obtain the answer~(\figref{progressive}).
...
\begin{example}
The empirical characterization of the monotone relation between \Fcol{the
accuracy of a visual SLAM solution} and \Rcol{the power consumption}
...
provides the \Fcol{"wireless access"} functionality, and requires
that the infrastructure provides the \Rcol{"Ethernet access"} resource.
\end{example}
...
a certain \Fcol{bandwidth}. Further refinements could include bounds
...
to achieve, one needs to choose "\Fcol{\emph{minus} the latency}"
and "\Fcol{\emph{minus} the packet drop probability}" for them
to count as functionality. As for the resources, apart from the \Rcol{transmission
power [W]}, one should consider at least \Rcol{the spectrum occupation},
...
\Fcol{to sweep an area}. If the functionality is fixed, one expects
...
functionalities are \Fcol{the throughput $T$ [Hz]} and \Fcol{the
application graph $A$}, and the \Rcol{resource graph $R$} is the
resource.
