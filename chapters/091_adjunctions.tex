...
$$
The category $F(G(\CatC))$ is larger than $\CatC$: starting with \CatC, the graph $G(\CatC)$ will contain edges for all the morphisms in $\CatC$, but it will forget their compositional interlinking. In particular, for example, it will forget which loops denote identity morphisms (\text{i.e. which morphisms act neutrally}) and it will forget when different compositions of morphism give the same result. In $F(G(\CatC))$, then, morphism compositions that might have given the same result in $\CatC$ will now be distinct. 
