...
\begin{definition}[Upper closure operator]
  ...
  \begin{equation}
    \begin{aligned}
      \upit  \colon \powerset(P)&\to \uppersets P\\
...
\end{lemma}
\begin{proof}
  Consider the posets~$\tup{\powerset(P),\subseteq}$ and~$\tup{\uppersets P,\supseteq}$, and~$S_1,S_2\in \powerset(P)$. It is clear that given~$S_1\subseteq S_2$, one has
  \begin{equation*}
...
\begin{definition}[Lower closure operator]
  ...
  \begin{equation*}
    \begin{aligned}
      \downit \colon \powerset(P)&\to \lowersets P\\
...
\begin{lemma}
  Given a poset~$\tup{P,\ordleq}$,~$\tupp{\antichains P,\ordleq_{\antichains P}}$ is a poset with
  \begin{equation}
