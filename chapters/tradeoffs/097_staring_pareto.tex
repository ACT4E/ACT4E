...
\end{remark}
\begin{remark}
  Note that given a poset~$\tup{\posA,\posAleq}$,~$\emptyset \in \antichains\posA$ since~$\emptyset$ contains no elements (and hence no comparable elements).
\end{remark}
...
\begin{example}
  Let's consider the poset~$\tup{\posA,\posAleq}$ where~$a\posAleq b$ if~$a$ is a divisor of~$b$ and~$P=\{1,5,10,11,13,15\}$. A chain of~$\posA$ is~$\{1,5,10,15\}$. An antichain of~$\posA$ is~$\{10,11,13\}$.
\end{example}
