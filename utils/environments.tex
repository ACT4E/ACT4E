...
% Theorem-related environments moved to environments-theorems.tex
...
\colorlet{thmshadecolor}{purple!10!blue!10}

\surroundwithmdframed[
    backgroundcolor=thmshadecolor,
    roundcorner=4pt,
    linewidth=0pt,
    innertopmargin=-1pt
]{theorem}
...
% ctdefinition moved to environments-theorems.tex
...
\colorlet{ctshadecolor}{SkyBlue!30}
...
% gradedexercise moved to environments-theorems.tex
...
% codeexercise moved to environments-theorems.tex
...
% Same shading as before, but the box is now kept inside \linewidth, so that
% it lines up with the exercise environments (homework, gradedexercise,
% examexercise, ...), which are mdframed boxes exactly \linewidth wide.
% The stock "shaded" environment of the framed package instead keeps the text
% at full measure and lets the coloured box bleed \FrameSep (= 3\fboxsep = 9pt)
% into both margins, which made every solution look wider than the exercise it
% belongs to.  The \advance\hsize-\width below is exactly what the framed
% package's own "framed" environment does.
% NB: we deliberately stay with the framed package rather than switching to
% mdframed here: a rounded mdframed box cannot break across pages, and
% solutions frequently do.
\newenvironment{gradedsolution}
{\colorlet{shadecolor}{LightPink!20}%
...
\setboolean{video}{false} % Set to true or false as needed
\newcommand{\linkvideo}[1]{%
    \ifthenelse{\boolean{video}}{% If 'video' is true
        \StrSubstitute{#1}{:}{_}[\result]%
        \input{\rootdir/videos/generated/\result/watch}%
    }{% If 'video' is false, do nothing or provide alternative action
        % You can provide alternative action here, or leave it empty to do nothing
    }%
}

%\setboolean{video}{false}
%\newcommand{\linkvideo}[1]{%
%    \StrSubstitute{#1}{:}{_}[\result]
%    \input{\rootdir/videos/generated/\result/watch}%
%}
...
% Helper macros for SYN warning messages (to avoid duplication)
\newcommand{\SYNwarningcannotmatch}[1]{%
    \typeoutoncekey{cannotmatch@#1}{SYN: Warning: cannot match the value (#1) to any existing entry.}%
}
\newcommand{\SYNwarninglabelnotfound}[1]{%
    \typeoutoncekey{labelnotfound@\pgfkeysvalueof{/mydefs/#1/label}}{SYN: Warning: label (\pgfkeysvalueof{/mydefs/#1/label}) is not found in this document.}%
}
\newcommand{\SYNwarningnolabel}[1]{%
    \typeoutoncekey{nolabel@#1}{SYN: Warning: there is no label associated to (#1).}%
}

% Typeout once functionality
\newcommand{\typeoutonce}[1]{%
    \expandafter\ifx\csname typeoutonce@#1\endcsname\relax%
        \typeout{#1}%
        \expandafter\gdef\csname typeoutonce@#1\endcsname{1}%
    \fi%
}

% Alternative version for parameterized messages
\newcommand{\typeoutoncekey}[2]{%
    \expandafter\ifx\csname typeoutonce@#1\endcsname\relax%
        \typeout{#2}%
        \expandafter\gdef\csname typeoutonce@#1\endcsname{1}%
    \fi%
    ...
    \pgfkeysifdefined{/mydefs/#1/index}{%
        \pgfkeysifdefined{/mydefs/#1/label}{%
            \ifelselabeldefined{\pgfkeysvalueof{/mydefs/#1/label}}{%
                % \typeout{SYN: linking to label (\pgfkeysvalueof{/mydefs/#1/label})}%
                \texorpdfstring{%
