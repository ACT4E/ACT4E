...
\newcommand{\sectionexercises}[1]{%
    \section[Code exercises - #1]{\texorpdfstring{\usebox{\chaptergear}}{Code exercises -} #1}
}
...
% AMSa font is only available with pdfTeX + amssymb
% For XeTeX with unicode-math, we use the Unicode minus sign
\ifx\XeTeXversion\undefined
  \DeclareMathSymbol{\shortminus}{\mathbin}{AMSa}{"39}
\else
  % With unicode-math, use the Unicode minus sign
...
\DeclareRobustCommand{\makelist}[1]{%
    \@makelists{\llbracket}{\commaspace}{\rrbracket}{#1}%
}

\newcommand{\smashsepcolor}{\mathcolor{formulasetcolor}\mid}
\newcommand{\smashseparator}{{\,\smashsepcolor\,}}

\DeclareRobustCommand{\makesmash}[1]{%
    \@makelists{\cleft[formulasetcolor]{[}}{\smashseparator}{\cright[formulasetcolor]{]}}{#1}%
}
...
\definecolor{level2}{rgb}{0.8,0.0,0.0}
\definecolor{level3}{rgb}{0.0,0.0,0.8}

\DeclareRobustCommand{\mF}[1]{% % monad formula without brackets
    \@makelists{}{\ast}{}{#1}%
}
\DeclareRobustCommand{\mFb}[1]{% % monad formula without brackets
    \@makelists{}{\mathcolor{level2}{\ast}}{}{#1}%
}
\DeclareRobustCommand{\MF}[1]{% % monad formula
    \@makelists{\mathopen{}\left[}{
        \mathcolor{level2}{\ast}%
    }{\right]\mathclose{}}{#1}%
}
\DeclareRobustCommand{\MFb}[1]{% % monad formula
    \@makelists{%
        \mathcolor{level2}{[}%
    }{%
        \mathcolor{level3}{\ast}
    }{%
        \mathcolor{level2}{]}%
    }{#1}%
}
\DeclareRobustCommand{\MFc}[1]{% % monad formula
    \@makelists{%
        \mathcolor{level3}{[}%
    }{\ast}{%
        \mathcolor{level3}{]}%
...
\newcommand{\writerMonoid}{\styleobj{L}}
\newcommand{\writerM}{\stylefunctors{W}}
...
\newcommand{\TriangleDiagramMapsto}[6]{%
    \begin{tikzcd}[every arrow/.append style={mapsto}]%
        {#1} \arrow[r, "{#2}"] \arrow[dr, "{#6}"] \pgfmatrixnextcell {#3}  \arrow[d, "{#4}"] \\
        \pgfmatrixnextcell {#5}%
    \end{tikzcd}%
